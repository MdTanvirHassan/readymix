...
class Sales_report extends Site_Controller {
    ...
    function targetReport($print = false) {
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sales_report';
        $this->titlebackend("Report");
        ...
        if (!empty($postData)) {
            ...
            $ae_sql = "select e.*,d.designation_name from employees e left join designation d on e.designation_id=d.id";
            $data['all_employees'] = $this->m_common->customeQuery($ae_sql);
            if (!empty($employee_id)) {
                $e_sql = "select e.*,d.designation_name from employees e left join designation d on e.designation_id=d.id where e.id=" . $employee_id;
                $data['employees'] = $this->m_common->customeQuery($e_sql);
            } else {
                $e_sql = "select e.*,d.designation_name from employees e left join designation d on e.designation_id=d.id";
                $data['employees'] = $this->m_common->customeQuery($e_sql);
            }
            foreach ($data['employees'] as $key => $value) {
                $sql = "select tstd.*,tpc.category_name,tpc.measurement_unit from tbl_sales_target_details tstd left join tbl_sales_target tst on tstd.st_id=tst.st_id left join tbl_product_categories tpc on tstd.category_id=tpc.category_id where tst.employee_id=" . $value['id'] . " and tst.month='" . $month . "' and tstd.is_active=1";
                $data['employees'][$key]['products'] = $this->m_common->customeQuery($sql);
                foreach ($data['employees'][$key]['products'] as $key1 => $v) {
                    $data['employees'][$key]['products'][$key1]['achieved_qty'] = $v['quantity'];
                    $data['employees'][$key]['products'][$key1]['achieved_amount'] = $v['amount'];
                }
            }
        ...
        if($print == false){
            $this->load->view('sales_report/v_all_sales_target', $data);
        }else{
            $html = $this->load->view('sales_report/print_all_sales_target', $data, true);
            echo $html;
            exit;
        }
    }

    function targetReportExcel($print = false) {
        ...
        $object->getActiveSheet()->setCellValueByColumnAndRow(8, 3, 'Sales Target Report');
        $style = array(
            'alignment' => array(
        ...
        $table_columns = array("SL", "EMPLOYEE", "DESIGNATION", "PRODUCT", "MU", "RATE", "QNTY", "VALUE", "TOTAL");
        $object->setActiveSheetIndex(0)->mergeCells('L4:N4');
        $object->getActiveSheet()->setCellValueByColumnAndRow(11, 4, 'TARGET');
        $column = 5;
        ...
        if (!empty($data['employees'])) {
            ...
            foreach ($data['employees'] as $employee) {
                ...
                $object->getActiveSheet()->setCellValueByColumnAndRow(6, $excel_row, $employee['name']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row, $employee['designation_name']);
                ...
                if (!empty($employee['products'])) {
                    $target_total = 0;
                    $achivement_total = 0;
                    $c = count($employee['products']);
                    $j = 0;

                    foreach ($employee['products'] as $pro) {
                        $j++;
                        $target_total = $target_total + $pro['amount'];
                        $achivement_total = $achivement_total + $pro['achieved_amount'];

                        $g_target_total = $g_target_total + $pro['amount'];
                        $g_target_total_achievement = $g_target_total_achievement + $pro['achieved_amount'];
                        if ($j == $c) {
                            ...
                            $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, $pro['category_name']);
                            $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, $pro['measurement_unit']);
                            $object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, $pro['unit_price']);
                            $object->getActiveSheet()->setCellValueByColumnAndRow(11, $excel_row,round($pro['quantity'], 2));
                            $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row,round($pro['amount'], 2));
                            $object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row,round($target_total, 2));
                        } else {
                            $object->getActiveSheet()->setCellValueByColumnAndRow(5, $excel_row, '');
                            $object->getActiveSheet()->setCellValueByColumnAndRow(6, $excel_row, '');
                            $object->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row, '');
            ...
            $object->getActiveSheet()->setCellValueByColumnAndRow(12,$excel_row,'Grand Total');
            $object->getActiveSheet()->setCellValueByColumnAndRow(13,$excel_row,round($g_target_total, 2));
        }
        $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row, 'Grand Total');
        $object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row,round($g_target_total, 2));

        $object->getActiveSheet()->getStyle("M$excel_row:N$excel_row")->getFont()->setSize('12');
        $object->getActiveSheet()->getStyle("M$excel_row:N$excel_row")->getFont()->setBold(true);
    ...
    function targetAchievementReport($print = false) {
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sales_report';
        $this->titlebackend("Report");
        ...
        if (!empty($postData)) {
            ...
            $first_date = $month . '-' . date('01', strtotime($month));
            $last_date = $month . '-' . date('t', strtotime($month));
            ...
            foreach ($data['employees'] as $key => $value) {
                $sql1 = "select tstd.*,tpc.category_name,tpc.measurement_unit from tbl_sales_target_details tstd left join tbl_sales_target tst on tstd.st_id=tst.st_id left join tbl_product_categories tpc on tstd.category_id=tpc.category_id where tst.employee_id=" . $value['id'] . " and tst.month='" . $month . "' and tstd.is_active=1";
                $data['employees'][$key]['products'] = $this->m_common->customeQuery($sql1);
                foreach ($data['employees'][$key]['products'] as $key1 => $v) {
                    //  $sql="select sum(tsod.quantity) as total_qty,sum(tsod.amount) as total_amount from tbl_sales_order_details tsod left join tbl_sales_orders tso on tsod.o_id=tso.o_id left join tbl_sales_products tsp on tsod.product_id=tsp.product_id left join tbl_product_categories tpc on tsp.category_id=tpc.category_id where tsp.category_id=".$v['category_id']." and tso.sale_person_id=".$value['id']." and tsod.is_active=1";
                    $sql = "select sum(tsod.quantity) as total_qty,sum(tsod.amount) as total_amount from tbl_sales_order_details tsod left join tbl_sales_orders tso on tsod.o_id=tso.o_id left join tbl_sales_products tsp on tsod.product_id=tsp.product_id left join tbl_product_categories tpc on tsp.category_id=tpc.category_id where tsp.category_id=" . $v['category_id'] . " and tso.sale_person_id=" . $value['id'] . " and tsod.is_active=1 and tsp.product_name!='Grouting' and (tso.sale_order_date>='$first_date' and tso.sale_order_date<='$last_date')";
                    $sale = $this->m_common->customeQuery($sql);
                    if (!empty($sale)) {
                        $data['employees'][$key]['products'][$key1]['achieved_qty'] = $sale[0]['total_qty'];
                        $data['employees'][$key]['products'][$key1]['achieved_amount'] = $sale[0]['total_amount'];
                    } else {
                        $data['employees'][$key]['products'][$key1]['achieved_qty'] = 0;
                        $data['employees'][$key]['products'][$key1]['achieved_amount'] = 0;
                    }
                }
            }
        ...
        if ($print == false) {
            $this->load->view('sales_report/v_all_sales_target_achievement', $data);
        } else {
            $html = $this->load->view('sales_report/print_all_sales_target_achievement', $data, true);
            echo $html;
            exit;
        }
    }

    function targetAchievementReportExcel($print = false) {
        $this->load->library("PHPExcel");
        $branch_id = $this->session->userdata('companyId');
        $data['branch_info'] = $this->m_common->get_row_array('department', array('d_id' => $branch_id), '*');
        $where = '';
        ...
        $object->getActiveSheet()->getStyle('I3')->getFont()->setSize('13');
        $object->getActiveSheet()->getStyle('I3')->getFont()->setBold(true);
        $object->getActiveSheet()->setCellValueByColumnAndRow(8, 3, 'Sales Target Achievement Report');
        $style = array(
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
            )
        );
        $object->getActiveSheet()->getStyle('I1:U3')->applyFromArray($style);

        $table_columns = array("SL", "EMPLOYEE", "DESIGNATION", "PRODUCT", "MU", "RATE", "QNTY", "VALUE", "TOTAL", "QNTY", "VALUE", "TOTAL", "%", "VALUE", "TOTAL", "%");

        $object->setActiveSheetIndex(0)->mergeCells('L4:N4');
        $object->getActiveSheet()->setCellValueByColumnAndRow(11, 4, 'TARGET');
        $object->setActiveSheetIndex(0)->mergeCells('O4:R4');
        $object->getActiveSheet()->setCellValueByColumnAndRow(14, 4, 'ACHIEVEMENT');
        $object->setActiveSheetIndex(0)->mergeCells('S4:U4');
        $object->getActiveSheet()->setCellValueByColumnAndRow(18, 4, 'VARIANCE');
        $object->getActiveSheet()->getStyle('L4:U4')->getFont()->setSize('12');
        $object->getActiveSheet()->getStyle('L4:U4')->getFont()->setBold(true);
        ...
        if (!empty($data['employees'])) {
                ...
                if (!empty($employee['products'])) {
                    ...
                    foreach ($employee['products'] as $pro) {
                        ...
                        if ($j == $c) {
                            ...
                            $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row,round($pro['amount'],2));
                            $object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row,round($target_total,2));
                            $object->getActiveSheet()->setCellValueByColumnAndRow(14, $excel_row,round($pro['achieved_qty'],2));
                            $object->getActiveSheet()->setCellValueByColumnAndRow(15, $excel_row,round($pro['achieved_amount'],2));
                            $object->getActiveSheet()->setCellValueByColumnAndRow(16, $excel_row,round($achivement_total,2));

                            $object->getActiveSheet()->setCellValueByColumnAndRow(17, $excel_row, round(($pro['achieved_amount'] * 100 / $pro['amount']), 2));
                            $object->getActiveSheet()->setCellValueByColumnAndRow(18, $excel_row, $pro['quantity'] - $pro['achieved_qty']);
                            $object->getActiveSheet()->setCellValueByColumnAndRow(19, $excel_row, $pro['amount'] - $pro['achieved_amount']);
                            $d = $pro['amount'] - $pro['achieved_amount'];
                            $object->getActiveSheet()->setCellValueByColumnAndRow(20, $excel_row, round(($d * 100 / $pro['amount']), 2));
                        } else {
                            $object->getActiveSheet()->setCellValueByColumnAndRow(5, $excel_row, '');
                            $object->getActiveSheet()->setCellValueByColumnAndRow(6, $excel_row, '');
                            $object->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row, '');
                            ...
                            $object->getActiveSheet()->setCellValueByColumnAndRow(17, $excel_row, round(($pro['achieved_amount'] * 100 / $pro['amount']), 2));
                            $object->getActiveSheet()->setCellValueByColumnAndRow(18, $excel_row, round($pro['quantity'] - $pro['achieved_qty']), 2);
                            $object->getActiveSheet()->setCellValueByColumnAndRow(19, $excel_row, round($pro['amount'] - $pro['achieved_amount']), 2);
                            $d = $pro['amount'] - $pro['achieved_amount'];
                            $object->getActiveSheet()->setCellValueByColumnAndRow(20, $excel_row, round(($d * 100 / $pro['amount']), 2));
                        }
                        $excel_row++;
            ...
            $object->getActiveSheet()->setCellValueByColumnAndRow(14, $excel_row,'');
            $object->getActiveSheet()->setCellValueByColumnAndRow(15, $excel_row,'');
            $object->getActiveSheet()->setCellValueByColumnAndRow(16, $excel_row,round($g_target_total_achievement, 2));
            $object->getActiveSheet()->setCellValueByColumnAndRow(17, $excel_row,'');
            $object->getActiveSheet()->setCellValueByColumnAndRow(18, $excel_row,'');
            $object->getActiveSheet()->setCellValueByColumnAndRow(19, $excel_row,'');
            $object->getActiveSheet()->setCellValueByColumnAndRow(20, $excel_row,'');
        ...
        $object->getActiveSheet()->getStyle("M$excel_row:U$excel_row")->getFont()->setSize('12');
        $object->getActiveSheet()->getStyle("M$excel_row:U$excel_row")->getFont()->setBold(true);
        ...
        $object->getActiveSheet()->getStyle('F5:U' . $excel_row)->applyFromArray($styleArray);
        $object->getActiveSheet()->getStyle('N6:U'.$excel_row)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
    ...
    function allSalesOrderExcel($print = false) {
        $this->load->library("PHPExcel");
        $branch_id = $this->session->userdata('companyId');
        $data['branch_info'] = $this->m_common->get_row_array('department', array('d_id' => $branch_id), '*');
        $where = '';
        ...
        $object->getActiveSheet()->getStyle('I3')->getFont()->setSize('13');
        $object->getActiveSheet()->getStyle('I3')->getFont()->setBold(true);
        $object->getActiveSheet()->setCellValueByColumnAndRow(8, 3, 'Sales Orders');
        $style = array(
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
            )
        ...
        $table_columns = array("SL", "S. Order Date.", "S. Order No.", "Customer Name", "Project", "Product Name", "M. Unit", "Quantity", "Unit Price", "Value");
        ...
        if (!empty($data['orders'])) {
            ...
            foreach ($data['orders'] as $order) {
                ...
                $object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, $order['product_name']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(11, $excel_row, $order['measurement_unit']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row, round($order['quantity'], 2));
                $object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row, round($order['unit_price'], 2));
                $object->getActiveSheet()->setCellValueByColumnAndRow(14, $excel_row, round($order['amount'], 2));

                $excel_row++;
            }
        }
        $object->getActiveSheet()->setCellValueByColumnAndRow(11, $excel_row, 'Total');
        $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row, round($total_qty, 2));
        ...
        $object->getActiveSheet()->getStyle('M6:O'.$excel_row)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
    ...
    function doBalanceReport($print = false) {
        ...
        if (!empty($postData)) {
            ...
            if (!empty($customer_id) && !empty($project_id)) {
                $data['all_orders'] = $this->m_common->get_row_array('tbl_sales_orders', array('project_id' => $project_id, 'customer_id' => $customer_id, 'is_active' => 1), '*');
            } else if (!empty($customer_id)) {
                $data['all_orders'] = $this->m_common->get_row_array('tbl_sales_orders', array('customer_id' => $customer_id, 'is_active' => 1), '*');
            } else if (!empty($project_id)) {
                $data['all_orders'] = $this->m_common->get_row_array('tbl_sales_orders', array('project_id' => $project_id, 'is_active' => 1), '*');
            } else {
                $data['all_orders'] = $this->m_common->get_row_array('tbl_sales_orders', array('is_active' => 1), '*');
            }
            ...
            if (!empty($f_date) & !empty($to_date)) {
                $sql = "select sod.*,so.project_name,so.order_no,so.sale_order_date,c.c_name,p.product_name,p.measurement_unit,tpc.category_name from tbl_sales_order_details sod left join tbl_sales_orders so on  sod.o_id=so.o_id left join tbl_sales_quotation sq on so.q_id=sq.q_id left join tbl_customers c on so.customer_id=c.id left join tbl_sales_products p on sod.product_id=p.product_id left join tbl_product_categories tpc on p.category_id=tpc.category_id where $where and sod.is_active=1 and so.sale_order_date>='" . $from_date . "' and so.sale_order_date<='" . $too_date . "' order by so.sale_order_date DESC";
            } else if (!empty($f_date)) {
                $sql = "select sod.*,so.project_name,so.order_no,so.sale_order_date,c.c_name,p.product_name,p.measurement_unit,tpc.category_name from tbl_sales_order_details sod left join tbl_sales_orders so on  sod.o_id=so.o_id left join tbl_sales_quotation sq on so.q_id=sq.q_id left join tbl_customers c on so.customer_id=c.id left join tbl_sales_products p on sod.product_id=p.product_id left join tbl_product_categories tpc on p.category_id=tpc.category_id where $where and sod.is_active=1 and so.sale_order_date>='" . $from_date . "' order by so.sale_order_date DESC";
            } else if (!empty($to_date)) {
                $sql = "select sod.*,so.project_name,so.order_no,so.sale_order_date,c.c_name,p.product_name,p.measurement_unit,tpc.category_name from tbl_sales_order_details sod left join tbl_sales_orders so on  sod.o_id=so.o_id left join tbl_sales_quotation sq on so.q_id=sq.q_id left join tbl_customers c on so.customer_id=c.id left join tbl_sales_products p on sod.product_id=p.product_id left join tbl_product_categories tpc on p.category_id=tpc.category_id where $where and sod.is_active=1 and so.sale_order_date<='" . $too_date . "' order by so.sale_order_date DESC";
            } else {
                $sql = "select sod.*,so.project_name,so.order_no,so.sale_order_date,c.c_name,p.product_name,p.measurement_unit,tpc.category_name from tbl_sales_order_details sod left join tbl_sales_orders so on  sod.o_id=so.o_id left join tbl_sales_quotation sq on so.q_id=sq.q_id left join tbl_customers c on so.customer_id=c.id left join tbl_sales_products p on sod.product_id=p.product_id left join tbl_product_categories tpc on p.category_id=tpc.category_id where $where and sod.is_active=1 order by so.sale_order_date DESC";
            }
            $data['orders'] = $this->m_common->customeQuery($sql);
            // $data['all_orders']=$this->m_common->get_row_array('tbl_sales_orders',array('is_active'=>1),'*');
            $data['products'] = $this->m_common->get_row_array('tbl_sales_products', array('is_active' => 1), '*');
            ...
            foreach ($data['orders'] as $key1 => $orders) {
                $sql = "select sum(dcd.quantity) as total_challan_qty from tbl_delivery_challan_details dcd left join tbl_delivery_challans dc on dcd.dc_id=dc.dc_id left join tbl_delivery_orders do on  dc.do_id=do.do_id left join tbl_sales_orders so on do.o_id=so.o_id  where dcd.s_item_id=" . $orders['product_id'] . " and so.o_id=" . $orders['o_id'] . " and dcd.is_active=1 and dc.status='Approved'";
                $total_challan_qty = $this->m_common->customeQuery($sql);
                $data['orders'][$key1]['total_challan_qty'] = $total_challan_qty[0]['total_challan_qty'];
            }
        } else {
        ...
        if ($print == false) {
            ...
            $html = $this->load->view('sales_report/print_do_balance_report', $data, true);
            echo $html;
            exit;
        }
    }
    ...
    function doBalanceReportExcel() {
        ...
        $table_columns = array("SL", "S. Order Date.", "S. Order No.", "Customer Name", "Project", "Product Name", "M. Unit", "So Order", "Do order", "Production", "Balance(DO)");
        ...
        if (!empty($data['orders'])) {
            ...
            foreach ($data['orders'] as $order) {
                ...
                $object->getActiveSheet()->setCellValueByColumnAndRow(11, $excel_row, $order['measurement_unit']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row, round($order['quantity'], 2));
                $object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row, round($order['do_qty'], 2));
                $object->getActiveSheet()->setCellValueByColumnAndRow(14, $excel_row, round($order['total_challan_qty'], 2));

                $due_c = round(($order['do_qty'] - $order['total_challan_qty']), 2);
                if ($order['product_name'] != 'Grouting') {
                    $total_chall_due_qty = $total_chall_due_qty + $due_c;
                }
                $object->getActiveSheet()->setCellValueByColumnAndRow(15, $excel_row, $due_c);
                $excel_row++;
            }
        }
        $object->getActiveSheet()->setCellValueByColumnAndRow(11, $excel_row,'Total');
        $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row,round($total_so_qty, 2));
        $object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row,round($total_do_qty, 2));

        $object->getActiveSheet()->setCellValueByColumnAndRow(14, $excel_row,round($total_chal_qty, 2));
        $object->getActiveSheet()->setCellValueByColumnAndRow(15, $excel_row,round($total_chall_due_qty, 2));
...
//           }
        ...
        $table_columns = array("SL", "C. Date.", "C. No.", "D.O.", "S.O.", "C.Name", "Project","Truck No", "D.Name","P.Type","P.Name", "M.Unit", "Quantity", "Unit Price", "Value","Bill Status");
    ...
    function allDeliveryChallanVat($print = false) {
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sales_report';
        $this->titlebackend("Report");
...
//           }
        ...
        if($print == false){
            $this->load->view('sales_report/v_vat_report', $data);
        }else{
            $html = $this->load->view('sales_report/print_vat_report', $data, true);
            echo $html;
            exit;
        }
    }

    function allDeliveryChallanVatExcel($print = false) {
        $this->load->library("PHPExcel");
        $branch_id = $this->session->userdata('companyId');
        $data['branch_info'] = $this->m_common->get_row_array('department', array('d_id' => $branch_id), '*');
        $where = '';
...
//            }
    ...
    //    $sql = "select pc.*,c.c_name,c_short_name,b.b_name,b.b_short_name,b.branch_name from tbl_payment_collections pc left join tbl_banks b on pc.bank_id=b.id  left join tbl_customers c on pc.customer_id=c.id  where pc.is_active=1 and pc.payment_status='Collected' and (collection_method='Pdc' or collection_method='Lc' or collection_method='Bg' or collection_method='Po') ";
        ...
        if(!empty($postData)){
            ...
            if($date_type=="Bank Deposit Date"){    
                if(!empty($f_date) & !empty($to_date)){              
                    $sql = "select pc.*,c.c_name,c_short_name,b.b_name,b.b_short_name,b.branch_name,tpc.category_name from tbl_payment_collections pc left join tbl_banks b on pc.bank_id=b.id  left join tbl_customers c on pc.customer_id=c.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and pc.bank_deposit_date>='" . $f_date . "' and pc.bank_deposit_date<='" . $to_date . "' and pc.payment_status='Collected' and (collection_method='Pdc' or collection_method='Lc' or collection_method='Bg' or collection_method='Po') order by pc.bank_deposit_date DESC";
                }else if (!empty($f_date)) {              
                    $sql = "select pc.*,c.c_name,c_short_name,b.b_name,b.b_short_name,b.branch_name,tpc.category_name from tbl_payment_collections pc left join tbl_banks b on pc.bank_id=b.id  left join tbl_customers c on pc.customer_id=c.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and pc.bank_deposit_date>='" . $f_date . "' and pc.payment_status='Collected' and (collection_method='Pdc' or collection_method='Lc' or collection_method='Bg' or collection_method='Po') order by pc.bank_deposit_date DESC";
                }else if (!empty($to_date)) {               
                    $sql = "select pc.*,c.c_name,c_short_name,b.b_name,b.b_short_name,b.branch_name,tpc.category_name from tbl_payment_collections pc left join tbl_banks b on pc.bank_id=b.id  left join tbl_customers c on pc.customer_id=c.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and pc.bank_deposit_date<='" . $to_date . "' and pc.payment_status='Collected' and (collection_method='Pdc' or collection_method='Lc' or collection_method='Bg' or collection_method='Po') order by pc.bank_deposit_date DESC";
                }else{                
                    $sql = "select pc.*,c.c_name,c_short_name,b.b_name,b.b_short_name,b.branch_name,tpc.category_name from tbl_payment_collections pc left join tbl_banks b on pc.bank_id=b.id  left join tbl_customers c on pc.customer_id=c.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and pc.payment_status='Collected' and (collection_method='Pdc' or collection_method='Lc' or collection_method='Bg' or collection_method='Po') order by pc.bank_deposit_date DESC";
                } 
            }else{
                if(!empty($f_date) & !empty($to_date)){              
                    $sql = "select pc.*,c.c_name,c_short_name,b.b_name,b.b_short_name,b.branch_name,tpc.category_name from tbl_payment_collections pc left join tbl_banks b on pc.bank_id=b.id  left join tbl_customers c on pc.customer_id=c.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and pc.receive_date>='" . $f_date . "' and pc.receive_date<='" . $to_date . "' and pc.payment_status='Collected' and (collection_method='Pdc' or collection_method='Lc' or collection_method='Bg' or collection_method='Po') order by pc.receive_date DESC";
    ...
    function handsToDepositExcel($print = false) {
        ...
        $object->getActiveSheet()->getStyle('I1:O3')->applyFromArray($style);

     //   $table_columns = array("SL", "S. Order", "Customer Name.", "Project", "Pdc/Lc/Bg No.", "Pdc/Lc/Bg Date", "Mat Date", "Bank", "Value");
        $table_columns = array("SL","Receive Date","Customer Name.","Product Type","Collection Method","Pdc/Lc/Bg No.", "Pdc/Lc/Bg Date","Bank Deposit Date","Bank", "Value");
        ...
        if (!empty($data['orders'])) {
            ...
            foreach ($data['orders'] as $order) {
                ...
                $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row, $order['bank_deposit_date']);

                $object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row, $order['b_name']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(14, $excel_row, round($order['amount'], 2));
        ...
        $object->getActiveSheet()->getStyle('F5:O'. $excel_row)->applyFromArray($styleArray);
        $object->getActiveSheet()->getStyle('N6:O'.$excel_row)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
   ...
   function dishonoredChequeAtHandExcel($print = false) {
    ...
    function customerBalanceReport($print=false){
        ...
        $data['all_customers']=$this->m_common->get_row_array('tbl_customers','','*','','','c_name','asc');
        $customer_id = $this->input->post('customer_id');
        $data['sales_person_id']=$sales_person_id=$this->input->post('sales_person_id');
        if(!empty($customer_id)){
            //$data['customers']=$this->m_common->get_row_array('tbl_customers',array('id'=>$customer_id),'*');
            $c_sql="select c.*,e.name from tbl_customers c left join employees e on c.sales_person_id=e.id where c.id=".$customer_id;
            $data['customers']=$this->m_common->customeQuery( $c_sql);
            $data['customer_id']=$customer_id;
        }else{
            if(!empty($sales_person_id)){
                //$data['customers']=$this->m_common->get_row_array('tbl_customers',array('sales_person_id'=>$sales_person_id),'*');
                $c_sql="select c.*,e.name from tbl_customers c left join employees e on c.sales_person_id=e.id where c.sales_person_id=".$sales_person_id;
                $data['customers']=$this->m_common->customeQuery( $c_sql);
            }else{
                //$data['customers']=$this->m_common->get_row_array('tbl_customers','','*');
                $c_sql="select c.*,e.name from tbl_customers c left join employees e on c.sales_person_id=e.id";
                $data['customers']=$this->m_common->customeQuery( $c_sql);
            }
            $data['customer_id']='';
        }
        foreach($data['customers'] as $key=>$value){
           ...
           // $deposit=$total_received[0]['total_amount']+$value['opening_balance']; 
            ...
            $pending_bill_sql = "select sum(dcd.amount) as total_amount from tbl_delivery_challan_details dcd left join tbl_delivery_challans dc on dcd.dc_id=dc.dc_id left join tbl_delivery_orders do on  dc.do_id=do.do_id left join tbl_sales_orders so on do.o_id=so.o_id where so.customer_id=$id and dcd.is_active=1 and dc.status='Approved' and dcd.bill_status='Pending' ";
            $total_pending_bill=$this->m_common->customeQuery($pending_bill_sql);
            $data['customers'][$key]['total_pending_bill']=$total_pending_bill[0]['total_amount']; 
...
//            $data['in_hand'] = $in_hand[0]['total_amount'];
    ...
    //            $data['in_hand'] = $in_hand[0]['total_amount'];
         ...
         //   $table_columns = array("SL", "S. Order", "Customer Name.", "Project", "Pdc/Lc/Bg No.", "Pdc/Lc/Bg Date", "Mat Date", "Bank", "Value");
            $table_columns = array("SL", "Customer Name","Adjustment","Vat","Ait","Payment Received","Total Deposit","Total Bill","Pending Bill","Due", "Surplus");
            ...
            if (!empty($data['customers'])) {
                ...
                foreach ($data['customers'] as $cust) {
                    ...
                    $object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row, $cust['total_pending_bill']);
                    if($due>0){
                        $object->getActiveSheet()->setCellValueByColumnAndRow(14, $excel_row, $due);
                    }else{
                        $object->getActiveSheet()->setCellValueByColumnAndRow(14, $excel_row,'');
                    }
                    if($due<0){
                        $object->getActiveSheet()->setCellValueByColumnAndRow(15, $excel_row,($cust['deposit']-($cust['total_bill']+$cust['total_pending_bill'])));
                    }else{
                        $object->getActiveSheet()->setCellValueByColumnAndRow(15, $excel_row,'');
                    }
            ...
            $object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row,$net_total_pending_bill);
            $object->getActiveSheet()->setCellValueByColumnAndRow(14, $excel_row,$net_due);
            $object->getActiveSheet()->setCellValueByColumnAndRow(15, $excel_row,$net_balance);
    ...
    function deposit_statement($print = false) {
        ...
        $postData = $this->input->post();
           ...
           if(!empty($postData)){
            ...
            if (!empty($postData['deposit_id'])) {
                $l=0;
                foreach($postData['deposit_id'] as $value1){
                    $l++;
                    if($l==1){
                        if($l==count($postData['deposit_id'])){
                            $where .= '(dr.id=' . "'" . $value1 . "')";
                        }else{
                            $where .= '(dr.id=' . "'" . $value1 . "'";
                        }
                        ...
                        if($l==count($postData['deposit_id'])){
                            $where .= ' OR dr.id=' . "'" . $value1 . "')";
                        }else{
                            $where .= ' OR dr.id=' . "'" . $value1 . "'";
                        }
                    }
                }
                $data['searched']=$postData;
            }else{
                $data['searched']='';
            }    
                ...
                $data['priority']=$priority=$this->input->post('priority');
                if($priority=="All"){
                    if(!empty($postData['bank_id'])){
                        if(!empty($where)){
                            $sql = "select dr.*,(select db.b_name from tbl_banks db where id=dr.bank_id)as d_bank_name,pc.check_date,pc.bank_deposit_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name from tbl_deposit_realization dr left join tbl_payment_collections pc on dr.collection_id=pc.id left join tbl_banks b on pc.bank_id=b.id left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and dr.bank_id='".$postData['bank_id']."' and dr.realization_status='Deposited' order by dr.deposit_date DESC";
                        }else{
                        ...
                        if(!empty($where)){
                            ...
                            $sql = "select dr.*,(select db.b_name from tbl_banks db where id=dr.bank_id)as d_bank_name,pc.check_date,pc.bank_deposit_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name from tbl_deposit_realization dr left join tbl_payment_collections pc on dr.collection_id=pc.id left join tbl_banks b on pc.bank_id=b.id left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where dr.realization_status='Deposited' order by dr.deposit_date DESC";
                        }    
                    }
                }else{
                    if(!empty($postData['bank_id'])){
                        if(!empty($where)){
                            $sql = "select dr.*,(select db.b_name from tbl_banks db where id=dr.bank_id)as d_bank_name,pc.check_date,pc.bank_deposit_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name from tbl_deposit_realization dr left join tbl_payment_collections pc on dr.collection_id=pc.id left join tbl_banks b on pc.bank_id=b.id left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and dr.priority='".$priority."' and dr.bank_id='".$postData['bank_id']."' and dr.realization_status='Deposited' order by dr.deposit_date DESC";
                        }else{
                            $sql = "select dr.*,(select db.b_name from tbl_banks db where id=dr.bank_id)as d_bank_name,pc.check_date,pc.bank_deposit_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name from tbl_deposit_realization dr left join tbl_payment_collections pc on dr.collection_id=pc.id left join tbl_banks b on pc.bank_id=b.id left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where dr.priority='".$priority."' and dr.bank_id='".$postData['bank_id']."' and dr.realization_status='Deposited' order by dr.deposit_date DESC";     
                        }
                    }else{
                        if(!empty($where)){
                            $sql = "select dr.*,(select db.b_name from tbl_banks db where id=dr.bank_id)as d_bank_name,pc.check_date,pc.bank_deposit_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name from tbl_deposit_realization dr left join tbl_payment_collections pc on dr.collection_id=pc.id left join tbl_banks b on pc.bank_id=b.id left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and dr.priority='".$priority."' and dr.realization_status='Deposited' order by dr.deposit_date DESC";
                        }else{
                            $sql = "select dr.*,(select db.b_name from tbl_banks db where id=dr.bank_id)as d_bank_name,pc.check_date,pc.bank_deposit_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name from tbl_deposit_realization dr left join tbl_payment_collections pc on dr.collection_id=pc.id left join tbl_banks b on pc.bank_id=b.id left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where dr.priority='".$priority."' and dr.realization_status='Deposited' order by dr.deposit_date DESC";
                        }    
                    }    
                } 
        ...
        $sql = "select dr.*,(select db.b_name from tbl_banks db where id=dr.bank_id)as d_bank_name,pc.check_date,pc.bank_deposit_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name from tbl_deposit_realization dr left join tbl_payment_collections pc on dr.collection_id=pc.id left join tbl_banks b on pc.bank_id=b.id left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where dr.realization_status='Deposited' order by dr.deposit_date DESC";
        $data['all_deposits'] = $this->m_common->customeQuery($sql);  
    ...
    function allDepositReport($print = false) {
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sales_report';
        $this->titlebackend("Report");
        ...
        if(!empty($postData['from_date'])){
            $data['from_date']=$from_date=date('Y-m-d',strtotime($postData['from_date']));
            $data['to_date']=$to_date=date('Y-m-d',strtotime($postData['to_date']));
        }else{
           ...
           if(!empty($from_date) && !empty($to_date)){
                $where.=" and dr.deposit_date>='$from_date' and deposit_date<='$to_date' ";
            }    

           if(empty($postData['bank_id'])){ 
                $sql="select dr.*,tb.b_name,tb.b_account_no from tbl_deposit_realization dr 
                left join tbl_banks tb on dr.bank_id=tb.id 
                where $where group by dr.bank_id";
                $data['deposit_banks']=$this->m_common->customeQuery($sql);
           }else{
                $sql="select dr.*,tb.b_name,tb.b_account_no from tbl_deposit_realization dr 
                left join tbl_banks tb on dr.bank_id=tb.id 
                where $where and dr.bank_id=".$postData['bank_id']." group by dr.bank_id";
                $data['deposit_banks']=$this->m_common->customeQuery($sql);
           }
           ...
           if(!empty($postData)){
                ...
                if(!empty($postData['category_id'])){
                    $where.=' and pc.payment_for_id='.$postData['category_id'];
                    $data['category_id']=$postData['category_id'];
                }
                
                if(!empty($postData['sales_person_id'])){
                    $where.=' and tc.sales_person_id='.$postData['sales_person_id'];
                    $data['sales_person_id']=$postData['sales_person_id'];
                }


                if(!empty($postData['realization_status'])){
                    $where.=" and dr.realization_status='".$postData['realization_status']."'";
                    $data['realization_status']=$postData['realization_status'];
                }
                ...
                if($priority=="All"){
                    ...
                    if(!empty($data['deposit_banks'])){

                   
                        foreach($data['deposit_banks'] as $k=>$value){
                            $sql = "select dr.*,(select db.b_name from tbl_banks db where id=dr.bank_id)as d_bank_name,
                            pc.check_date,pc.bank_deposit_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.receive_date,
                            pc.mrr_no,pc.remark as c_remark,
                            pc.collection_method,pc.collection_method,pc.amount,pc.no,
                            b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name,
                            e.name as sales_person 
                            from tbl_deposit_realization dr 
                            left join tbl_payment_collections pc on dr.collection_id=pc.id 
                            left join tbl_banks b on pc.bank_id=b.id 
                            ...
                            left join employees e on tc.sales_person_id=e.id
                            where $where and dr.bank_id=".$value['bank_id']." order by dr.deposit_date DESC";
    
                            $data['deposit_banks'][$k]['deposits'] = $this->m_common->customeQuery($sql);
                ...
                //     exit;
                    ...
                    if(!empty($data['deposit_banks'])){
                        foreach($data['deposit_banks'] as $k=>$value){
                            ...
                            where $where and dr.bank_id=".$value['bank_id']." and dr.priority='".$priority."' order by dr.deposit_date DESC";
                            $data['deposit_banks'][$k]['deposits'] = $this->m_common->customeQuery($sql);
                        }
                    }    
               ...
               if(!empty($data['deposit_banks'])){
                    ...
                    foreach($data['deposit_banks'] as $k=>$value){
                        ...
                        left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id 
                        left join employees e on tc.sales_person_id=e.id
                        where $where order by dr.deposit_date DESC";
        ...
        if ($print == false) {
            $this->load->view('sales_report/all_deposit_statement', $data);
        } else {
            $html = $this->load->view('sales_report/all_deposit_statement_print', $data, true);
            echo $html;
            exit;
        }
    }
...
//                $object->getActiveSheet()->setCellValueByColumnAndRow(16, $excel_row, number_format($invoice['driver_name'], 2));
    ...
    function misReport(){
       ...
       // $where="inv.is_active=1";
        ...
        //Present Status of not working equipment/Vechile/Pump
        $sql="select vs.*,t.truck_no from tbl_vehicle_status vs left join tbl_truck t on vs.truck_id=t.truck_id where vs.is_active=1 and vs.status='Inprogress'";
        
        $data['vehicle_status']=$this->m_common->customeQuery($sql);
        $sql1="select vs.*,t.truck_no from tbl_vehicle_status vs left join tbl_truck t on vs.truck_id=t.truck_id where vs.is_active=1 and vs.status='Completed' and vs.date='2022-04-20'";
        $data['vehicle_status_completed']=$this->m_common->customeQuery($sql1);
        $years[]=date('Y') - 2;
        $years[]=date('Y') - 1;
        $years[] = date('Y');
        ...
        $data['categories'] = $this->m_common->get_row_array('tbl_product_categories', array('is_active' => 1), '*', '', '', 'category_id');
        
        foreach($data['categories'] as $key=>$value){

            
            foreach($years as $key1=>$year){
                $sql_receive="select sum(inhouse) as inhouse,sum(private) as private,sum(inhouse_receive) as inhouse_receive,sum(private_receive) as private_receive from tbl_sales_closing where category_id=".$value['category_id']." and year=$year"; 
                $rec_sales=$this->m_common->customeQuery($sql_receive);
                
                $data['categories'][$key][$year]['inhouse'] = $rec_sales[0]['inhouse'];
                $data['categories'][$key][$year]['private']= $rec_sales[0]['private'];
                $data['categories'][$key][$year]['inhouse_receive'] = $rec_sales[0]['inhouse_receive'];
                $data['categories'][$key][$year]['private_receive']= $rec_sales[0]['private_receive'];
        ...
        $data['categories_payable'] = $this->m_common->get_row_array('item_category', array('group_id' =>5), '*', '', '', 'c_id');
        
        foreach($data['categories_payable'] as $pkey=>$value){
            $due_amount=0;
            $today_paid=0;
            $new_bill=0;
            $payable_sql="select sum(bill_amount) as total_bill_amount,sum(paid_amount) as total_paid_amount from tbl_payable where category_id=".$value['c_id']." and year='".date('Y')."'"; 
            $payable_info=$this->m_common->customeQuery($payable_sql);
            if(isset($payable_info[0]['total_bill_amount'])){
                $data['categories_payable'][$pkey]['due_amount']=$payable_info[0]['total_bill_amount']-$payable_info[0]['total_paid_amount'];
                $due_amount=$payable_info[0]['total_bill_amount']-$payable_info[0]['total_paid_amount'];
            }else{
                $data['categories_payable'][$pkey]['due_amount']=0;
            }    
            $data['categories_payable'][$pkey]['today_paid']=0;
            $data['categories_payable'][$pkey]['new_bill']=0;
            $data['categories_payable'][$pkey]['present_due']=$due_amount-$today_paid+$new_bill;

            
        }
        
        $stock_sql="select ics.*,i.item_name,tmu.meas_unit from tbl_item_closing_stock ics left join items i on ics.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id where year='".date('Y')."'";
        $data['item_stocks']=$this->m_common->customeQuery($stock_sql);
        $data['years'] = $years;
        $this->load->view('sales_report/mis_report',$data);
    }
