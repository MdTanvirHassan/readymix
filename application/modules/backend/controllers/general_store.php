...
class General_store extends Site_Controller {
    ...
    function details_supplier($id) {
       ...
       // $pc_sql = "select pc.*,c.c_name,c_short_name from tbl_payment_collections pc left join tbl_customers c on pc.customer_id=c.id where pc.is_active=1 and pc.payment_status='Collected' order by pc.id desc";
        ...
        $this->load->view('general_store/v_supplier_details', $data);
    }
...
// Start  Designation 
    ...
    function add_action_designation() {
        ...
        $this->titlebackend("Add Designation");
        $data = $this->input->post(); 
        $designation=trim($this->input->post('designation_name'));
        $pre_designation=$this->m_common->get_row_array('designation',array('designation_name'=>$designation),'*');
        if(!empty($pre_designation)){
            redirect_with_msg('general_store/add_designation', 'This designation already exits');
        }
        if (!empty($data)) {    
            $id = $this->m_common->insert_row('designation', $data);
            if (!empty($id)) {    
                redirect_with_msg('general_store/add_designation', 'Successfully Added this Designation');
            } else {
...
// Start Empoloyee
    ...
    function add_action_employee() {
        ...
        $data = $this->input->post(); 
        $name=trim($data['name']);
        $pre_employee=$this->m_common->get_row_array('employees',array('name'=>$name,'designation_id'=>$data['designation_id']),'*');
        if(!empty($pre_employee)){
            redirect_with_msg('general_store/add_employee', 'This employee already exits');
        }
        if (!empty($data)) {    
...
//Start IPO Material Indent   
    ...
    function ipo_material_indent(){
        ...
        if(!empty($indent_no)){
            ...
            $where.=" and ipo_number='$indent_no'";
        }
       ...
       // $sql="select * from v_ipo_material_indent where (budgeted_status='pending' or budgeted_status='partial_budgeted')  " ;
        if($user_type==1 || $user_type==3 || $user_type==4){
           // $sql="select * from v_ipo_material_indent where department_id=".$companyId;
            if(!empty($f_date) && !empty($f_date)){
                $f_date=date('Y-m-d',strtotime($f_date));
                $to_date=date('Y-m-d',strtotime($to_date));
                
                $sql="select * from v_ipo_material_indent where $where and department_id=$branch_id and status!='Rejected' and (date>='$f_date' and date<='$to_date') order by ipo_m_id DESC";
            }else{
                $sql="select * from v_ipo_material_indent where $where and department_id=$branch_id and status!='Rejected' order by ipo_m_id DESC limit 20";
            }
        }else{
            if(!empty($f_date) && !empty($f_date)){
                $f_date=date('Y-m-d',strtotime($f_date));
                ...
                $sql="select * from v_ipo_material_indent where $where and employeeId=".$employee_id." or approver_id=".$employee_id." and (date>='$f_date' and date<='$to_date') order by ipo_m_id DESC";
            }else{
                $sql="select * from v_ipo_material_indent where $where and employeeId=".$employee_id." or approver_id=".$employee_id." order by ipo_m_id DESC limit 20";
            } 
        }
...
//            $sql="select * from v_ipo_material_indent_details where department_id=$branch_id and status!='Rejected' order by ipo_m_id DESC";
//        }else{
//            $sql="select * from v_ipo_material_indent_details where employeeId=".$employee_id." or approver_id=".$employee_id." order by ipo_m_id DESC";
//        }
        ...
        if(!empty($f_date) && !empty($f_date)){
            ...
            $to_date=date('Y-m-d',strtotime($to_date));
            if(!empty($where)){
                $sql="select * from v_ipo_material_indent_details where $where and department_id=$branch_id and status!='Rejected' and (indent_date>='$f_date' and indent_date<='$to_date') order by ipo_m_id DESC";
            }else{
                $sql="select * from v_ipo_material_indent_details where department_id=$branch_id and status!='Rejected' and (indent_date>='$f_date' and indent_date<='$to_date') order by ipo_m_id DESC";
            }
        }else{
            if(!empty($where)){
                $sql="select * from v_ipo_material_indent_details where $where and department_id=$branch_id and status!='Rejected' order by ipo_m_id DESC limit 100";
            }else{
                $sql="select * from v_ipo_material_indent_details where department_id=$branch_id and status!='Rejected' order by ipo_m_id DESC limit 100";
            }
        }
        $data['ipo_materials']=$this->m_common->customeQuery($sql);
        $this->load->view('general_store/v_indent_list',$data);
    }
    
    function detailsIndent($id,$print=false) {
        $this->menu = 'general_store';
    ...
    function indentAllPuchaseOrder($in_d_id=false){
           ...
           $sql="select pod.*,imid.indent_no,po.purchase_order_date,po.order_no,po.purchase_type,d.dep_description,i.item_name,tmu.meas_unit,tsu.unit_name,s.SUP_NAME from tbl_purchase_order_details pod left join tbl_purchase_orders po on po.o_id=pod.o_id left join department d on po.unit_id=d.d_id left join ipo_material_indent_details imid on pod.indent_d_id=imid.id left join items i on pod.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join tbl_size_unit tsu on i.size_unit_id=tsu.size_unit_id left join tbl_indent_type tit on po.order_type=tit.id left join supplier s on po.supplier_id=s.ID where pod.indent_d_id='$in_d_id' and po.purchase_type='Direct' order by po.purchase_order_date ASC";           
           $data['spot_purchases']=$this->m_common->customeQuery($sql);
           foreach($data['spot_purchases'] as $key=>$value){
              // $r_sql="select sum(receive_qty) as total_receive_qty from tbl_material_receive_requisition_details mrrd left join material_receive_requisition mrr on mrrd.mrr_id=mrr.mrr_id where mrr.po_id=".$value['o_id']." and mrrd.item_id=".$value['item_id'];  //14-01-2021
               $r_sql="select sum(receive_qty) as total_receive_qty from tbl_material_receive_requisition_details mrrd left join material_receive_requisition mrr on mrrd.mrr_id=mrr.mrr_id where mrrd.o_details_id=".$value['o_details_id']." and mrrd.item_id=".$value['item_id'];  
               $recive=$this->m_common->customeQuery($r_sql);
               if(!empty($recive)){
                    $data['spot_purchases'][$key]['receive_quantity']=$recive[0]['total_receive_qty'];
                    $data['spot_purchases'][$key]['remaining_quantity']=$value['quantity']-$recive[0]['total_receive_qty'];
               }else{
                   $data['spot_purchases'][$key]['receive_quantity']='';
                   $data['spot_purchases'][$key]['remaining_quantity']=$value['quantity'];
               }
           }
           
           
            
           $b_sql="select imid.*,b.b_type,i.item_name,tmu.meas_unit from budget_details imid left join budget b on imid.b_id=b.b_id left join items i on imid.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id  where imid.indent_d_id='$in_d_id'  order by imid.b_date DESC";            
           
           
           
           $data['indents']=$this->m_common->customeQuery($b_sql);
           ...
           $this->load->view('general_store/v_indent_workorder',$data); 
    }
...
//         }
            ...
            $data['item_list'][$key]['stock_qty']=($opening_qty+$recive_qty+$adj_qty+$transfer_receive_qty)-($consumption_qty+$transfer_qty);
...
//                exit;
    ...
    function details_ipo_material_indent($id,$print=false) {
        $this->menu = 'general_store';
        $this->sub_menu = 'material_indent';
        $this->sub_inner_menu = 'material_indent';
        $this->titlebackend("Details Ipo Material Indent ");      
        $companyId = $this->session->userdata('companyId');
        $employee_id = $this->session->userdata('employeeId');
        $user_type = $this->session->userdata('user_type');
 ...
 // Start Budget  
    ...
    function budget(){
      ...
      //  $data['budgets'] = $this->m_common->get_row_array('budget', '', '*');
        $sql="select * from budget where branch_id=".$branch_id." and (b_status='pending' or b_status='partial_received'  ) and b_approve_status!='Rejected' and b_date>'2021-09-31' order by b_id DESC";
    ...
    function add_budget(){
     ...
     //   $sql="select imid.*,d.dep_description,d.short_name,i.item_code,i.item_name,tsu.unit_name from ipo_material_indent_details imid left join items i on imid.item_id=i.id left join tbl_size_unit tsu on i.size_unit_id=tsu.size_unit_id left join department d on imid.department_id=d.d_id left join ipo_material_indent imi on imid.ipo_m_id=imi.ipo_m_id left join tbl_indent_type tit on imi.indent_type=tit.id where imid.department_id=".$branch_id." and imi.status='Approved' and (imid.budgeted_status='pending' or imid.budgeted_status='partial_budgeted'  ) and (tit.type_name='Material' or tit.type_name='Asset') and imid.indent_date>'2021-05-31' order by imid.indent_no DESC"; //2021-09-18
        $sql="select imid.*,d.dep_description,d.short_name,i.item_code,i.item_name,tsu.unit_name from ipo_material_indent_details imid left join items i on imid.item_id=i.id left join tbl_size_unit tsu on i.size_unit_id=tsu.size_unit_id left join department d on imid.department_id=d.d_id left join ipo_material_indent imi on imid.ipo_m_id=imi.ipo_m_id left join tbl_indent_type tit on imi.indent_type=tit.id where (imid.department_id=".$branch_id." and imi.status='Approved' and (imid.budgeted_status='pending' or imid.budgeted_status='partial_budgeted'  ) and (tit.type_name='Material' or tit.type_name='Asset') ) and imid.indent_date>'2021-12-31' order by imid.indent_no DESC ";
   ...
   //    $data['budget_items'] = $this->m_common->customeQuery($sql);
    ...
    function details_budget($id,$print=false,$type=false){
        $this->menu = 'general_store';
        $this->sub_menu = 'budget';
        $this->sub_inner_menu = 'budget';
        $this->titlebackend("Budget");
     ...
     //   $data['budget_info'] = $this->m_common->get_row_array('budget',array('b_id'=>$id), '*');
      ...
      //  $this->load->view('general_store/v_details_budget',$data);
          if($print==false){
               ...
               if($type=='draft'){
                  $html=$this->load->view('general_store/print_budget_draft', $data,true);
               }else{
...
// Start Material Receive Requisition(MRR)  
    ...
    //    $sql="select * from material_receive_requisition ORDER BY mrr_id DESC ";
        if(!empty($f_date) && !empty($to_date)){
           ...
           $sql="select * from material_receive_requisition where unit_id=".$branch_id." and mrr_date>='" . $f_date . "' and mrr_date<='" . $to_date . "' ORDER BY mrr_id DESC";
        }else{
            $data['f_date'] = '';
            $data['to_date'] = '';
            $f_date = '';
            $to_date = '';
            $sql="select * from material_receive_requisition where unit_id=".$branch_id." ORDER BY mrr_id DESC limit 30";
        }
...
//       }
    //   $sql="Select * from tbl_purchase_orders where (status='Pending' or status='Partially Received') and approve_status='Approved' and unit_id=".$branch_id; //26-11-2020
       $sql="Select tpo.*,s.SUP_NAME from tbl_purchase_orders tpo left join supplier s on tpo.supplier_id=s.ID  where (tpo.status='Pending' or tpo.status='Partially Received') and tpo.closing_status!='Closed' and tpo.approve_status='Approved' and tpo.unit_id=".$branch_id; 
       $data['purchase_orders'] = $this->m_common->customeQuery($sql);
           ...
           // $insertData['unit_id'] =$branch_id;
            ...
            if (!empty($id)) {
               //  $this->m_common->insert_row('material_receive_code', array('mrr_code'=>$mrr_code));
                ...
                foreach ($postData['item_id'] as $key => $each) {
                    $r++; 
                     if(in_array($r,$postData['item_select'])){
                                $insertData1=array();
                                $insertData1['item_id'] = $each;
...
//join cost_center cc on cc.c_c_id = tic.c_c_id where unit_id =$branch_id";
   ...
   left join cost_center cc on cc.c_c_id = tic.c_c_id left join tbl_departments d on tic.dept_id=d.id left join tbl_item_brand tib on tic.brand_id=tib.id where tic.unit_id=$branch_id and tic.consumption_date>='" . $f_date . "' and tic.consumption_date<='" . $to_date . "' order by consumption_id DESC ";
         }else{
            $data['f_date'] = '';
            $data['to_date'] = '';
            $f_date = '';
            $to_date = '';
             $sql ="select tic.*,i.item_name,cc.c_c_name,d.dept_name,tib.brand_name from tbl_item_comsumption tic 
left join items i on i.id = tic.item_id
left join cost_center cc on cc.c_c_id = tic.c_c_id left join tbl_departments d on tic.dept_id=d.id left join tbl_item_brand tib on tic.brand_id=tib.id where tic.unit_id=$branch_id order by consumption_id DESC limit 30";
         } 
   ...
   //Added by alauddin End
        ...
        if(!empty($_POST)){
           ...
           foreach ($postData['item_id'] as $key=>$row){
             ...
             $consuptionInsert['total_quantity']= $postData['total_quantity_hidden'][$key];
    ...
    function add_consumption_31_03_2022() {
        $this->menu = 'general_store';
        $this->sub_menu = 'consumption';
        $this->titlebackend("Add Consumption");
        $branch_id= $this->session->userdata('companyId');
...
//        }
        ...
        $data['items']=$this->m_common->get_row_array('items',array('is_active'=>1),'*');
        ...
        if(!empty($_POST)){
           ...
           $dataInsert['rate']= round(($postData['price']/$postData['transfer_quantity']),2);
           $dataInsert['remark']= $postData['remarks'];
           $dataInsert['transfer_status']="Pending";
           $dataInsert['receive_status']="Pending";
...
JOIN items i on i.id = t.item_id where to_unit_id=$branch_id";
    ...
    function transferItemApprove( $transfer_id ) {
        ...
        $this->sub_inner_menu = 'from_transfer';
        $this->titlebackend("Item Consumption");
         $this->m_common->update_row('tbl_item_transfer',array('transfer_id'=>$transfer_id),array('transfer_status'=>'Transfered'));
         
        $data['branch_id']= $branch_id;
        redirect_with_msg('general_store/transfer', 'Item Transfer Approve Successfully Inserted'); 
    }
    function receivedTransferItem(){
        $this->menu = 'general_store';
        $this->sub_menu = 'transfer';
        $this->sub_inner_menu = 'from_transfer';
        $this->titlebackend("Item Transfer");
        $received_quantity = $this->input->post('received_quantity');
        $transfer_id = $this->input->post('transferID');
        $transferItem = $this->m_common->get_row_array('tbl_item_transfer',array('transfer_id'=>$transfer_id),'*');
         if($transferItem){
          ...
          $receive_value=round(($transferItem[0]['rate']*$received_quantity),2);
          $this->m_common->update_row('tbl_item_transfer',array('transfer_id'=>$transfer_id),array('receive_status'=>'Received','received_quantity'=>$received_quantity,'receive_value'=>$receive_value));
...
//            $data['brands']=$brands;
        ...
        //   $ope_sql="select sum(opening_stock) as total_opening_stock,sum(opening_amount) as total_opening_value from tbl_item_opening_stock where item_id=$item_id and unit_id=$branch_id and brand_id=$brand_id";
           $ope_sql="select sum(opening_stock) as total_opening_stock,sum(opening_amount) as total_opening_value from tbl_item_opening_stock where item_id=$item_id and unit_id=$branch_id";
           $opeing_info=$this->m_common->customeQuery($ope_sql);
           
           $adjustment_info=array();
           $adj_sql="select sum(qty) as total_adjustment,sum(amount) as total_adjustment_amount from  tbl_item_adjustment  where is_active=1 and status='Confirmed' and unit_id=".$branch_id." and item_id=".$item_id;
           $adjustment_info=$this->m_common->customeQuery($adj_sql);
           
           
           $transfer_info=array();
           $transfer_sql="select sum(transfer_quantity) as total_transfer_qty from  tbl_item_transfer  where transfer_status='Transfered' and from_unit_id=".$branch_id." and item_id=".$item_id;
           $transfer_info=$this->m_common->customeQuery($transfer_sql);
           if(!empty($transfer_info)){
                $transfer_qty=$transfer_info[0]['total_transfer_qty'];
           }else{
           ...
           $transfer_receive_info=array();
           $transfer_receive_sql="select sum(received_quantity) as total_receive_qty from  tbl_item_transfer  where receive_status='Received' and to_unit_id=".$branch_id." and item_id=".$item_id;
           $transfer_receive_info=$this->m_common->customeQuery($transfer_receive_sql);
           if(!empty($transfer_receive_info)){
               $transfer_receive_qty=$transfer_receive_info[0]['total_receive_qty'];
           }else{
           ...
           $sql='Select sum(tmrrd.receive_qty) as re_qty,sum(tmrrd.amount) as re_value from tbl_material_receive_requisition_details tmrrd left join material_receive_requisition mrr on tmrrd.mrr_id=mrr.mrr_id  where tmrrd.item_id='.$item_id.' and tmrrd.receive_date>="2021-01-01" and mrr.mrr_status="received" and mrr.unit_id='.$branch_id;
           $receive_qty=$this->m_common->customeQuery($sql);

           $sql='Select sum(return_qty) as is_ret_qty,sum(return_value) as is_ret_value from issue_return_details where item_id='.$item_id.' and ir_d_status="received" ';
           $issue_return_qty=$this->m_common->customeQuery($sql);

           $sql='Select sum(mrrd.receive_qty) as mrr_receive_qty,sum(mrrd.receive_value) as mrr_receive_value from mrr_return_receive_details mrrd left join mrr_return_receive mrr on mrrd.mrr_rr_id=mrr.mrr_rr_id where mrrd.item_id='.$item_id.' and mrrd.mrr_rr_d_status="received" and mrr.branch_id='.$branch_id;
           ...
           $sql='Select sum(consumption_quantity) as issue_qty,sum(amount) as issue_value from tbl_item_comsumption where item_id='.$item_id.' and status="Approved" and unit_id='.$branch_id;
           $issue_qty=$this->m_common->customeQuery($sql);

           $sql='Select sum(rrd.return_qty) as ret_qty,sum(rrd.return_value) as ret_value from return_receive_details rrd left join return_receive rr on rrd.rr_id=rr.rr_id where rrd.item_id='.$item_id.' and rrd.rr_d_status="returned" and rr.branch_id='.$branch_id;
...
//           $current_rate=round($available_value/$available_qty,2);
           ...
           $available_qty=($total_receive_qty+$opeing_info[0]['total_opening_stock']+$adjustment_info[0]['total_adjustment']+$transfer_receive_qty)-($total_issue_qty+$transfer_qty);
           $available_value=$total_receive_value+$opeing_info[0]['total_opening_value']+$adjustment_info[0]['total_adjustment_amount']-$total_issue_value;
           $current_rate=round($available_value/$available_qty,2);
