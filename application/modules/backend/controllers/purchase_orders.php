...
class Purchase_orders extends Site_Controller {
    ...
    function index() {
       ...
       // $sql="select o.*,d.dep_description,d.short_name as dep_short_name,s.SUP_NAME,tit.type_name from tbl_purchase_orders o left join department d on o.unit_id=d.d_id left join supplier s on o.supplier_id=s.ID left join tbl_indent_type tit on o.order_type=tit.id where o.is_active=1 and o.purchase_type='By Order' order by o_id DESC";
        $sql="select o.*,d.dep_description,d.short_name as dep_short_name,s.SUP_NAME,tit.type_name from tbl_purchase_orders o left join department d on o.unit_id=d.d_id left join supplier s on o.supplier_id=s.ID left join tbl_indent_type tit on o.order_type=tit.id where o.purchase_order_date>='2020-12-31' and o.is_active=1 and o.purchase_type='By Order' order by o_id DESC";
        $data['purchase_orders']=$this->m_common->customeQuery($sql);
        
        $this->load->view('purchase_orders/v_purchase_order',$data);
    }
     ...
     function add_purchase_order() {
       ...
       // $data['indent_types'] = $this->m_common->get_row_array('tbl_indent_type',array('is_active'=>1), '*');
        ...
        $data['term_condition']=$this->m_common->get_row_array('tbl_term_condition_template',array('is_active'=>1),'*');
        
        
        $sql="select bd.*,i.item_name,tmu.meas_unit,tsu.unit_name from budget_details bd left join items i on bd.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join tbl_size_unit tsu on i.size_unit_id=tsu.size_unit_id where bd.department_id=".$branch_id." order by bd.indent_no DESC";
           
        $data['item_list']=$this->m_common->customeQuery($sql);
        
        $this->load->view('purchase_orders/v_add_purchase_order',$data);
    }
    
    function get_term_condition_info(){
        $this->setOutputMode(NORMAL);
        $tem_id=$this->input->post('tem_id');
        $data['term_condition']=$this->m_common->get_row_array('tbl_term_condition_details',array('tem_id'=>$tem_id),'*');
     ...
     function add_purchase_order_action() {
         ...
         if(!empty($postData)){
             ...
             if(empty($postData['select_item'])){
                redirect_with_msg('purchase_orders/add_purchase_order', 'Please Select Item');
             }
...
//             if($postData['order_from']=="Money Indent"){
//                if(!empty($postData['purchase_type'])){
//                    $insertData['purchase_type']=$postData['purchase_type'];
//                }
//             }else{
//                  $insertData['purchase_type']="By Order";
//             }
...
//             }
           ...
           //  $insertData['receive_status']='Pending';
             ...
             if(!empty($result)){
                 ...
                 $insertPaymentCondition['pm_id'] =$postData['pm_id'];
                 
                 $insertPaymentSecurity['is_active'] =1;
                 $insertPaymentSecurity['o_id'] =$result;
                 $insertPaymentSecurity['security_id'] =$postData['security_id'];
                 //$this->m_common->insert_row('tbl_purchase_order_code',array('o_code'=>$postData['o_code'],'supplier_id'=>$postData['supplier_id'],'unit_id'=>$branch_id));
                 $this->m_common->insert_row('tbl_purchase_order_code',array('o_code'=>$postData['o_code'],'supplier_id'=>$postData['supplier_id']));
                // $this->m_common->insert_row('tbl_po_payment_condition',$insertPaymentCondition);
                // $this->m_common->insert_row('tbl_po_payment_security',$insertPaymentSecurity);
                      ...
                      //                      }
                                        ...
                                        if(!empty($postData['remark'][$key])) { 
                                          ...
                                          if(!empty($success)){
                                                ...
                                                if(!empty($postData['bu_d_id'][$key])) {
                                                        $sql="select * from budget_details where bu_d_id=".$postData['bu_d_id'][$key];
                                                        $pre_mi_info=$this->m_common->customeQuery($sql);
                                                        $net_d_order_qty=$pre_mi_info[0]['direct_p_order_qty']+$postData['quantity'][$key];                                                       
                                                        $this->m_common->update_row('budget_details',array('bu_d_id'=>$postData['bu_d_id'][$key]),array('direct_p_order_qty'=>$net_d_order_qty));
                                                }
                                                
                                                
                                                if($postData['order_from']=="Direct") {
                                                        $sql="select * from ipo_material_indent_details where ipo_m_id=".$postData['indent_d_id'][$key];
                                                        $pre_mi_info=$this->m_common->customeQuery($sql);
                                                        $net_d_order_qty=$pre_mi_info[0]['direct_pc_order_qty']+$postData['quantity'][$key];                                                       
                                                        $this->m_common->update_row('ipo_material_indent_details',array('id'=>$postData['indent_d_id'][$key]),array('direct_pc_order_qty'=>$net_d_order_qty));
                                                }
               ...
               if(isset($_FILES['order_image']['name']) && !empty($_FILES['order_image']['name'])) {
                  ...
                  foreach ($postData['t_or_c_name'] as $key => $each) {
                      $insertData2=array();
                      $insertData2['o_id'] = $result;
                      if(empty($each)){
                          continue;
                      }
                      if(!empty($postData['t_or_c_name'][$key])) {
                           $insertData2['t_or_c_name'] = $postData['t_or_c_name'][$key];
                       }
                      if(!empty($postData['t_or_c_description'][$key])) { 
                          $insertData2['t_or_c_description'] = $postData['t_or_c_description'][$key];
                      }    
                      $this->m_common->insert_row('tbl_purchase_order_conditions',$insertData2);
                  }
                  
                  
                  foreach ($postData['copy_description'] as $key => $each) {
                      $insertData2=array();
                      $insertData2['o_id'] = $result;
                      if(empty($each)){
                          continue;
                      ...
                      if(!empty($postData['copy_description'][$key])) { 
                          $insertData2['copy_description'] = $postData['copy_description'][$key];
                      }    
                      $this->m_common->insert_row('tbl_purchase_order_copy_to',$insertData2);
                  }
                  ...
                  redirect_with_msg('purchase_orders', 'Successfully Inserted');
             }
         }else{
              redirect_with_msg('purchase_orders/add_purchase_order', 'Please fill the form and submit');
         }
...
//        $data['payment_info']=$this->m_common->get_row_array('tbl_purchase_order_payment_condition',array('o_id'=>$id),'*');
      ...
      //  $data['copy_to']=$this->m_common->get_row_array('tbl_purchase_order_copy_to',array('o_id'=>$id),'*');
        ...
        $this->load->view('purchase_orders/v_edit_purchase_order',$data);
    }
...
//             }
             ...
             if($result>=0){
                              ...
                              if(!empty($success)){
                                   if(!empty($postData['mi_d_id'][$key])) {
                                    ...
                                    if(!empty($postData['bu_d_id'][$key])) {
                                                $sql="select * from budget_details where bu_d_id=".$postData['bu_d_id'][$key];
                                                $pre_mi_info=$this->m_common->customeQuery($sql);
                                                $net_d_order_qty=$pre_mi_info[0]['direct_p_order_qty']+$postData['quantity'][$key]-$postData['pre_quantity'][$key];                                             
                                                $this->m_common->update_row('budget_details',array('bu_d_id'=>$postData['bu_d_id'][$key]),array('direct_p_order_qty'=>$net_d_order_qty));
                                    }
                                    
                                    if($postData['order_from']=="Direct"){
                                        $sql="select * from ipo_material_indent_details where id=".$postData['indent_d_id'][$key];
                                        $pre_mi_info=$this->m_common->customeQuery($sql);
                                        $net_d_order_qty=$pre_mi_info[0]['direct_pc_order_qty']+$postData['quantity'][$key]-$postData['pre_quantity'][$key];                                                       
                                        $this->m_common->update_row('ipo_material_indent_details',array('id'=>$postData['indent_d_id'][$key]),array('direct_pc_order_qty'=>$net_d_order_qty));
                                    }
                ...
                if(isset($_FILES['order_image']['name']) && !empty($_FILES['order_image']['name'])) {
                 ...
                 $this->m_common->delete_row('tbl_purchase_order_conditions',array('o_id'=>$o_id));
                  foreach ($postData['t_or_c_name'] as $key => $each) {
                      $insertData2=array();
                  ...
                  $this->m_common->delete_row('tbl_purchase_order_copy_to',array('o_id'=>$o_id));
                  foreach ($postData['copy_description'] as $key => $each) {
                      $insertData2=array();
                      $insertData2['o_id'] = $o_id;
                      if(empty($each)){
                  ...
                  redirect_with_msg('purchase_orders', 'Successfully Updated');
             }
         }else{
              redirect_with_msg('purchase_orders/add_purchase_order', 'Please fill the form and submit');
         }
...
//        $data['payment_info']=$this->m_common->get_row_array('tbl_purchase_order_payment_condition',array('o_id'=>$id),'*');
     ...
     //   $sql="select d.*,i.item_name,tmu.meas_unit,s.service_name,imid.indent_no from tbl_purchase_order_details d left join items i on d.item_id=i.id left join tbl_services s on d.service_id=s.id left join ipo_material_indent_details imid on d.indent_d_id=imid.id left join budget_details bd on d.bu_d_id=bd.bu_d_id left join tbl_money_indent_details mid on d.mi_d_id=mid.mi_d_id left join tbl_measurement_unit tmu on i.mu_id=tmu.id  where d.is_active=1 and d.o_id=".$id;
        ...
        if($print==false){
            $this->load->view('purchase_orders/v_details_purchase_order',$data);
        }else{
            $html=$this->load->view('purchase_orders/print_purchase_order',$data,true);
            echo $html;
            exit; 
        }
...
//       $data['payment_info']=$this->m_common->get_row_array('tbl_po_payment_condition',array('o_id'=>$id),'*');
       ...
       // $sql="select d.*,i.item_name,tmu.meas_unit,s.service_name,imid.indent_no from tbl_purchase_order_details d left join items i on d.item_id=i.id left join tbl_services s on d.service_id=s.id left join ipo_material_indent_details imid on d.indent_d_id=imid.id left join budget_details bd on d.bu_d_id=bd.bu_d_id left join tbl_money_indent_details mid on d.mi_d_id=mid.mi_d_id left join tbl_measurement_unit tmu on i.mu_id=tmu.id  where d.is_active=1 and d.o_id=".$id;
        ...
        $data['payment_securities']=$this->m_common->get_row_array('tbl_payment_security',array('is_active'=>1),'*');
        if($print==false){
            $this->load->view('purchase_orders/v_purchase_order_letter',$data);
        }else{
            $html=$this->load->view('purchase_orders/print_purchase_order_letter',$data,true);
            echo $html;
            exit; 
        }
    ...
    function delete_purchase_order($id) {
        if(!empty($id)) {
            ...
            if(!empty($id)) {
                $this->m_common->update_row('tbl_purchase_quotation', array('q_id' => $q_id[0]['q_id']),array('status'=>"Pending"));
                $this->m_common->update_row('tbl_purchase_quotation_details', array('q_id' => $q_id[0]['q_id']),array('is_active'=>"1"));
                $this->m_common->update_row('tbl_purchase_order_details', array('o_id' => $id),array('is_active'=>0));
                $this->m_common->update_row('tbl_purchase_order_payment_condition', array('o_id' => $id),array('is_active'=>0));
                
                redirect_with_msg('purchase_orders/index', 'Successfully Deleted');
            } else {
                redirect_with_msg('purchase_orders/index', 'Data not deleted for an unexpected error');
            }
        } else {
            redirect_with_msg('purchase_orders/index', 'Please click on delete button');
        }
    }
   ...
   function get_purchase_items(){
       ...
       if($data['quotation_info'][0]['type_name']=="Material" || $data['quotation_info'][0]['type_name']=="Asset"){
            ...
            //    $sql="select imid.*,i.item_name,tmu.meas_unit,s.service_name,s.id as s_id from ipo_material_indent_details imid left join items i on imid.item_id=i.id left join tbl_services s on imid.service_id=s.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id where imid.department_id=".$unit_id;
             ...
             //   $sql="select bd.*,i.item_name,tmu.meas_unit,tsu.unit_name from budget_details bd left join items i on bd.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join tbl_size_unit tsu on i.size_unit_id=tsu.size_unit_id where bd.department_id=".$unit_id." order by bd.indent_no DESC"; //Backup 2021
             //   $sql="select bd.*,i.item_name,tmu.meas_unit,tsu.unit_name from budget_details bd left join items i on bd.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join tbl_size_unit tsu on i.size_unit_id=tsu.size_unit_id where bd.b_date>'2021-08-30' and bd.department_id=".$unit_id." order by bd.indent_no DESC";
                $sql="select bd.*,i.item_name,tmu.meas_unit,tsu.unit_name from budget_details bd left join items i on bd.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join tbl_size_unit tsu on i.size_unit_id=tsu.size_unit_id where (bd.b_date>'2021-12-31' and bd.department_id=".$unit_id.") order by bd.indent_no DESC";
   ...
   function forward_purchase_order($id) {
        ...
        redirect_with_msg('purchase_orders', 'Forward Successfull');
    }
   ...
   function reject_purchase_order($id){
        ...
        redirect_with_msg('purchase_orders', 'Rejected Successfully');
    }
...
//           }
        ...
        redirect_with_msg('purchase_orders', 'Approved Successfully');
    }
   

    
     function close_purchase_order($id){
         $user_id=$this->session->userdata('user_id');
         $result=$this->m_common->update_row('tbl_purchase_orders',array('o_id'=>$id),array('closing_status'=>'Closed','updated_by'=>$user_id));
         if($result>0){
             
                redirect_with_msg('purchase_orders/index', 'Successfully Closed'); 
             
         }else{
             
                redirect_with_msg('purchase_orders/index', 'Not Closed'); 
             
         }
     }
     
     function open_purchase_order($id){
         $user_id=$this->session->userdata('user_id');
         $result=$this->m_common->update_row('tbl_purchase_orders',array('o_id'=>$id),array('closing_status'=>'Open','updated_by'=>$user_id));
         if($result>0){
             redirect_with_msg('purchase_orders/index', 'Successfully Open');
         }else{
             redirect_with_msg('purchase_orders/index', 'Not Open');
         }
     }
