...
class Sale_target extends Site_Controller
{
    ...
    function sales_commission()
    {
        ...
        // $data['sale_order']=$this->m_common->get_row_array('tbl_sales_sale_order
        // $sql="select o.*,q.reference_no,c.c_name,c.c_short_name from tbl_sales_orders o left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on q.customer_id=c.id where o.is_active=1";
        $sql = "select * from sales_commission as sc 
        left join department as d on d.d_id=sc.project_id
    ...
    function add_sale_commission()
    {
        ...
        $this->sub_inner_menu = 'sales_commission';
        $this->titlebackend("Sale Commission");
        $data['employees']=$this->m_common->get_row_array('employees',array('is_active'=>1),'*');
    ...
    function sales_incentive()
    {
        ...
        $sql = "select * from sales_incentive";
    ...
    function distribution()
    {
        ...
        foreach ($teams as $key => $team) {
            ...
            $sql = "select st.team_name,st.id as team_id,stu.id as team_member_id,stu.contribution,e.*,d.designation_name from sales_team st join sales_team_user stu on st.id=stu.team_id join employees as e on e.id=stu.employee_id left join designation d on e.designation_id=d.id where stu.team_id=" . $team['id'];
    ...
    function achivement()
    {
        ...
        $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity from tbl_delivery_challan_details dcd 
                    join tbl_delivery_challans dc on dcd.dc_id=dc.dc_id where dc.delivery_challan_date between '" . $start_date . "' and '" . $end_date . "'";
        ...
        foreach ($teams as $key => $team) {
            ...
            foreach ($target as $t => $each) {
                ...
                foreach ($cat as $c) {
                    ...
                    $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity from tbl_delivery_challan_details dcd 
                    join tbl_delivery_challans dc on dcd.dc_id=dc.dc_id
                    join tbl_delivery_orders as do on dc.do_id=do.do_id
                    join tbl_sales_orders as so on do.o_id=so.o_id
                    where dcd.s_item_id=" . $c['category_id'] . " and so.sale_person_id=" . $each['employee_id'] . " and dc.delivery_challan_date between '" . $start_date . "' and '" . $end_date . "'";
    ...
    function provide_incentive()
    {
        ...
        foreach ($sales_team as $key => $team) {
            ...
            $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity,group_concat(DISTINCT so.customer_id) as customer_id from tbl_delivery_challan_details dcd 
                    join tbl_delivery_challans dc on dcd.dc_id=dc.dc_id
                    join tbl_delivery_orders as do on dc.do_id=do.do_id
                    join tbl_sales_orders as so on do.o_id=so.o_id
                    where so.sale_person_id in(" . $sales_persons[0]['sales_person'] . ") and dc.delivery_challan_date between '" . $start_date . "' and '" . $end_date . "'";
            $aciv = $this->m_common->customeQuery($sql);
    ...
    function provide_incentive_employee($team_id = 0)
    {
        ...
        foreach ($sales_team as $key => $team) {
            ...
            $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity,group_concat(DISTINCT so.customer_id) as customer_id from tbl_delivery_challan_details dcd 
                    ...
                    join tbl_delivery_orders as do on dc.do_id=do.do_id
                    join tbl_sales_orders as so on do.o_id=so.o_id
                    where so.sale_person_id in(" . $team['employee_id'] . ") and dc.delivery_challan_date between '" . $start_date . "' and '" . $end_date . "'";
            $aciv = $this->m_common->customeQuery($sql);
            if (!empty($aciv[0]['customer_id'])) {
                $sql = "select sum(amount) as total from tbl_payment_collections where customer_id in (" . $aciv[0]['customer_id'] . ") and payment_status='Received'  and receive_date between '" . $start_date . "' and '" . $end_date . "'";
                $collection = $this->m_common->customeQuery($sql);
    ...
    function team_member($id)
    {
        ...
        $sql = "select st.team_name,stu.id as team_member_id,stu.contribution,e.*,d.designation_name from sales_team st join sales_team_user stu on st.id=stu.team_id join employees as e on e.id=stu.employee_id left join designation d on e.designation_id=d.id where stu.team_id=" . $id;
