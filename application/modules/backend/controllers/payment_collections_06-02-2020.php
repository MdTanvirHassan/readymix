...
class Payment_collections extends Site_Controller {
    ...
    function index() {
        ...
        $sql = "select pc.*,so.order_no,c.c_name,c_short_name from tbl_payment_collections pc left join tbl_sales_orders so on pc.o_id=so.o_id left join tbl_sales_quotation sq on so.q_id=sq.q_id left join tbl_customers c on so.customer_id=c.id where pc.is_active=1 and pc.payment_status='Collected' and pc.unit_id=" . $branch_id.' order by pc.id desc';
        ...
        try {
            ...
            if (!empty($postData)) {
                ...
                foreach ($postData['o_id'] as $o_id){
                    ...
                    if (!empty($o_id)) {
                        $insertData['o_id'] = $o_id;
                    } else {
                        continue;
                        //redirect_with_msg('payment_collections/add_collection', 'Please select sales order');
                    }

                    $order = $this->m_common->get_row_array('tbl_sales_orders', array('o_id' => $o_id), '*');
                    $payment_condition = $this->m_common->get_row_array('tbl_sales_order_payment_condition', array('o_id' => $o_id), '*');

                    if (!empty($order[0]['total_amount'])) {
                        if (($order[0]['total_amount'] - $order[0]['receive_amount']) >= $balance)
                            $insertData['amount'] = $balance;
                        else
                            $insertData['amount'] = ($order[0]['total_amount'] - $order[0]['receive_amount']);
...
//                 }
                    ...
                    $balance = $balance - $insertData['amount'];
                    ...
                    if (!empty($result)) {
                        ...
                        if ($payment_condition[0]['b_cash'] == "Cash") {
                            $sql = "select sum(amount) as total from  tbl_payment_collections  where payment_status='Received' and collection_method='Cash' and is_active=1 and o_id=" . $o_id;
                        ...
                        if ($payment_condition[0]['b_bg'] == "Bg") {
                            $sql = "select sum(amount) as total from  tbl_payment_collections  where (payment_status='Received' or collection_condition='Collection') and collection_method='Bg' and is_active=1 and o_id=" . $o_id;
                        ...
                        if ($payment_condition[0]['b_pdc'] == "Pdc") {
                            if ($payment_condition[0]['b_pdc_condition'] == "Collection") {
                                $sql = "select sum(amount) as total from  tbl_payment_collections  where (payment_status='Received' or collection_condition='Collection') and collection_method='Pdc' and is_active=1 and o_id=" . $o_id;
                            } else {
                                $sql = "select sum(amount) as total from  tbl_payment_collections  where payment_status='Received' and collection_method='Pdc' and is_active=1 and o_id=" . $o_id;
                            }
                        ...
                        if ($payment_condition[0]['b_lc'] == "Lc") {
                            if ($payment_condition[0]['b_lc_condition'] == "Collection") {
                                $sql = "select sum(amount) as total from  tbl_payment_collections  where (payment_status='Received' or collection_condition='Collection') and collection_method='Lc' and is_active=1 and o_id=" . $o_id;
                            } else {
                                $sql = "select sum(amount) as total from  tbl_payment_collections  where payment_status='Received' and collection_method='Lc' and is_active=1 and o_id=" . $o_id;
                            }
                        ...
                        $this->m_common->update_row('tbl_sales_orders', array('o_id' => $o_id), array('status' => $approve));
...
//                }  
                ...
                if (!empty($postData['from_deposit'])) {
                   // $customer = $this->m_common->get_row_array('tbl_customers', array('id' => $postData['c_id']), '*');
//                    if ($customer[0]['deposit'] >= $balance)
//                        $sql = "update tbl_customers set deposit=deposit-" . $balance . " where id=" . $postData['c_id'];
//                    else
                        $sql = "update tbl_customers set deposit=" . $balance . " where id=" . $postData['c_id'];
    ...
    function edit_collection($id) {
        ...
        $all_collection_sql = 'select * from tbl_payment_collections where payment_status="Received" and o_id=' . $data['collection_info'][0]['o_id'];
...
where so.customer_id=$id and (receive_status='Pending' or receive_status='Partial Received')";
    ...
    function payment_received() {
        ...
        // $data['quotations']=$this->m_common->get_row_array('tbl_sales_quotation
        // $sql="select pc.*,so.order_no,c.c_name,c_short_name from tbl_payment_collections pc left join tbl_sales_orders so on pc.o_id=so.o_id left join tbl_sales_quotation sq on so.q_id=sq.q_id left join tbl_customers c on sq.customer_id=c.id where pc.is_active=1";
        $sql = "select pc.*,so.order_no,c.c_name,c_short_name from tbl_payment_collections pc left join tbl_sales_orders so on pc.o_id=so.o_id left join tbl_sales_quotation sq on so.q_id=sq.q_id left join tbl_customers c on so.customer_id=c.id where pc.is_active=1 and pc.payment_status='Received' and pc.unit_id=" . $branch_id;
        $data['collections'] = $this->m_common->customeQuery($sql);
