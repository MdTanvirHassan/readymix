...
class Report extends Site_Controller
{
    ...
    function materialStock($print = false)
    {
        ...
        if (!empty($postData)) {
            ...
            foreach ($data['items'] as $key => $item) {
                ...
                $transfer_info=array();
                $transfer_sql="select sum(transfer_quantity) as total_transfer_qty from  tbl_item_transfer  where transfer_status='Transfered' and from_unit_id=".$branch_id." and item_id=".$item['id'];
                $transfer_info=$this->m_common->customeQuery($transfer_sql);
                if(!empty($transfer_info)){
                    $transfer_qty=$transfer_info[0]['total_transfer_qty'];
                }else{
                    $transfer_qty=0;
                }
                ...
                $transfer_receive_sql="select sum(received_quantity) as total_receive_qty from  tbl_item_transfer  where receive_status='Received' and to_unit_id=".$branch_id." and item_id=".$item['id'];
                $transfer_receive_info=$this->m_common->customeQuery($transfer_receive_sql);
                if(!empty($transfer_receive_info)){
                    $transfer_receive_qty=$transfer_receive_info[0]['total_receive_qty'];
                }else{
                    $transfer_receive_qty=0;
                }
               ...
               // $data['items'][$key]['stock_amount'] = $opening_qty + $recive_qty + $adj_qty - $consumption_qty;
                ...
                $data['items'][$key]['stock_amount']=($opening_qty+$recive_qty+$adj_qty+$transfer_receive_qty)-($consumption_qty+$transfer_qty);
    ...
    function storeLedger($print = false)
    {
        ...
        if (!empty($postData)) {
            ...
            if ($report_format == 'general') {
                ...
                if ($item == "all") {
                    $it_sql="select i.*,tmu.meas_unit from items i left join tbl_measurement_unit tmu on i.mu_id=tmu.id where i.item_type='Consumable'";
                    $data['items'] = $this->m_common->customeQuery($it_sql);
                   ...
                   // $data['items'] = $this->m_common->get_row_array('items', array('id' => $item), '*');
                    $it_sql="select i.*,tmu.meas_unit from items i left join tbl_measurement_unit tmu on i.mu_id=tmu.id where i.id=$item";
                    $data['items'] = $this->m_common->customeQuery($it_sql);
                }
            } else {
                ...
                if ($item == 'all') {
                    ...
                    foreach ($data['group'] as $key => $group) {
                       // $data['group'][$key]['group_items'] = $this->m_common->get_row_array('items', array('item_group' => $group['id']), '*', '', '', 'item_name');
                        $it_sql="select i.*,tmu.meas_unit from items i left join tbl_measurement_unit tmu on i.mu_id=tmu.id where i.item_group=".$group['id'];
                        $data['group'][$key]['group_items'] = $this->m_common->customeQuery($it_sql);
                    }
                } else {
                   // $data['items'] = $this->m_common->get_row_array('items', array('item_group' => $item), '*', '', '', 'item_name');
                    $it_sql="select i.*,tmu.meas_unit from items i left join tbl_measurement_unit tmu on i.mu_id=tmu.id where i.item_group=$item";
                    $data['items'] = $this->m_common->customeQuery($it_sql);
            ...
            if (($report_format == 'general' && $item == "all") || $report_format == 'group') {
                    ...
                    foreach ($data['group'] as $key1 => $gr) {
                        ...
                        foreach ($data['group'][$key1]['group_items'] as $key => $item_info) {
                            ...
                            if(!empty($issue_qty[0]['issue_qty'])){  
                              if($issue_qty[0]['issue_qty']<=$total_receive_qty){  
                                 ...
                                 $total_issue_qty = $total_receive_qty+$delivery_issue_qty[0]['issue_qty'] + $return_qty[0]['ret_qty'];
                                 $total_issue_value =$total_receive_value+ $delivery_issue_qty[0]['issue_value'] + $return_qty[0]['ret_value'];  
                              }
                            }else{
                              $total_issue_qty = $delivery_issue_qty[0]['issue_qty'] + $return_qty[0]['ret_qty'];
                              $total_issue_value = $delivery_issue_qty[0]['issue_value'] + $return_qty[0]['ret_value'];  
                            }

                            $opening_qty = $total_receive_qty + $opeing_info[0]['total_opening_stock'] - $total_issue_qty;
                            $opening_value = round(($total_receive_value + $opeing_info[0]['total_opening_value'] - $total_issue_value),2);
                            ...
                            if (!empty($issue_qty[0]['issue_qty'])) {
                              ...
                              if($issue_qty[0]['issue_qty']<=$avilable_quantity){  
                                $general_issue=$issue_qty[0]['issue_qty'];  
                                $data['group'][$key1]['group_items'][$key]['issue_qty'] = $issue_qty[0]['issue_qty'];
                                $data['group'][$key1]['group_items'][$key]['issue_value'] = round($available_rate * $issue_qty[0]['issue_qty'],2);
                                $data['group'][$key1]['group_items'][$key]['issue_rate'] = $available_rate;
                              }else{
                                $general_issue=$avilable_quantity;   
                                $data['group'][$key1]['group_items'][$key]['issue_qty'] = $avilable_quantity;
                                $data['group'][$key1]['group_items'][$key]['issue_value'] = $avilable_value;
                                $data['group'][$key1]['group_items'][$key]['issue_rate'] = $available_rate;  
                              }  
                            } else {
                            ...
                            if (!empty($d_issue_qty[0]['delivery_qty'])) {
                                ...
                                $data['group'][$key1]['group_items'][$key]['delivery_rate'] = round($d_issue_qty[0]['issue_value'] / $d_issue_qty[0]['issue_qty'],2);
                            } else {
                            ...
                            if (!empty($return_qty[0]['ret_qty'])) {
                                ...
                                $gr['group_items'][$key]['return_rate'] = round($return_qty[0]['ret_value'] / $return_qty[0]['ret_qty'],2);
                            } else {
                            ...
                            $total_issue_qty = $general_issue+$data['group'][$key1]['group_items'][$key]['return_qty'] + $data['group'][$key1]['group_items'][$key]['delivery_qty'];
                            $total_issue_value = round($total_issue_qty * $available_rate,2);
                            $total_issue_rate = $available_rate;
                          ...
                          if($avilable_quantity>0){  
                            $balance = $avilable_quantity - $total_issue_qty;
                            $balance_value = round($available_rate * $balance,2);
                            $balance_rate = $available_rate;
                          }else{
                            $balance = 0;
                            $balance_value = 0;
                            $balance_rate = 0;  
                    ...
                    foreach ($data['items'] as $key => $item_info) {
                       ...
                       // $total_issue_value = $issue_qty[0]['issue_value'] + $delivery_issue_qty[0]['issue_value'] + $return_qty[0]['ret_value'];
                        ...
                        if (!empty($adjustment_qty[0]['adjust_qty'])) {
                            ...
                            $data['items'][$key]['adjust_rate'] = round($adjustment_qty[0]['adjust_value'] / $adjustment_qty[0]['adjust_qty'],2);
                        } else {
                        ...
                        if (!empty($issue_qty[0]['issue_qty'])) {
                          ...
                          if($issue_qty[0]['issue_qty']<=$avilable_quantity){  
                            $general_issue=$issue_qty[0]['issue_qty'];
                            $data['items'][$key]['issue_qty'] = $issue_qty[0]['issue_qty'];
                            $data['items'][$key]['issue_value'] = round($available_rate * $issue_qty[0]['issue_qty'],2);
                            $data['items'][$key]['issue_rate'] = $available_rate;
                          }else{
                            $general_issue=$avilable_quantity;  
                            $data['items'][$key]['issue_qty'] =$avilable_quantity;
                            $data['items'][$key]['issue_value'] = $avilable_value;
                            $data['items'][$key]['issue_rate'] = $available_rate;  
                          }
                        ...
                        $total_issue_qty = $general_issue+$data['items'][$key]['return_qty'] + $data['items'][$key]['delivery_qty'];
                        $total_issue_value = round($available_rate * $total_issue_qty,2);
                        $total_issue_rate = $available_rate;
               ...
               // $total_issue_value = $issue_qty[0]['issue_value'] + $delivery_issue_qty[0]['issue_value'] + $return_qty[0]['ret_value'];
                ...
                for ($i = $from_date; $i <= $too_date; $i = date('Y-m-d', strtotime('+1 day', strtotime($i)))) {
                    ...
                    //                   $details_info[$j]['receive_rate']=$receive_qty[0]['re_value']/$receive_qty[0]['re_qty'];
                    ...
                    $mrr_return_receive = $this->m_common->customeQuery($sql);
                    //                   $details_info[$j]['mrr_receive_qty']=$issue_return_qty[0]['mrr_receive_qty'];
                    //                   $details_info[$j]['mrr_receive_value']=$issue_return_qty[0]['mrr_receive_value'];
                    ...
                    if (!empty($adjustment_qty[0]['adjust_qty'])) {
                        ...
                        $details_info[$j]['adjust_rate'] = round($adjustment_qty[0]['adjust_value'] / $adjustment_qty[0]['adjust_qty'],2);
                    } else {
                    ...
                    //                   $details_info[$j]['total_rec_ret_rate']=round($avilable_value/$avilable_quantity,2);
                    ...
                    // $issue_rate=round($issue_qty[0]['issue_value']/$issue_qty[0]['issue_qty'],2);//01-03-2021
                    $issue_rate = $avilable_rate;
                    $general_issue=0;
                    if (!empty($issue_qty[0]['issue_qty'])) {
                        if($issue_qty[0]['issue_qty']<=$avilable_quantity){
                            $general_issue=$issue_qty[0]['issue_qty'];
                            $details_info[$j]['issue_qty'] = $issue_qty[0]['issue_qty'];
                            $details_info[$j]['issue_value'] = round($issue_rate * $issue_qty[0]['issue_qty'],2);
                            $details_info[$j]['issue_rate'] = $issue_rate;
                        }else{
                            $general_issue=$avilable_quantity;
                            $details_info[$j]['issue_qty'] = $avilable_quantity;
                            $details_info[$j]['issue_value'] = $avilable_value;
                            $details_info[$j]['issue_rate'] = $issue_rate;
                        }
                    } else {
                    ...
                    $return_qty = $this->m_common->customeQuery($sql);
                    //                   $details_info[$j]['return_qty']=$return_qty[0]['ret_qty'];
                    //                   $details_info[$j]['return_value']=$return_qty[0]['ret_value'];
                    ...
                    $total_issue_qty = $general_issue+$delivery_issue_qty[0]['issue_qty'] + $return_qty[0]['ret_qty'];
                    $total_issue_value = round($issue_rate * $total_issue_qty,2);
                    $total_issue_rate = $issue_rate;
                  ...
                  if($avilable_quantity>0){    
                    $closing_qty = $total_receive_qty + $opening_qty - $total_issue_qty;
                    $closing_value = round($issue_rate * $closing_qty,2);
                    $closing_rate = $issue_rate;
                  }else{
                     $closing_qty =0;
                     $closing_value =0;
                     $closing_rate =0; 
                    ...
                    if ($j > 0 && $details_info[$j]['receive_qty'] == '-' && $details_info[$j]['issue_return_qty'] == '-' && $details_info[$j]['mrr_receive_qty'] == '-' && $details_info[$j]['issue_qty'] == '-' && $details_info[$j]['delivery_qty'] == '-' && $details_info[$j]['return_qty'] == '-' && $details_info[$j]['adjust_qty'] == '-') {
    ...
    function binCartReport_($print = false)
    {
        $this->menu = 'general_store';
        $this->sub_menu = 'report';
        $branch_id = $this->session->userdata('companyId');
            ...
            if (($report_format == 'general' && $item == "all") || $report_format == 'group') {
                if ($item == "all" && $report_format == 'group') {
                    foreach ($data['group'] as $key1 => $gr) {
                        ...
                        foreach ($data['group'][$key1]['group_items'] as $key => $item_info) {
                            ...
                            if (!empty($issue_return_qty[0]['is_ret_qty'])) {
                                $data['group'][$key1]['group_items'][$key]['issue_return_qty'] = $issue_return_qty[0]['is_ret_qty'];
                                $data['group'][$key1]['group_items'][$key]['issue_return_value'] = $issue_return_qty[0]['is_ret_value'];
                                $data['group'][$key1]['group_items'][$key]['issue_return_rate'] = round($issue_return_qty[0]['is_ret_value'] / $issue_return_qty[0]['is_ret_qty'],2);
                            } else {
                                $data['group'][$key1]['group_items'][$key]['issue_return_qty'] = '-';
                                $data['group'][$key1]['group_items'][$key]['issue_return_value'] = '-';
                                $data['group'][$key1]['group_items'][$key]['issue_return_rate'] = '-';
                            ...
                            if (!empty($mrr_return_receive[0]['mrr_receive_qty'])) {
                                $data['group'][$key1]['group_items'][$key]['mrr_receive_qty'] = $mrr_return_receive[0]['mrr_receive_qty'];
                                $data['group'][$key1]['group_items'][$key]['mrr_receive_value'] = $mrr_return_receive[0]['mrr_receive_value'];
                                $data['group'][$key1]['group_items'][$key]['mrr_receive_rate'] = round($mrr_return_receive[0]['mrr_receive_value'] / $mrr_return_receive[0]['mrr_receive_qty'],2);
                            } else {
                                $data['group'][$key1]['group_items'][$key]['mrr_receive_qty'] = '-';
                                $data['group'][$key1]['group_items'][$key]['mrr_receive_value'] = '-';
                                $data['group'][$key1]['group_items'][$key]['mrr_receive_rate'] = '-';
                            ...
                            if (!empty($adjustment_qty[0]['adjust_qty'])) {
                                $data['group'][$key1]['group_items'][$key]['adjust_qty'] = $adjustment_qty[0]['adjust_qty'];
                                $data['group'][$key1]['group_items'][$key]['adjust_value'] = $adjustment_qty[0]['adjust_value'];
                                $data['group'][$key1]['group_items'][$key]['adjust_rate'] = round($adjustment_qty[0]['adjust_value'] / $adjustment_qty[0]['adjust_qty'],2);
                            } else {
                                $data['group'][$key1]['group_items'][$key]['adjust_qty'] = '-';
                                $data['group'][$key1]['group_items'][$key]['adjust_value'] = '-';
                                $data['group'][$key1]['group_items'][$key]['adjust_rate'] = '-';
                    ...
                    foreach ($data['items'] as $key => $item_info) {
                        ...
                        if($avilable_quantity>0){
                            $balance = $avilable_quantity - $total_issue_qty;
                            $balance_value = round($balance * $available_rate,2);
                            $balance_rate = $available_rate;
                        }else{
                            $balance =0;
                            $balance_value =0;
                ...
                for ($i = $from_date; $i <= $too_date; $i = date('Y-m-d', strtotime('+1 day', strtotime($i)))) {
                    ...
                    // if(!empty($issue_rate)){
                    if (!empty($details_info[$j]['issue_qty'])) {
                        $details_info[$j]['issue_rate'] = $issue_rate;
                    } else {
        ...
        if (!empty($postData)) {
            ...
            if ($report_format == "general") {
                if ($report_type == "summary") {
                    if ($item == "all") {
                        if (!empty($group_id)) {
                            $sql = "select i.*,tmu.meas_unit,ig.item_group as group_name from items i left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join item_groups ig on i.item_group=ig.id where i.item_group=$group_id";
                        } else {
                            $sql = "select i.*,tmu.meas_unit,ig.item_group as group_name from items i left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join item_groups ig on i.item_group=ig.id";
                        }
                        $data['items'] = $this->m_common->customeQuery($sql);
                    } else {
                        //$data['items']=$this->m_common->get_row_array('items',array('id'=>$item),'*'); 
                        $sql = "select i.*,tmu.meas_unit,item_group as group_name from items i left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join item_groups ig on i.item_group=ig.id where i.id=" . $item;
                        $data['items'] = $this->m_common->customeQuery($sql);
                    }
                    ...
                    foreach ($data['items'] as $key => $item_info) {
                        $issue_qty = array();
                        if (!empty($c_c_id)) {
                            $sql = 'Select sum(consumption_quantity) as issue_qty,sum(amount) as issue_value,cc.c_c_name from tbl_item_comsumption tic left join cost_center cc on tic.c_c_id=cc.c_c_id where tic.consumption_date>="' . $from_date . '" and tic.consumption_date<="' . $too_date . '" and tic.item_id=' . $item_info['id'] . ' and tic.status="Approved" and tic.unit_id=' . $branch_id . ' and tic.c_c_id=' . $c_c_id . ' order by consumption_date ';
                        } else {
                            $sql = 'Select sum(consumption_quantity) as issue_qty,sum(amount) as issue_value,cc.c_c_name from tbl_item_comsumption tic left join cost_center cc on tic.c_c_id=cc.c_c_id where tic.consumption_date>="' . $from_date . '" and tic.consumption_date<="' . $too_date . '" and tic.item_id=' . $item_info['id'] . ' and tic.status="Approved" and tic.unit_id=' . $branch_id;
                        }
                        $issue_qty = $this->m_common->customeQuery($sql);
                        if (!empty($issue_qty[0]['issue_qty'])) {
                            $data['items'][$key]['issue_qty'] = $issue_qty[0]['issue_qty'];
                            $data['items'][$key]['issue_value'] = $issue_qty[0]['issue_value'];
                            $data['items'][$key]['c_c_name'] = $issue_qty[0]['c_c_name'];
                        } else {
                    ...
                    if ($item == "all") {
                        ...
                        if (!empty($c_c_id)) {
                            $sql = 'Select tic.*,i.item_name,tmu.meas_unit,cc.c_c_name,ig.item_group as group_name from tbl_item_comsumption tic left join items i on tic.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join cost_center cc on tic.c_c_id=cc.c_c_id left join item_groups ig on i.item_group=ig.id where tic.consumption_date>="' . $from_date . '" and tic.consumption_date<="' . $too_date . '" and tic.status="Approved" and tic.unit_id=' . $branch_id . ' and tic.c_c_id=' . $c_c_id . ' order by tic.consumption_date ';
                        } else {
                            $sql = 'Select tic.*,i.item_name,tmu.meas_unit,cc.c_c_name,ig.item_group as group_name from tbl_item_comsumption tic left join items i on tic.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join cost_center cc on tic.c_c_id=cc.c_c_id left join item_groups ig on i.item_group=ig.id where tic.consumption_date>="' . $from_date . '" and tic.consumption_date<="' . $too_date . '" and tic.status="Approved" and tic.unit_id=' . $branch_id . ' order by tic.consumption_date ';
                        }
                    } else {
                        if (!empty($c_c_id)) {
                            $sql = 'Select tic.*,i.item_name,tmu.meas_unit,cc.c_c_name,ig.item_group as group_name from tbl_item_comsumption tic left join items i on tic.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join cost_center cc on tic.c_c_id=cc.c_c_id left join item_groups ig on i.item_group=ig.id where tic.consumption_date>="' . $from_date . '" and tic.consumption_date<="' . $too_date . '" and tic.item_id=' . $item . ' and tic.status="Approved" and tic.unit_id=' . $branch_id . ' and tic.c_c_id=' . $c_c_id . ' order by tic.consumption_date ';
                        } else {
                            $sql = 'Select tic.*,i.item_name,tmu.meas_unit,cc.c_c_name,ig.item_group as group_name from tbl_item_comsumption tic left join items i on tic.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id left join cost_center cc on tic.c_c_id=cc.c_c_id left join item_groups ig on i.item_group=ig.id where tic.consumption_date>="' . $from_date . '" and tic.consumption_date<="' . $too_date . '" and tic.item_id=' . $item . ' and tic.status="Approved" and tic.unit_id=' . $branch_id;
                        }
                    }
        ...
        if (!empty($postData)) {
            ...
            if ($item == 'all') {
                ...
                foreach ($data['project'] as $key1 => $gr) {
                    $sql = "select ic.*,i.item_code,i.item_name,i.item_type,i.origin,d.dep_description,cc.c_c_name from  tbl_item_comsumption as ic 
                                    left join items i on i.id=ic.item_id
                                    left join department d on d.d_id=ic.unit_id
                                    left join cost_center cc on cc.c_c_id=ic.c_c_id
                                    where dept_id=" . $gr['d_id'] . " and i.item_type='ASSET'";
                ...
                $sql = "select ic.*,i.item_code,i.item_name,i.item_type,i.origin,d.dep_description,cc.c_c_name from  tbl_item_comsumption as ic 
                   ...
                   left join department d on d.d_id=ic.unit_id
                   left join cost_center cc on cc.c_c_id=ic.c_c_id
                   where dept_id=" . $item . " and i.item_type='ASSET'";
                $data['items'] = $this->m_common->customeQuery($sql);
                if (!empty($data['items'])) {
    ...
    function assetMovementMachineReport($print = false)
    {
        $this->menu = 'general_store';
        $this->sub_menu = 'report';

        $sql = "select ic.*,sum(ic.consumption_quantity) as total,i.item_code,i.item_name,i.item_type,i.origin,d.dep_description,cc.c_c_name from  tbl_item_comsumption as ic 
        left join items i on i.id=ic.item_id
        left join department d on d.d_id=ic.unit_id
        left join cost_center cc on cc.c_c_id=ic.c_c_id
        where i.item_type='ASSET' group by ic.c_c_id,ic.unit_id";
        $data['project_items'] = $this->m_common->customeQuery($sql);


        if ($print == false) {
            $this->load->view('report/asset_movement_machine_report', $data);
        } else {
            $html = $this->load->view('report/print_asset_movement_machine_report', $data, true);
            echo $html;
            exit;
        }
    }
    ...
    function supplierLedgerReport()
    {
        ...
        if (!empty($postData)) {
            ...
            //          if(!empty($supplier_id)){ 
            //            $sql="select v.*,s.SUP_NAME,s.opening_balance,s.ID,d.dep_description from tbl_purchase_invoices v left join supplier s on v.supplier_id=s.ID left join department d on v.branch_id=d.d_id where v.supplier_id=".$supplier_id." group by v.supplier_id";
            //          }else{
            //            $sql="select v.*,s.SUP_NAME,s.opening_balance,s.ID,d.dep_description from tbl_purchase_invoices v left join supplier s on v.supplier_id=s.ID left join department d on v.branch_id=d.d_id group by v.supplier_id";
            //          }
            //          $data['suppliers']= $this->m_common->customeQuery($sql);


            $data['suppliers'] = $this->m_common->get_row_array('supplier', array('ID' => $supplier_id), '*');
        } else {
            // $sql="select v.*,s.SUP_NAME,s.opening_balance,s.ID,d.dep_description from tbl_purchase_invoices v left join supplier s on v.supplier_id=s.ID left join department d on v.branch_id=d.d_id group by v.supplier_id";
            //   $data['suppliers']= $this->m_common->customeQuery($sql);
    ...
    function purchaseInvoiceReport()
    {
        ...
        if (!empty($postData)) {
            ...
            if (!empty($f_date) & !empty($to_date)) {
                $sql = "select v.*,o.order_no,s.SUP_NAME,d.dep_description,tbr.supplier_bill_no from tbl_purchase_invoices v left join tbl_bill_register tbr on v.bill_id=tbr.id left join tbl_purchase_orders o on v.po_id=o.o_id left join supplier s on tbr.supplier_id=s.ID left join department d on o.unit_id=d.d_id where $where and v.invoice_date>='" . $from_date . "' and v.invoice_date<='" . $too_date . "' order by v.invoice_date ASC";
            } else {
                $sql = "select v.*,o.order_no,s.SUP_NAME,d.dep_description,tbr.supplier_bill_no from tbl_purchase_invoices v left join tbl_bill_register tbr on v.bill_id=tbr.id left join tbl_purchase_orders o on v.po_id=o.o_id left join supplier s on tbr.supplier_id=s.ID left join department d on o.unit_id=d.d_id where $where  order by v.invoice_date ASC";
            }
    ...
    function invoiceAgingSummaryReport($print = false)
    {
        ...
        foreach ($data['suppliers'] as $key => $value) {
            ...
            $sql = "select v.*,s.SUP_NAME,tbr.supplier_bill_no from tbl_purchase_invoices v left join tbl_bill_register tbr on v.bill_id=tbr.id  left join supplier s on tbr.supplier_id=s.ID where v.is_active=1 and tbr.supplier_id=" . $value['ID'];
    ...
    function supplierBalanceReport($print = false)
    {
        $data['suppliers'] = $this->m_common->get_row_array('supplier', '', '*');
        $supplier_id = $this->input->post('supplier_id');
        ...
        foreach ($data['suppliers'] as $key => $value) {
            $total_check_payment = array();
            $total_cash_payment = array();

            $id = $value['id'];

            $sql = "select sum(ac.amount) as total_amount from assigned_cheque ac LEFT JOIN supplier s on ac.p_id= s.ID where ac.status='Completed' and ac.p_id=" . $value['ID'];
            $total_checkpayment = $this->m_common->customeQuery($sql);

            $cash_sql = "select sum(amount) as total_cash_payment from tbl_bills_payment where supplier_id=" . $value['ID'];
            $total_cashpayment = $this->m_common->customeQuery($cash_sql);

            $data['suppliers'][$key]['total_paid'] = $total_cashpayment[0]['total_cash_payment'] + $total_checkpayment[0]['total_amount'] + $value['opening_balance'];

            $tb_sql = "select sum(net_payable_amount) as total_bill_amount from tbl_purchase_invoices v where v.is_active=1 and v.supplier_id=" . $value['ID'];
            $total_bill = $this->m_common->customeQuery($tb_sql);

            $data['suppliers'][$key]['total_bill'] = $total_bill[0]['total_bill_amount'];
            if (empty($data['suppliers'][$key]['total_paid']) && empty($total_bill[0]['total_bill_amount'])) {
                unset($data['suppliers'][$key]);
            }
        }
        if ($print == false) {
            $this->load->view('report/v_supplier_balance_report', $data);
        } else {
            $html = $this->load->view('report/print_supplier_balance', $data, true);
            echo $html;
            exit;
        }
    }
    function supplierBalanceExcell($print = false)
    {
        ...
        if (!empty($postData)) {
            ...
            $object->getActiveSheet()->setCellValueByColumnAndRow(8, 2, 'Supplier Balance Report');
            ...
            $object->getActiveSheet()->getStyle('H1:K3')->applyFromArray($style);

            //   $table_columns = array("SL", "S. Order", "Customer Name.", "Project", "Pdc/Lc/Bg No.", "Pdc/Lc/Bg Date", "Mat Date", "Bank", "Value");
            $table_columns = array("SL", "Supplier Name", "Paid Amount", "Total Bill", "Due", "Surplus");
            ...
            if (!empty($data['suppliers'])) {
                ...
                $total_paid = 0;
                $i = 0;
                foreach ($data['suppliers'] as $cust) {
                    ...
                    $object->getActiveSheet()->setCellValueByColumnAndRow(6, $excel_row, $cust['SUP_NAME']);
                    $object->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row, number_format($cust['total_paid']),2);
                    $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, number_format($cust['total_bill'],2));
                    if($due>0)
                     $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, number_format($due,2));
                    else
                     $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, '');
                    if($due<0) 
                        $object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, number_format(($cust['total_paid']-$cust['total_bill']),2));
                    else
            ...
            $object->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row, $total_paid);
            $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, $net_bill);
            $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, $net_due);
            $object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, $net_balance);
            $object->getActiveSheet()->getStyle("H$excel_row:K$excel_row")->getFont()->setSize('12');
            $object->getActiveSheet()->getStyle("H$excel_row:K$excel_row")->getFont()->setBold(true);
            ...
            $object->getActiveSheet()->getStyle('H6:K' . $excel_row)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
            ...
            header('Content-Disposition: attachment;filename="supplier_balance.xls"');
