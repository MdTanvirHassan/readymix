      ...
      //  $this->sub_menu = 'item_information';
       ...
       // $data['items']=$this->m_common->get_row_array('v_items', '', '*');
        foreach($data['items'] as $key=>$item){
            ...
            $data['items'][$key]['file_name'] = $this->m_common->get_row_array('item_document', array('item_id'=>$item['id']), '*');
...
//        $exist_info=$this->m_common->get_row_array('items',array('item_group'=>$group_id,'item_type'=>$item_type,'item_name'=>$item_name,'origin'=>$origin,'port_no'=>$parts_no),'*');
//        if(!empty($exist_info)){
...
//        $data['purchase_orders'] = $this->m_common->customeQuery($sql);
 ...
 //       $mr_sql="select mrr.*,tpo.order_no from material_receive_requisition mrr left join tbl_purchase_orders tpo on mrr.po_id=tpo.o_id where mrr.mrr_id=".$id; //13-12-2020
        $mr_sql="select mrr.*,tpo.order_no,s.SUP_NAME from material_receive_requisition mrr left join tbl_purchase_orders tpo on mrr.po_id=tpo.o_id left join supplier s on tpo.supplier_id=s.ID where mrr.mrr_id=".$id;
