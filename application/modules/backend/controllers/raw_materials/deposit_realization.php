...
class Deposit_realization extends Site_Controller {
    ...
    function index() {
        ...
        $this->menu = 'trading';
        $this->sub_menu = 'rm_deposit_realization';
       ...
       // $where = "dr.unit_id=$branch_id";
        ...
        if (!empty($postData)) {
            ...
            if (!empty($customer_id)) {
             ...
             if(!empty($f_date) & !empty($to_date)){              
                 $sql = "select dr.*,pc.check_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id left join tbl_customers tc on pc.customer_id=tc.id where $where and dr.deposit_date>='" . $f_date . "' and dr.deposit_date<='" . $to_date . "' and dr.realization_status='Deposited' order by dr.deposit_date DESC";
             }else if (!empty($f_date)) {          
                 $sql = "select dr.*,pc.check_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id left join tbl_customers tc on pc.customer_id=tc.id where $where and dr.deposit_date>='".$f_date."' and dr.realization_status='Deposited' order by dr.deposit_date DESC";
             }else if (!empty($to_date)) {               
                 $sql = "select dr.*,pc.check_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id left join tbl_customers tc on pc.customer_id=tc.id where $where and dr.deposit_date<='".$to_date."' and dr.realization_status='Deposited' order by dr.deposit_date DESC";
             }else{
                
                 $sql = "select dr.*,pc.check_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id left join tbl_customers tc on pc.customer_id=tc.id where $where and dr.realization_status='Deposited' order by dr.deposit_date DESC";
             }   
        }else{
          
            $sql = "select dr.*,pc.check_date,pc.bg_issue_date,pc.po_date,pc.lc_date,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id left join tbl_customers tc on pc.customer_id=tc.id where dr.is_active=1 and dr.realization_status='Deposited' order by dr.deposit_date DESC";
            $data['f_date'] = $from_date = '';
            $data['to_date'] = $too_date = '';
        }
        
        $data['deposits'] = $this->m_common->customeQuery($sql);
        $this->load->view('raw_materials/deposit_realization/v_deposit_realization', $data);
    }
    ...
    function add_deposit() {
        ...
        $sql = "select tpc.*,tc.c_name,c_short_name from rm_payment_collections tpc left join tbl_customers tc on tpc.customer_id=tc.id where tpc.is_active=1 and tpc.collection_method!='Cash' and tpc.payment_status='Collected'";
        $data['collections'] = $this->m_common->customeQuery($sql);
        
        foreach ($data['customers'] as $key => $value){
            $collection_info = array();        
            $collection_sql = "select tpc.*,tc.c_name,c_short_name from rm_payment_collections tpc join tbl_customers tc on tpc.customer_id=tc.id where tpc.is_active=1 and (tpc.collection_method!='Cash') and (tpc.payment_status='Collected' || tpc.payment_status='Dishonored') and tpc.customer_id=". $value['id'];
        ...
        $this->load->view('raw_materials/deposit_realization/v_add_deposit', $data);
    }
     ...
     function collection_info() {
        ...
        $sql = "select tpc.*,tc.c_name,c_short_name from rm_payment_collections tpc left join tbl_customers tc on tpc.customer_id=tc.id where tpc.is_active=1 and (tpc.collection_method='Pdc' || tpc.collection_method='Bg' || tpc.collection_method='Lc' || tpc.collection_method='Po') and (tpc.payment_status='Collected' || tpc.payment_status='Dishonored') and tpc.customer_id=".$customer_id;
        $data['collections'] = $this->m_common->customeQuery($sql);
    ...
    function add_deposit_action(){
        ...
        if(!empty($postData)){
            if(empty($postData['collection_id'])){
                redirect_with_msg('raw_materials/deposit_realization/add_deposit', 'Please select at least one pdc/Bg/Lc');
            }
                     
            foreach($postData['collection_id'] as $value){
                $insertData = array();
                $pre_deposit='';
                $pre_deposit=$this->m_common->get_row_array('rm_deposit_realization', array('collection_id' =>$value,'realization_status' =>"Deposited", 'is_active' => 1), '*');
                ...
                $result=$this->m_common->insert_row('rm_deposit_realization',$insertData);
                if(!empty($result)){
                    $res=$this->m_common->update_row('rm_payment_collections', array('id' =>$value),array('payment_status'=>"Deposited"));
                }
            }
            if (!empty($result)) {
                redirect_with_msg('raw_materials/deposit_realization', 'Successfully Inserted');
            }
        } else {
            redirect_with_msg('raw_materials/deposit_realization/add_deposit', 'Please fill the form and submit');
        }
    }
    ...
    function add_deposit_action_() {
        ...
        if (!empty($postData)) {
            ...
            if (!empty($postData['collection_id'])) {
                ...
                $pre_deposit = $this->m_common->get_row_array('rm_deposit_realization', array('collection_id' => $postData['collection_id'], 'realization_status' =>"Deposited", 'is_active' => 1), '*');
                if(!empty($pre_deposit)){
                    redirect_with_msg('raw_materials/deposit_realization/add_deposit', 'This collection already exists');
                }else{
                    $pre_deposit_info = $this->m_common->get_row_array('rm_deposit_realization',array('collection_id' => $postData['collection_id'], 'realization_status' => "Dishonored", 'is_active' => 1), '*');
                    if (count($pre_deposit_info) == 3) {
                        redirect_with_msg('raw_materials/deposit_realization/add_deposit', 'You are not able to deposit more than three times');
                    }
                }
            }
            if (!empty($postData['deposit_date'])) {
    ...
    function edit_deposit($id) {
        ...
        $data['banks'] = $this->m_common->get_row_array('rm_banks', array('is_active' => 1), '*');
        $sql = "select dr.*,pc.collection_method,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id where dr.is_active=1 and dr.id=" . $id;
        $data['deposit_info'] = $this->m_common->customeQuery($sql);
        
        $sql1 = "select tpc.*,tc.c_name,c_short_name from rm_payment_collections tpc left join tbl_customers tc on tpc.customer_id=tc.id where tpc.is_active=1 and tpc.collection_method!='Cash' and tpc.payment_status!='Returned'";
        $data['collections'] = $this->m_common->customeQuery($sql1);
        $this->load->view('raw_materials/deposit_realization/v_edit_deposit', $data);
    }
    ...
    function edit_deposit_action($id) {
        ...
        if (!empty($postData)) {
            ...
            $result = $this->m_common->update_row('rm_deposit_realization', array('id' => $id), $insertData);
            if ($result >= 0) {
                redirect_with_msg('raw_materials/deposit_realization', 'Successfully Updated');
            }
        } else {
            redirect_with_msg('raw_materials/deposit_realization/edit_deposit/' . $id, 'Please fill the form and submit');
        }
    }

    function delete_deposit($id){
        if(!empty($id)){
            $deposit_info=$this->m_common->get_row_array('rm_deposit_realization',array('id'=>$id),'*');
            $col_info=$this->m_common->get_row_array('rm_payment_collections',array('id'=>$deposit_info[0]['collection_id']),'*');
            $dishonor_info=$this->m_common->get_row_array('rm_deposit_realization',array('collection_id'=>$deposit_info[0]['collection_id'],'realization_status'=>"Dishonored"));
            $count=count($dishonor_info);
            $id=$this->m_common->update_row('rm_deposit_realization',array('id' =>$id),array('is_active'=>0));
            if(!empty($id)){
                if($count>0){
                    $this->m_common->update_row('rm_payment_collections',array('id'=>$deposit_info[0]['collection_id']),array('payment_status'=>'Dishonored'));
                }else{
                    $this->m_common->update_row('rm_payment_collections',array('id'=>$deposit_info[0]['collection_id']),array('payment_status'=>'Collected'));
                }
                redirect_with_msg('raw_materials/deposit_realization/index', 'Successfully Deleted');
            } else {
                redirect_with_msg('raw_materials/deposit_realization/index', 'Data not deleted for an unexpected error');
            }
        } else {
            redirect_with_msg('raw_materials/deposit_realization/index', 'Please click on delete button');
        }
    }
    ...
    function get_collection_info() {
        ...
        $sql ="select pc.*,b.b_name,b.branch_name,b.b_short_name from rm_payment_collections pc left join tbl_banks b on pc.bank_id=b.id where  pc.id=".$id;
    ...
    function realization() {
        ...
        if($honor_status=='honor'){
            $result=$this->m_common->update_row('rm_deposit_realization', array('id' => $deposit_id), array('realization_status' => "Honored", 'realization_date' => date('Y-m-d', strtotime($realization_date))));
            if($result>=0){
                $deposit_info = $this->m_common->get_row_array('rm_deposit_realization', array('id' => $deposit_id), '*');
                $collection_info = $this->m_common->get_row_array('rm_payment_collections', array('id' => $deposit_info[0]['collection_id']), '*');
                $customer_info = $this->m_common->get_row_array('tbl_customers', array('id' => $collection_info[0]['customer_id']), '*');
                $res = $this->m_common->update_row('rm_payment_collections', array('id' => $deposit_info[0]['collection_id']), array('payment_status' =>"Received"));
            ...
            $deposit_info = $this->m_common->get_row_array('rm_deposit_realization', array('id' => $deposit_id), '*');
            $result=$this->m_common->update_row('rm_deposit_realization',array('id' =>$deposit_id),array('realization_status' =>"Dishonored",'realization_date' =>date('Y-m-d',strtotime($realization_date))));
            $res = $this->m_common->update_row('rm_payment_collections', array('id' => $deposit_info[0]['collection_id']), array('payment_status'=>"Dishonored",'dishonor_date'=>date('Y-m-d',strtotime($realization_date))));
            if ($result >= 0){
                $data['status'] = 'success';
            }
        }
       ...
       // $where = "dr.unit_id=$branch_id";
        ...
        if (!empty($postData)) {
            ...
            if (!empty($customer_id)) {
             ...
             if(!empty($f_date) & !empty($to_date)){                              
                 $sql = "select dr.*,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id left join tbl_customers tc on pc.customer_id=tc.id  where $where and dr.deposit_date>='" . $f_date . "' and dr.deposit_date<='" . $to_date . "' and dr.realization_status='Honored' order by dr.deposit_date DESC";
             }else if (!empty($f_date)){              
                 $sql = "select dr.*,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id left join tbl_customers tc on pc.customer_id=tc.id  where $where and dr.deposit_date>='".$f_date."' and dr.realization_status='Honored' order by dr.deposit_date DESC";
             }else if (!empty($to_date)){              
                 $sql = "select dr.*,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id left join tbl_customers tc on pc.customer_id=tc.id  where $where and dr.deposit_date<='" . $to_date . "' and dr.realization_status='Honored' order by dr.deposit_date DESC";
             }else{
               
                 $sql = "select dr.*,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id left join tbl_customers tc on pc.customer_id=tc.id  where $where and dr.realization_status='Honored' order by dr.deposit_date DESC";
             }   
        }else{
        
            $sql = "select dr.*,pc.mrr_no,pc.remark as c_remark,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name from rm_deposit_realization dr left join rm_payment_collections pc on dr.collection_id=pc.id left join rm_banks b on dr.bank_id=b.id left join tbl_customers tc on pc.customer_id=tc.id where dr.is_active=1 and dr.realization_status='Honored' order by dr.deposit_date DESC";
            $data['f_date'] = $from_date = '';
            $data['to_date'] = $too_date = '';
        }
        
        $data['deposits'] = $this->m_common->customeQuery($sql);
        $this->load->view('raw_materials/deposit_realization/v_realized_cheque', $data);
    }
    ...
    function return_deposit_action(){
        ...
        $deporit_row = $this->m_common->get_row_array('rm_deposit_realization',array('id'=>$diposit_id),'collection_id');
        ...
        $insertData = array();
            ...
            $id = $this->m_common->insert_row('rm_payment_return', $insertData);
            if(!empty($id)){
                $this->m_common->update_row('rm_deposit_realization',array('id'=>$diposit_id),array('realization_status'=>'Returned'));  
            }
        redirect_with_msg('raw_materials/deposit_realization/index', 'Deposit Amount Returned successfully'); 
    }
