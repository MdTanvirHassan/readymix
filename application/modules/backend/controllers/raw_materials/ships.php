...
class Ships extends Site_Controller {
    ...
    function index() {
        ...
        $this->sub_inner_menu = 'ship';
        $this->titlebackend("Ships");
        $data['trucks']=$this->m_common->get_row_array('tbl_truck',array('is_active'=>1,'transport_type'=>'Ship'),'*');
        $this->load->view('raw_materials/ship/v_truck',$data);
    }
    ...
    function add_truck() {
        ...
        $this->sub_menu = 'rm_challan';
        $this->sub_inner_menu = 'ship';
        $this->titlebackend("Add Ship");   
        $this->load->view('raw_materials/ship/v_add_truck');
    }
         ...
         if(!empty($postData)){
            ...
            $insertData['transport_type']='Ship';
             ...
             if(!empty($result)){
                  ...
                  redirect_with_msg('raw_materials/ships', 'Successfully Inserted');
             }
         }else{
              redirect_with_msg('raw_materials/ships/add_truck', 'Please fill the form and submit');
         }
    ...
    function edit_truck($id) {
        ...
        $this->sub_menu = 'rm_challan';
        $this->sub_inner_menu = 'ship';
        $this->titlebackend("Edit Ship");
        $data['truck_info']=$this->m_common->get_row_array('tbl_truck',array('truck_id'=>$id),'*');           
        $this->load->view('raw_materials/ship/v_edit_truck',$data);
    }
        ...
        $employee_id = $this->session->userdata('employeeId');
         ...
         if(!empty($postData)){
            ...
            if(!empty($postData['tax_token'])){
             ...
             if($result>=0){              
                  redirect_with_msg('raw_materials/ships', 'Successfully Updated');
             }
         }else{
              redirect_with_msg('raw_materials/ships', 'Please fill the form and submit');
         }
    ...
    function details_truck($id,$print=false) {
        ...
        $this->sub_menu = 'rm_challan';
        $this->sub_inner_menu = 'ship';
        $this->titlebackend("Details Ship");
        $data['truck_info']=$this->m_common->get_row_array('tbl_truck',array('truck_id'=>$id),'*');      
        $this->load->view('raw_materials/ship/v_details_truck',$data);
    ...
    function delete_truck($id) {
        ...
        if(!empty($id)) {
            ...
            if(!empty($id)) {
                redirect_with_msg('raw_materials/ships/index', 'Successfully Deleted');
            } else {
                redirect_with_msg('raw_materials/ships/index', 'Data not deleted for an unexpected error');
            }
        } else {
            redirect_with_msg('raw_materials/ships/index', 'Please click on delete button');
        }
    }
