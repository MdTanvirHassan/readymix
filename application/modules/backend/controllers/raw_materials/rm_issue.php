...
class Rm_issue extends Site_Controller {
    ...
    function index() {
        ...
        $this->sub_inner_menu = 'rm_issue';
        $this->titlebackend("RM Issue");
       
        $branch_id= $this->session->userdata('companyId');

     
        $sql="select tic.*,tc.sr_no,tc.sr_date,tc.issue_no,tc.issue_date,i.item_name,i.item_code,d.dept_name from rm_issue_details tic 
left join rm_issue tc on tic.issue_id=tc.id             
left join rm_items i on i.id = tic.item_id
left join tbl_departments d on tc.dept_id=d.id where tc.issue_type='Production Issue' and tc.branch_id=".$branch_id ." order by tic.id DESC";
         
        $data['item_consumption']=$this->m_common->customeQuery($sql);
        $this->load->view('raw_materials/rm_issue/v_issue',$data);
    }
    
    
    
    
    function add_rm_issue() {
        $this->menu = 'general_store';
        ...
        $all_issue=$this->m_common->get_row_array('rm_issue',array('branch_id'=>$branch_id,'issue_type'=>"Production Issue",'is_active'=>1),'*'); 
        if(!empty($all_issue)){
        ...
        if(!empty($_POST)){
           ...
           $consumptionInfo['sr_date']= date('Y-m-d',  strtotime($postData['sr_date']));
           $consumptionInfo['sr_no']= $postData['sr_no'];           
           $consumptionInfo['dept_id']= $postData['dept_id'];
           $consumptionInfo['created_date']=date('Y-m-d');
           $consumptionInfo['created_by']=$employee_id;
           $consumptionInfo['issue_type']="Production Issue";
         ...
         redirect_with_msg('raw_materials/rm_issue', 'Consumption  Successfully Inserted');  
        }else{
            
         
            
            
         $this->load->view('raw_materials/rm_issue/v_add_issue',$data);   
        }
        
    }
    
    function edit_rm_issue($issue_id){
        $this->menu = 'general_store';
        $this->sub_menu = 'consumption';
        $this->sub_inner_menu = 'rm_issue';
        $this->titlebackend("RM Issue");
...
left join tbl_departments d on tc.dept_id=d.id  where tic.issue_id=".$issue_id;
        ...
        if(!empty($_POST)){
         ...
         redirect_with_msg('raw_materials/rm_issue/details_rm_issue/'.$issue_id, 'Successfully Updated');  
        }else{                        
           $this->load->view('raw_materials/rm_issue/v_edit_issue',$data);   
        }
        
    }
    
    function details_rm_issue($issue_id,$print=false){
        $this->menu = 'general_store';
        $this->sub_menu = 'consumption';
        $this->sub_inner_menu = 'rm_issue';
        $this->titlebackend("RM Issue");
...
left join tbl_departments d on tc.dept_id=d.id  where tic.issue_id=".$issue_id;
        ...
        if($print==false){
            $this->load->view('raw_materials/rm_issue/v_details_issue',$data);   
        }else{
           $html=$this->load->view('raw_materials/rm_issue/print_issue',$data,true);
           echo $html;exit; 
        }
    ...
    function delete_rm_issue($consumption_id){
        if (!empty($consumption_id)){
            ...
            if (!empty($id)) {
                ...
                redirect_with_msg('raw_materials/rm_issue', 'Successfully Deleted');
            } else {
                redirect_with_msg('raw_materials/rm_issue', 'Data not deleted for an unexpected error');
            }
        } else {
            redirect_with_msg('raw_materials/rm_issue', 'Please click on delete button');
        }
    }
    ...
    function approved_consumption($consumption_id){
        ...
        if(!empty($consumption_id)){
            $consumption=$this->m_common->get_row_array('rm_issue_details',array('issue_id'=>$consumption_id),'*');           
            if(!empty($consumption)){
                       
                $this->m_common->update_row('rm_issue_details',array('issue_id'=>$consumption_id),array('status'=>'Approved'));
               
               redirect_with_msg('raw_materials/rm_issue', 'Successfully Approved');
            } else {
                redirect_with_msg('raw_materials/rm_issue', 'Data not Approved for an unexpected error');
            }
        } else {
            redirect_with_msg('raw_materials/rm_issue', 'Please click on Approved button');
        }
    }
