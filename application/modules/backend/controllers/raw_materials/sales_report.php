...
class Sales_report extends Site_Controller {
    ...
    function salesMisReport(){
        ...
        foreach($data['lcs'] as $key=>$value){
           $lc_receive_info=array(); 
           $lc_receive_sql="select sum(lrd.survey_qty) as total_receive_qty,lrd.unit_price from rm_lc_receive_details
             lrd left join import_lc_details ld on lrd.lc_details_id=ld.id
             left join import_lc lc on ld.lc_id=lc.lc_id
             where lc.lc_id=".$value['lc_id'];
           $lc_receive_info=$this->m_common->customeQuery($lc_receive_sql);
           
           $hook_sale_info=array();
           $hook_sale_or_receive_sql="select sum(dcd.challan_qty) as total_challan_qty,sum(dcd.amount) as total_amount from rm_delivery_challan_details dcd
             left join rm_delivery_challans dc on dcd.dc_id=dc.dc_id
             left join import_lc_details ld on dcd.lc_details_id=ld.id
             left join import_lc lc on ld.lc_id=lc.lc_id
             where dc.delivery_location='Hook' and  lc.lc_id=".$value['lc_id'];
           $hook_sale_info=$this->m_common->customeQuery($hook_sale_or_receive_sql);
           
           
           $yard_sale_info=array();
           $yard_sale_or_receive_sql="select sum(dcd.challan_qty) as total_challan_qty,sum(dcd.amount) as total_amount from rm_delivery_challan_details dcd
             left join rm_delivery_challans dc on dcd.dc_id=dc.dc_id
             left join import_lc_details ld on dcd.lc_details_id=ld.id
             left join import_lc lc on ld.lc_id=lc.lc_id
             where dc.delivery_location='Yard' and  lc.lc_id=".$value['lc_id'];
           $yard_sale_info=$this->m_common->customeQuery($yard_sale_or_receive_sql);
           
           
           $yard_receive_info=array(); 
           $yard_receive_sql="select sum(lrd.survey_qty) as total_receive_qty,sum(lrd.amount) as total_amount from rm_receive_details
             lrd left join import_lc_details ld on lrd.lc_details_id=ld.id
             left join import_lc lc on ld.lc_id=lc.lc_id
             where lc.lc_id=".$value['lc_id'];
           $yard_receive_info=$this->m_common->customeQuery($yard_receive_sql);
           
          
           $landed_cost_info=array();
           $lc_sql="select * from import_lc_landed_cost where lc_id=".$value['lc_id'];
           $landed_cost_info=$this->m_common->customeQuery($lc_sql);
           
           
           
           $data['lcs'][$key]['lc_qty']=$lc_receive_info[0]['total_receive_qty']; 
           $data['lcs'][$key]['lc_acceptance_amount']=$lc_receive_info[0]['total_receive_qty']*$lc_receive_info[0]['unit_price']; 
           
           $total_receive=0;
           //$yard_sale=0;
           $yard_receive=0;
           $yard_stock=0;
           $hook_average_sale_price=0;
           $yard_average_sale_price=0;
           $hook_sale_qty=0;
           $yard_sale_qty=0;
           $hook_sale_amount=0;
           $yard_sale_amount=0;
           $hook_profit_or_loss=0;
           $yard_profit_or_loss=0;
           
           if(isset($hook_sale_info[0]['total_challan_qty'])){       
              $data['lcs'][$key]['hook_receive']=$hook_sale_info[0]['total_challan_qty'];
              $total_receive=$total_receive+$hook_sale_info[0]['total_challan_qty'];
              $hook_average_sale_price=round($hook_sale_info[0]['total_amount']/$hook_sale_info[0]['total_challan_qty'],2);
           }else{
              $data['lcs'][$key]['hook_receive']=0; 
           }
           
           if(isset($yard_receive_info[0]['total_receive_qty'])){
               $data['lcs'][$key]['yard_receive']=$yard_receive_info[0]['total_receive_qty']; 
               $total_receive=$total_receive+$yard_receive_info[0]['total_receive_qty']; 
               $yard_receive=$yard_receive_info[0]['total_receive_qty']; 
           }else{
               $data['lcs'][$key]['yard_receive']=0;
           }
           
           $data['lcs'][$key]['total_receive']=$total_receive; 
           
           if(isset($hook_sale_info[0]['total_challan_qty'])){ 
                $data['lcs'][$key]['hook_sales_qty']=$hook_sale_info[0]['total_challan_qty'];
                $hook_sale_qty=$hook_sale_info[0]['total_challan_qty'];
           }else{
               $data['lcs'][$key]['hook_sales_qty']=0; 
           }
           
           if(isset($yard_sale_info[0]['total_challan_qty'])){
               $data['lcs'][$key]['yard_sales_qty']=$yard_sale_info[0]['total_challan_qty']; 
               $yard_sale_qty=$yard_sale_info[0]['total_challan_qty']; 
               $yard_average_sale_price=round($yard_sale_info[0]['total_challan_qty']/$yard_sale_info[0]['total_challan_qty'],2);
           }else{
               $data['lcs'][$key]['yard_sales_qty']=0;
           }
           
           $yard_stock=$yard_receive-$yard_sale_qty;
           
           $data['lcs'][$key]['yard_stock']=$yard_receive-$yard_sale_qty; 
           
           if(!empty($landed_cost_info)){
               $yard_rate=round($landed_cost_info[0]['grand_total']/$lc_receive_info[0]['total_receive_qty'],2);
               $hook_rate=round($landed_cost_info[0]['grand_total']/$lc_receive_info[0]['total_receive_qty'],2);
               $data['lcs'][$key]['hook_landed_cost']=$hook_rate;  
               $data['lcs'][$key]['yard_landed_cost']=$yard_rate; 
           }else{
               $data['lcs'][$key]['hook_landed_cost']=0;  
               $data['lcs'][$key]['yard_landed_cost']=0;
           }
           
           
          
           
           $data['lcs'][$key]['hook_sale_avg_price']=$hook_average_sale_price;
           $data['lcs'][$key]['yard_sale_avg_price']=$yard_average_sale_price;
           
           
           $data['lcs'][$key]['hook_profit']=$hook_average_sale_price-$hook_rate; 
           $data['lcs'][$key]['yard_profit']=$yard_average_sale_price-$yard_rate; 
           
           if(isset($hook_sale_info[0]['total_amount'])){
                $data['lcs'][$key]['hook_sale_amount']=$hook_sale_info[0]['total_amount'];
                $hook_sale_amount=$yard_sale_info[0]['total_amount'];
           }else{
                $data['lcs'][$key]['hook_sale_amount']=0;
           }
           
           if(isset($yard_sale_info[0]['total_amount'])){
              $data['lcs'][$key]['yard_sale_amount']=$yard_sale_info[0]['total_amount'];
              $yard_sale_amount=$yard_sale_info[0]['total_amount'];
           }else{
              $data['lcs'][$key]['yard_sale_amount']=0;
           }
           
           $data['lcs'][$key]['total_sale_amount']=$hook_sale_amount+$yard_sale_amount;
           $hook_cost_amount=$hook_rate*$hook_sale_qty;
           $yard_cost_amount=$yard_rate*$yard_sale_qty;
           
           $hook_profit_or_loss=$hook_sale_amount-$hook_cost_amount;
           $yard_profit_or_loss=$yard_sale_amount-$yard_cost_amount;
           
           $data['lcs'][$key]['hook_profit_or_loss']=$hook_profit_or_loss;
           $data['lcs'][$key]['yard_profit_or_loss']=$yard_profit_or_loss;
           $data['lcs'][$key]['total_profit_or_loss']=$hook_profit_or_loss+$yard_profit_or_loss;
           
           $data['lcs'][$key]['discharge_profit']=0;
           $data['lcs'][$key]['goods_shortage']=0;
           $data['lcs'][$key]['grand_total_profit']=$hook_profit_or_loss+$yard_profit_or_loss;
        }
        $this->load->view('raw_materials/sales_report/v_sales_mis_report',$data);
    }
    ...
    function delivery_order_info() {
       ...
       // $data['order_info'] = $this->m_common->get_row_array('rm_delivery_orders', array('customer_id' =>$customer_id,'is_active' => 1),'*');
        $do_sql="select * from rm_delivery_orders where (customer_id=$customer_id and is_active=1 and branch_id=$branch_id) and do_location='Yard'";
        $data['order_info'] = $this->m_common->customeQuery($do_sql);
    ...
    function doBalanceReport($print = false) {
        ...
        $data['closing_status']=$status=$this->input->post('closing_status');
        if(!empty($status)){
            $where = "do.branch_id=".$branch_id." and do.closing_status='$status' and dod.is_active=1 and do.do_location='Yard'";
        }else{
            $where = "do.branch_id=".$branch_id." and dod.is_active=1 and do.do_location='Yard'";
        }
        $postData = $this->input->post();
        if (!empty($postData)) {
            ...
            if(!empty($customer_id)){
               // $data['all_orders'] = $this->m_common->get_row_array('rm_delivery_orders', array('customer_id' =>$customer_id,'is_active' => 1,'branch_id'=>$branch_id),'*');
                $do_sql="select * from rm_delivery_orders where (is_active=1 and branch_id=$branch_id and customer_id=$customer_id) and do_location='Yard'";
                $data['all_orders'] = $this->m_common->customeQuery($do_sql);
            }else{
                
                //$data['all_orders'] = $this->m_common->get_row_array('rm_delivery_orders', array('is_active' => 1,'branch_id'=>$branch_id),'*');
                $do_sql="select * from rm_delivery_orders where (is_active=1 and branch_id=$branch_id) and do_location='Yard'";
                $data['all_orders'] = $this->m_common->customeQuery($do_sql);
            } 
            ...
            if (!empty($f_date) & !empty($to_date)) {
                $sql="select dod.*,(select sum(quantity) as total_c_qty from rm_delivery_challan_details where do_details_id=dod.do_details_id) as total_challan_qty,do.closing_status,do.delivery_no,do.delivery_order_date,tc.c_name,ri.item_name,tmu.meas_unit from rm_delivery_orders_details dod 
          ...
          //  $data['all_orders'] = $this->m_common->get_row_array('rm_delivery_orders', array('is_active' => 1,'branch_id'=>$branch_id), '*');
          //  $do_sql="select * from rm_delivery_orders where (is_active=1 and branch_id=$branch_id) or do_location='Hook'";
            $do_sql="select * from rm_delivery_orders where do_location='Yard' and is_active=1 and branch_id=$branch_id";
            $data['all_orders'] = $this->m_common->customeQuery($do_sql);
    ...
    function doBalanceReportExcel() {
        ...
        //$where = "do.branch_id=".$branch_id." and do.closing_status='Open' and dod.is_active=1 and do.do_location='Yard'";
        $postData = $this->input->post();
        if (!empty($postData)) {
            ...
            if (!empty($f_date) & !empty($to_date)) {
                $sql="select dod.*,do.closing_status,do.delivery_no,do.delivery_order_date,tc.c_name,ri.item_name,tmu.meas_unit from rm_delivery_orders_details dod 
                  left join rm_delivery_orders do on dod.do_id=do.do_id
                  left join tbl_customers tc on do.customer_id=tc.id
                  left join rm_items ri on dod.s_item_id=ri.id
                  left join tbl_measurement_unit tmu on ri.mu_id=tmu.id
    ...
    function hookDoBalanceReport($print = false) {
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sales_report';
        $this->titlebackend("Report");
        ...
        $where = '';
        //$where = "do.branch_id=$branch_id";
        $where = "dod.is_active=1 and do.do_location='Hook'";
        $postData = $this->input->post();
        if (!empty($postData)) {
            ...
            if(!empty($customer_id)){
               // $data['all_orders'] = $this->m_common->get_row_array('rm_delivery_orders', array('customer_id' =>$customer_id,'is_active' => 1,'branch_id'=>$branch_id),'*');
                $do_sql="select * from rm_delivery_orders where (is_active=1 and customer_id=$customer_id) and do_location='Hook'";
                $data['all_orders'] = $this->m_common->customeQuery($do_sql);
            }else{
                
                //$data['all_orders'] = $this->m_common->get_row_array('rm_delivery_orders', array('is_active' => 1,'branch_id'=>$branch_id),'*');
                $do_sql="select * from rm_delivery_orders where is_active=1 and do_location='Hook'";
                $data['all_orders'] = $this->m_common->customeQuery($do_sql);
            } 
          ...
          //  $data['all_orders'] = $this->m_common->get_row_array('rm_delivery_orders', array('is_active' => 1,'branch_id'=>$branch_id), '*');
          //  $do_sql="select * from rm_delivery_orders where (is_active=1 and branch_id=$branch_id) or do_location='Hook'";
            $do_sql="select * from rm_delivery_orders where do_location='Hook' and is_active=1";
            $data['all_orders'] = $this->m_common->customeQuery($do_sql);
        ...
        if ($print == false) {
            $this->load->view('raw_materials/sales_report/v_hook_do_balance_report', $data);
        } else {
            $html = $this->load->view('raw_materials/sales_report/print_hook_do_balance_report', $data, true);
            echo $html;
            exit;
        }
    }

    function hookDoBalanceReportExcel() {
        $this->load->library("PHPExcel");
        $branch_id = $this->session->userdata('companyId');
        $data['branch_info'] = $this->m_common->get_row_array('department', array('d_id' => $branch_id), '*');
        $where = '';
        ...
        $object->getActiveSheet()->getStyle('I3')->getFont()->setSize('13');
        $object->getActiveSheet()->getStyle('I3')->getFont()->setBold(true);
        $object->getActiveSheet()->setCellValueByColumnAndRow(8, 3, 'Hook Delivery Order Balance');
        $style = array(
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
            )
        ...
        $object_writer = PHPExcel_IOFactory::createWriter($object, 'Excel5');
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="hook_delivery_order_balance_report.xls"');
        $object_writer->save('php://output');
    }
...
//           }
    ...
    function allDeliveryChallan($print = false) {
        ...
        $data['branch_info'] = $this->m_common->get_row_array('department', array('d_id' => $branch_id), '*');
        $where = '';
        $where = "dc.delivery_location='Yard' and dc.is_active=1 and dc.unit_id=$branch_id ";
        $postData = $this->input->post();
        if (!empty($postData)) {
            ...
            if(!empty($customer_id)){
                ...
                //$do_sql="select * from rm_delivery_orders where (is_active=1 and customer_id=$customer_id and branch_id=$branch_id) or do_location='Hook'";
                $do_sql="select * from rm_delivery_orders where do_location='Yard' and is_active=1 and customer_id=$customer_id and branch_id=$branch_id";
                $data['all_orders'] = $this->m_common->customeQuery($do_sql);
            }else{
...
//            if($delivery_location=="Yard" || $delivery_location=''){
//                if (empty($order_id)) {
//                    $where = " and dc.unit_id=$branch_id ";
//                }
//            }
            ...
            if(!empty($f_date) & !empty($to_date)){
               // $sql = "select dcd.*,dc.dc_no,dc.delivery_challan_date,do.delivery_no,do.delivery_order_date,c.c_name,p.item_name,dc.driver_name,dc.truck_no,tmu.meas_unit from rm_delivery_challan_details dcd left join rm_delivery_challans dc on dcd.dc_id=dc.dc_id left join rm_delivery_orders_details dod on dcd.do_details_id=dod.do_details_id left join rm_delivery_orders do on dod.do_id=do.do_id left join tbl_customers c on do.customer_id=c.id left join rm_items p on dcd.s_item_id=p.id left join tbl_measurement_unit tmu on p.mu_id=tmu.id left join import_lc_details lcd on dcd.lc_details_id=lcd.id where $where and dc.status='Approved' and dc.delivery_challan_date>='" . $from_date . "' and dc.delivery_challan_date<='" . $too_date . "' order by dc.delivery_challan_date desc";
                $sql = "select dcd.*,dc.dc_no,dc.manual_dc_no,dc.delivery_challan_date,dc.print_date_time,do.delivery_no,do.delivery_order_date,c.c_name,p.item_name,dc.driver_name,dc.truck_no,tmu.meas_unit,rlcr.mother_vessel_name,dod.transport_cost,dod.base_price,dod.commission
...
//            }
        ...
        $table_columns = array("SL","Vessel Name", "C. Date.","C. Time", "C. No.","M. C. No.", "D.O.","C.Name","Truck No", "D.Name","P.Name", "M.Unit", "Quantity","Base Price", "Stone Value","Commission Rate", "Commission Value","Transport Rate","Transport Amount", "Total Unit Price", "Total Value","Bill Status");
        ...
        $object->getActiveSheet()->getStyle('F5:AA5')->getFont()->setSize('12');
        $object->getActiveSheet()->getStyle('F5:AA5')->getFont()->setBold(true);
        ...
        if (!empty($data['challans'])) {
            ...
            foreach ($data['challans'] as $challan) {
                ...
                if(!empty($challan['print_date_time'])){
                    $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, date('d-m-Y h:i a', strtotime($challan['print_date_time'])));
                }else{
                    $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row,'');
                }
                $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, $challan['dc_no']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, $challan['manual_dc_no']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(11, $excel_row, $challan['delivery_no']);
                
                $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row, $challan['c_name']);
                ...
                $object->getActiveSheet()->setCellValueByColumnAndRow(15, $excel_row, $challan['item_name']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(16, $excel_row, $challan['meas_unit']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(17, $excel_row, round($challan['quantity'], 2));
                
                $object->getActiveSheet()->setCellValueByColumnAndRow(18, $excel_row, $challan['base_price']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(19, $excel_row, round($challan['quantity']*$challan['base_price'], 2));
                
                $object->getActiveSheet()->setCellValueByColumnAndRow(20, $excel_row, $challan['commission']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(21, $excel_row, round($challan['quantity']*$challan['commission'], 2));
                
                $object->getActiveSheet()->setCellValueByColumnAndRow(22, $excel_row, $challan['transport_cost']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(23, $excel_row, round($challan['quantity']*$challan['transport_cost'], 2));
                
                $object->getActiveSheet()->setCellValueByColumnAndRow(24, $excel_row, $challan['unit_price']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(25, $excel_row, round($challan['quantity']*$challan['unit_price'], 2));
                $object->getActiveSheet()->setCellValueByColumnAndRow(26, $excel_row, $challan['bill_status']);
        ...
        $object->getActiveSheet()->setCellValueByColumnAndRow(19, $excel_row,round($total_stone_amount, 2));
        
        $object->getActiveSheet()->setCellValueByColumnAndRow(20, $excel_row, '');
        $object->getActiveSheet()->setCellValueByColumnAndRow(21, $excel_row,round($total_commission, 2));
        
        $object->getActiveSheet()->setCellValueByColumnAndRow(22, $excel_row, '');
        $object->getActiveSheet()->setCellValueByColumnAndRow(23, $excel_row, round($total_transport_value, 2));

        
        $object->getActiveSheet()->setCellValueByColumnAndRow(24, $excel_row, '');
        $object->getActiveSheet()->setCellValueByColumnAndRow(25, $excel_row,round($total_value, 2));
        ...
        $object->getActiveSheet()->getStyle('F5:AA' . $excel_row)->applyFromArray($styleArray);
        ...
        $object_writer = PHPExcel_IOFactory::createWriter($object, 'Excel5');
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="challan_report.xls"');
        $object_writer->save('php://output');
    }    
    function allHookDeliveryChallan($print = false) {
        $this->menu = 'trading';
        $this->sub_menu = 'report';
        $this->sub_inner_menu = 'report';
        $this->titlebackend("Report");
        ...
        $data['branch_info'] = $this->m_common->get_row_array('department', array('d_id' => $branch_id), '*');
        $where = '';
        $where = "dc.delivery_location='Hook' and dc.is_active=1";
        $postData = $this->input->post();
        if (!empty($postData)) {
            ...
            if(!empty($customer_id)){
                //$data['all_orders'] = $this->m_common->get_row_array('rm_delivery_orders', array('is_active' =>1,'customer_id'=>$customer_id), '*');
                //$do_sql="select * from rm_delivery_orders where (is_active=1 and customer_id=$customer_id and branch_id=$branch_id) or do_location='Hook'";
                $do_sql="select * from rm_delivery_orders where do_location='Hook' and is_active=1 and customer_id=$customer_id";
                $data['all_orders'] = $this->m_common->customeQuery($do_sql);
            }else{
                //$data['all_orders'] = $this->m_common->get_row_array('rm_delivery_orders', array('is_active' => 1), '*');
              //  $do_sql="select * from rm_delivery_orders where (is_active=1 and branch_id=$branch_id) or do_location='Hook'";
...
//            }
            ...
            if(!empty($f_date) & !empty($to_date)){
               // $sql = "select dcd.*,dc.dc_no,dc.delivery_challan_date,do.delivery_no,do.delivery_order_date,c.c_name,p.item_name,dc.driver_name,dc.truck_no,tmu.meas_unit from rm_delivery_challan_details dcd left join rm_delivery_challans dc on dcd.dc_id=dc.dc_id left join rm_delivery_orders_details dod on dcd.do_details_id=dod.do_details_id left join rm_delivery_orders do on dod.do_id=do.do_id left join tbl_customers c on do.customer_id=c.id left join rm_items p on dcd.s_item_id=p.id left join tbl_measurement_unit tmu on p.mu_id=tmu.id left join import_lc_details lcd on dcd.lc_details_id=lcd.id where $where and dc.status='Approved' and dc.delivery_challan_date>='" . $from_date . "' and dc.delivery_challan_date<='" . $too_date . "' order by dc.delivery_challan_date desc";
                $sql = "select dcd.*,dc.dc_no,dc.manual_dc_no,dc.delivery_challan_date,dc.print_date_time,do.delivery_no,do.delivery_order_date,c.c_name,p.item_name,dc.driver_name,dc.truck_no,tmu.meas_unit,rlcr.mother_vessel_name
                        from rm_delivery_challan_details dcd
                        left join rm_delivery_challans dc on dcd.dc_id=dc.dc_id 
                        left join rm_delivery_orders_details dod on dcd.do_details_id=dod.do_details_id 
                        left join rm_delivery_orders do on dod.do_id=do.do_id 
        ...
        $data['items']=$this->m_common->get_row_array('rm_items',array('is_active'=>1),'*');
        if($print == false){
            $this->load->view('raw_materials/sales_report/v_all_hook_delivery_challan', $data);
        }else{
            $html = $this->load->view('raw_materials/sales_report/print_all_hook_delivery_challan', $data, true);
            echo $html;
            exit;
        }
    }

    function allHookDeliveryChallanExcel($print = false) {
        $this->load->library("PHPExcel");
        $branch_id = $this->session->userdata('companyId');
        $data['branch_info'] = $this->m_common->get_row_array('department', array('d_id' => $branch_id), '*');
        $where = '';
...
//            }
        ...
        $object->getActiveSheet()->getStyle('I3')->getFont()->setSize('13');
        $object->getActiveSheet()->getStyle('I3')->getFont()->setBold(true);
        $object->getActiveSheet()->setCellValueByColumnAndRow(8, 3, 'Hook Delivery Challan');
        $style = array(
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
            )
        ...
        $table_columns = array("SL","Vessel Name", "C. Date.","C. Time", "C. No.","M. C. No.", "D.O.","C.Name","Truck No", "D.Name","P.Name", "M.Unit", "Quantity", "Unit Price", "Value","Bill Status");
        ...
        $object_writer = PHPExcel_IOFactory::createWriter($object, 'Excel5');
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="hook_challan_report.xls"');
        $object_writer->save('php://output');
    }    
...
//            $data['challans'] = $this->m_common->customeQuery($sql);
    ...
    function customerwiseChallan($print = false) {
        ...
        $where = "dc.status='Approved' and dc.delivery_location='Yard' and dc.is_active=1 and dc.unit_id=$branch_id";
        ...
        if (!empty($postData)) {
            ...
            if (!empty($customer_id)) {
                ...
                if (empty($where)) {
                    ...
                    $where .= " and dc.customer_id=$customer_id";
                }
            ...
            if (!empty($f_date) & !empty($to_date)) {
                ...
                $sql = "select dcd.*,sum(dcd.quantity) as total_qty,dc.dc_no,dc.manual_dc_no,dc.delivery_challan_date,dc.print_date_time,do.delivery_no,do.delivery_order_date,c.c_name,p.item_name,dc.driver_name,dc.truck_no,tmu.meas_unit 
                        from rm_delivery_challan_details dcd
                        left join rm_delivery_challans dc on dcd.dc_id=dc.dc_id 
                        left join rm_delivery_orders_details dod on dcd.do_details_id=dod.do_details_id 
                        left join rm_delivery_orders do on dod.do_id=do.do_id 
                        ...
                        where $where and dc.delivery_challan_date>='" . $from_date . "' and dc.delivery_challan_date<='" . $too_date . "' GROUP BY dc.customer_id,dc.delivery_challan_date,p.item_name order by dc.delivery_challan_date desc";
                ...
                $sql = "select dcd.*,sum(dcd.quantity) as total_qty,dc.dc_no,dc.manual_dc_no,dc.delivery_challan_date,dc.print_date_time,do.delivery_no,do.delivery_order_date,c.c_name,p.item_name,dc.driver_name,dc.truck_no,tmu.meas_unit 
                        ...
                        where $where and dc.delivery_challan_date>='" . $from_date . "' GROUP BY dc.customer_id,dc.delivery_challan_date,p.item_name order by dc.delivery_challan_date desc";
                ...
                $sql = "select dcd.*,sum(dcd.quantity) as total_qty,dc.dc_no,dc.manual_dc_no,dc.delivery_challan_date,dc.print_date_time,do.delivery_no,do.delivery_order_date,c.c_name,p.item_name,dc.driver_name,dc.truck_no,tmu.meas_unit 
                        ...
                        where $where and dc.delivery_challan_date<='" . $too_date . "' GROUP BY dc.customer_id,dc.delivery_challan_date,p.item_name order by dc.delivery_challan_date desc";
                ...
                $sql = "select dcd.*,sum(dcd.quantity) as total_qty,dc.dc_no,dc.manual_dc_no,dc.delivery_challan_date,dc.print_date_time,do.delivery_no,do.delivery_order_date,c.c_name,p.item_name,dc.driver_name,dc.truck_no,tmu.meas_unit 
                        ...
                        where $where GROUP BY dc.customer_id,dc.delivery_challan_date,p.item_name order by dc.delivery_challan_date desc";
            }
            $data['challans'] = $this->m_common->customeQuery($sql);
            $do_sql="select * from rm_delivery_orders where do_location='Yard' and is_active=1 and branch_id=$branch_id";
            $data['all_orders'] = $this->m_common->customeQuery($do_sql);
       ...
       // $data['product_categories']=$this->m_common->get_row_array('tbl_product_categories', array('is_active' => 1), '*');
        ...
        if($print == false){
            $this->load->view('raw_materials/sales_report/v_customer_delivery_challan', $data);
        }else{
            $html = $this->load->view('raw_materials/sales_report/print_customer_delivery_challan', $data, true);
            echo $html;
            exit;
        }
    }
    ...
    function customerwiseChallanExcel($print = false) {
       ...
       // $table_columns = array("SL", "C. Date.", "C.Name", "Project","P.Type", "P.Name", "M. Unit", "Quantity", "CUM Qty","Unit Price","Value");
        $table_columns = array("SL", "C. Date.", "C.Name","P.Name", "M. Unit", "Quantity","Unit Price","Value");
        $column = 5;
        ...
        if (!empty($data['challans'])) {
            ...
            foreach ($data['challans'] as $key => $challan) {
                ...
                $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, $challan['item_name']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, $challan['meas_unit']);
                $object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, round($challan['total_qty'], 2));

                

                
                $object->getActiveSheet()->setCellValueByColumnAndRow(11, $excel_row, round($challan['unit_price'], 2));
                $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row, round(($challan['unit_price']*$challan['total_qty']), 2));
                $excel_row++;
            }
        }
        ...
        $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row,round($total_value, 2));

        $object->getActiveSheet()->getStyle("K$excel_row:M$excel_row")->getFont()->setSize('12');
        $object->getActiveSheet()->getStyle("K$excel_row:M$excel_row")->getFont()->setBold(true);
