...
class Masters extends Site_Controller {
    ...
    function index() {
        ...
        $this->sub_inner_menu = 'master';
        $this->titlebackend("Masters");
        $data['drivers']=$this->m_common->get_row_array('tbl_driver',array('is_active'=>1,'transport_type'=>"Ship"),'*');
        $this->load->view('raw_materials/master/v_driver',$data);
    }
    ...
    function add_driver() {
        ...
        $this->sub_menu = 'rm_challan';
        $this->sub_inner_menu = 'master';
        $this->titlebackend("Add Master");   
        $this->load->view('raw_materials/master/v_add_driver');
    }
    function add_driver_action() {
         ...
         if(!empty($postData)){
            ...
            if(!empty($postData['blood_group'])){
             ...
             $insertData['transport_type']="Ship";
             ...
             if(!empty($result)){
                  ...
                  redirect_with_msg('raw_materials/masters', 'Successfully Inserted');
             }
         }else{
              redirect_with_msg('raw_materials/masters/add_driver', 'Please fill the form and submit');
         }
    ...
    function edit_driver($id) {
        ...
        $this->sub_menu = 'rm_challan';
        $this->sub_inner_menu = 'master';
        $this->titlebackend("Edit Master");
        $data['driver_info']=$this->m_common->get_row_array('tbl_driver',array('driver_id'=>$id),'*');           
        $this->load->view('raw_materials/master/v_edit_driver',$data);
    }
    ...
    function edit_driver_action($id) {
         ...
         if(!empty($postData)){
            ...
            if(!empty($postData['blood_group'])){
             ...
             if($result>=0){              
                  redirect_with_msg('raw_materials/masters', 'Successfully Updated');
             }
         }else{
              redirect_with_msg('raw_materials/masters', 'Please fill the form and submit');
         }
    ...
    function details_driver($id,$print=false) {
        ...
        $this->sub_menu = 'rm_challan';
        $this->sub_inner_menu = 'master';
        $this->titlebackend("Master Details");
        $data['driver_info']=$this->m_common->get_row_array('tbl_driver',array('driver_id'=>$id),'*');      
        $this->load->view('raw_materials/master/v_details_driver',$data);
    ...
    function delete_driver($id) {
        ...
        if(!empty($id)) {
            ...
            if(!empty($id)){
                redirect_with_msg('raw_materials/masters/index', 'Successfully Deleted');
            }else{
                redirect_with_msg('raw_materials/masters/index', 'Data not deleted for an unexpected error');
            }
        }else{
            redirect_with_msg('raw_materials/masters/index', 'Please click on delete button');
        }
    }
