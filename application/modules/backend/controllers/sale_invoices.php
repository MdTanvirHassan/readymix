...
class Sale_invoices extends Site_Controller {
    ...
    function index_($cust_id=false){
       ...
       // $sql = "select v.*,do.delivery_no,c.c_name,c.c_short_name,c.deposit,tpc.category_name from tbl_sales_invoices v left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on o.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id where v.is_active=1 and v.unit_id=" . $branch_id . ' order by v.sale_invoice_date desc';//29-06-2020
       // $sql = "select v.*,do.delivery_no,c.c_name,c.c_short_name,c.deposit,tpc.category_name,tp.project_name as tp_project_name from tbl_sales_invoices v left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on v.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id left join tbl_project tp on v.project_id=tp.project_id where v.is_active=1 and v.unit_id=" . $branch_id . ' order by v.sale_invoice_date desc'; 
      //  $sql = "select v.*,do.delivery_no,c.c_name,c.c_short_name,c.deposit,c.id as customer_id,tpc.category_name,tp.project_name as tp_project_name from tbl_sales_invoices v left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on v.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id left join tbl_project tp on v.project_id=tp.project_id where v.status!='Canceled' and v.is_active=1 and v.unit_id=" . $branch_id . ' order by v.sale_invoice_date desc'; //27-09-2020
        
        //  $sql = "select v.*,o.order_no,do.delivery_no,c.c_name,c.c_short_name,c.deposit,c.id as customer_id,tpc.category_name,tp.project_name as tp_project_name,sum(tsid.quantity) as total_qty,tsid.unit_price,tsid.mu_name from tbl_sales_invoices v left join tbl_sales_invoice_details tsid on v.inv_id=tsid.inv_id left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on v.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id left join tbl_project tp on v.project_id=tp.project_id where v.status!='Canceled' and v.is_active=1 and v.unit_id=" . $branch_id . " and v.customer_id=" .$cust_id.' group by v.inv_id order by v.sale_invoice_date desc'; 
           

            $sql = "select v.*,o.order_no,do.delivery_no,c.c_name,
            c.c_short_name,c.deposit,c.id as customer_id,tpc.category_name,
            tp.project_name as tp_project_name,sum(tsid.quantity) as total_qty,
            tsid.unit_price,tsid.mu_name 
            from tbl_sales_invoices v 
            left join tbl_sales_invoice_details tsid on v.inv_id=tsid.inv_id 
            left join tbl_delivery_orders do on v.do_id=do.do_id 
            left join tbl_sales_orders o on do.o_id=o.o_id 
            left join tbl_sales_quotation q on o.q_id=q.q_id 
            left join tbl_customers c on v.customer_id=c.id 
            left join tbl_product_categories tpc on v.category_id=tpc.category_id 
            left join tbl_project tp on v.project_id=tp.project_id 
            where v.status!='Canceled' and v.status!='Received'  and v.is_active=1  and v.customer_id=" .$cust_id." and v.unit_id=" . $branch_id . ' group by v.inv_id order by v.sale_invoice_date desc';
            $data['invoices'] = $this->m_common->customeQuery($sql);

       }else{
           $sql = "select v.*,o.order_no,do.delivery_no,c.c_name,c.c_short_name,c.deposit,c.id as customer_id,tpc.category_name,tp.project_name as tp_project_name,sum(tsid.quantity) as total_qty,tsid.unit_price,tsid.mu_name from tbl_sales_invoices v left join tbl_sales_invoice_details tsid on v.inv_id=tsid.inv_id left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on v.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id left join tbl_project tp on v.project_id=tp.project_id where v.status!='Canceled' and v.status!='Received' and v.is_active=1 and v.unit_id=" . $branch_id ." group by v.inv_id order by v.sale_invoice_date desc limit 20";        
           $data['invoices'] = $this->m_common->customeQuery($sql);
       }
...
//        
    ...
    function index($cust_id=false){
        ...
        if(!empty($postData)){
            ...
            if(!empty($customer_id)){
                $where.=' and v.customer_id='.$customer_id;
            }
            ...
            if(!empty($postData['from_date']) && !empty($postData['to_date'])){
                ...
                $where.=" and (sale_invoice_date>='".$from_date."' and sale_invoice_date<='".$to_date."')";
            ...
            if( $user_category==2){ 
                ...
                left join tbl_product_categories tpc on v.category_id=tpc.category_id 
                left join tbl_project tp on v.project_id=tp.project_id 
                where (v.sale_invoice_date>='$prev_date' and v.sale_invoice_date<='$currentDate') and v.status!='Canceled' and v.status!='Received'  
                and v.unit_id=" . $branch_id . ' group by v.inv_id order by v.sale_invoice_date desc';
                $data['invoices'] = $this->m_common->customeQuery($sql);
            }else{
                $sql = "select v.*,o.order_no,do.delivery_no,c.c_name,
                c.c_short_name,c.deposit,c.id as customer_id,tpc.category_name,
                ...
                left join tbl_product_categories tpc on v.category_id=tpc.category_id 
                left join tbl_project tp on v.project_id=tp.project_id 
                where $where and v.status!='Canceled' and v.status!='Received'  
                and v.unit_id=" . $branch_id . ' group by v.inv_id order by v.sale_invoice_date desc';
                $data['invoices'] = $this->m_common->customeQuery($sql);
            }
           ...
           if( $user_category==2){ 
                $sql = "select v.*,o.order_no,do.delivery_no,c.c_name,c.c_short_name,
                c.deposit,c.id as customer_id,tpc.category_name,
                tp.project_name as tp_project_name,sum(tsid.quantity) as total_qty,
                tsid.unit_price,tsid.mu_name from tbl_sales_invoices v 
                left join tbl_sales_invoice_details tsid on v.inv_id=tsid.inv_id 
                left join tbl_delivery_orders do on v.do_id=do.do_id 
                left join tbl_sales_orders o on do.o_id=o.o_id 
                left join tbl_sales_quotation q on o.q_id=q.q_id 
                ...
                left join tbl_product_categories tpc on v.category_id=tpc.category_id 
                left join tbl_project tp on v.project_id=tp.project_id 
                where (v.sale_invoice_date>='$prev_date' and v.sale_invoice_date<='$currentDate') and v.status!='Canceled' and v.status!='Received' and v.is_active=1 and v.unit_id=" . $branch_id ." group by v.inv_id order by v.sale_invoice_date desc limit 20"; 
           }else{
                $sql = "select v.*,o.order_no,do.delivery_no,c.c_name,c.c_short_name,c.deposit,c.id as customer_id,tpc.category_name,tp.project_name as tp_project_name,sum(tsid.quantity) as total_qty,tsid.unit_price,tsid.mu_name from tbl_sales_invoices v left join tbl_sales_invoice_details tsid on v.inv_id=tsid.inv_id left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on v.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id left join tbl_project tp on v.project_id=tp.project_id where v.status!='Canceled' and v.status!='Received' and v.is_active=1 and v.unit_id=" . $branch_id ." group by v.inv_id order by v.sale_invoice_date desc limit 20";
           }       
           $data['invoices'] = $this->m_common->customeQuery($sql);
    ...
    function paidInvoice($cust_id=false){
       ...
       if($user_category==2){ 
            if(!empty($cust_id)){                    
                ...
                left join tbl_product_categories tpc on v.category_id=tpc.category_id 
                left join tbl_project tp on v.project_id=tp.project_id 
                where (v.sale_invoice_date>='$prev_date' and v.sale_invoice_date<='$currentDate') and v.status='Received'  and v.is_active=1 and v.unit_id=" . $branch_id . " and v.customer_id=" .$cust_id.' group by v.inv_id order by v.sale_invoice_date desc';
                $data['invoices'] = $this->m_common->customeQuery($sql);
            }else{
                $sql = "select v.*,o.order_no,do.delivery_no,c.c_name,c.c_short_name,c.deposit,c.id as customer_id,
                tpc.category_name,tp.project_name as tp_project_name,
                sum(tsid.quantity) as total_qty,tsid.unit_price,tsid.mu_name 
                from tbl_sales_invoices v 
                left join tbl_sales_invoice_details tsid on v.inv_id=tsid.inv_id 
                left join tbl_delivery_orders do on v.do_id=do.do_id 
                left join tbl_sales_orders o on do.o_id=o.o_id 
                ...
                left join tbl_product_categories tpc on v.category_id=tpc.category_id 
                left join tbl_project tp on v.project_id=tp.project_id 
                where (v.sale_invoice_date>='$prev_date' and v.sale_invoice_date<='$currentDate') and v.status='Received' and v.is_active=1 and v.unit_id=" . $branch_id . ' group by v.inv_id order by v.sale_invoice_date desc';        
                $data['invoices'] = $this->m_common->customeQuery($sql);
       ...
       $data['customer_id']=$cust_id;
        ...
        $data['customers']=$this->m_common->get_row_array('tbl_customers',array('is_active'=>1),'*','','','c_name');         
        $this->load->view('sale_invoices/v_paid_sales_invoice', $data);
    }
    ...
    function get_customer_balance(){
        ...
        $tb_sql="select sum(received_amount) as total_received from tbl_sales_invoices v where v.status!='Canceled' and v.is_active=1 and v.customer_id=".$id;
    ...
    function cancelInvoice($id,$cust_id=false){
         ...
         $result=$this->m_common->update_row('tbl_sales_invoices',array('inv_id'=>$id),array('status'=>'Canceled','updated_by'=>$user_id,'updated_date'=>date('Y-m-d')));
         if(!empty($result)){
             if(!empty($cust_id)){
                redirect_with_msg('sale_invoices/index/'.$cust_id, 'Successfully Canceled');
             }else{
                redirect_with_msg('sale_invoices/index', 'Successfully Canceled'); 
             }
         }else{
             if(!empty($cust_id)){
                redirect_with_msg('sale_invoices/index/'.$cust_id, 'Not Cancel');
             }else{
                redirect_with_msg('sale_invoices/index', 'Not Cancel'); 
             }
         }
     }
...
//        }
    ...
    function project_info() {
       ...
       // $data['order_info'] = $this->m_common->get_row_array('tbl_sales_orders', array('customer_id' => $customer_id), '*');
        ...
        foreach ($data['data'] as $key => $value){
            ...
            $challan_info = array();
            $challan_sql = "select dcd.*,tso.project_id from tbl_delivery_challan_details dcd left join tbl_delivery_challans dc on dcd.dc_id=dc.dc_id left join tbl_delivery_orders do on dc.do_id=do.do_id left join tbl_sales_orders tso on do.o_id=tso.o_id where dcd.bill_status='Pending' and tso.project_id=".$value['project_id'];
            $challan_info = $this->m_common->customeQuery($challan_sql);
            if (empty($challan_info)){
               // unset($data['data'][$key]);                
            }else{                
                $data['project'][$i]=$data['data'][$key];
                $i++;
            } 
        }
    ...
    function customer_project_info() {
        $this->setOutputMode(NORMAL);
        $customer_id = $this->input->post('customer_id');
        $data['project'] = $this->m_common->get_row_array('tbl_project', array('customer_id' => $customer_id), '*');
    ...
    function salesOrderInfoForPump(){
        $this->setOutputMode(NORMAL);
        $branch_id = $this->session->userdata('companyId');
        $project_id = $this->input->post('project_id');
        $data['order_info'] = $this->m_common->get_row_array('tbl_sales_orders',array('project_id'=>$project_id,'unit_id'=>$branch_id,'is_active'=>1),'*');
    ...
    function deliveryOrderInfoSaleOrderWise() {
        ...
        foreach ($data['order_info'] as $key => $value){
            $challan_info = array();
            $challan_sql = "select dcd.* from tbl_delivery_challan_details dcd left join tbl_delivery_challans dc on dcd.dc_id=dc.dc_id left join tbl_delivery_orders do on dc.do_id=do.do_id left join tbl_sales_orders tso on do.o_id=tso.o_id where dcd.bill_status='Pending' and do.do_id=" . $value['do_id'];
            $challan_info = $this->m_common->customeQuery($challan_sql);
            if (empty($challan_info)){
                unset($data['order_info'][$key]);
            }else{
    ...
    function add_sale_invoice_action() {
        ...
        if (!empty($postData)) {
            ...
            if(!empty($postData['inv_no'])){
                ...
                $pre_invoice_info=$this->m_common->get_row_array('tbl_sales_invoices',array('inv_no'=>$postData['inv_no']),'*');
                if(!empty($pre_invoice_info)){
                    redirect_with_msg('sale_invoices', 'This invoice already exists');
                }
            }
...
//                                $insertData1['received_status'] = 'Partial Received';
//                            } else {
...
//                        $adjust_amount = $adjust_amount - $insertData1['received_amount'];
    ...
    function edit_sale_invoice($id) {
        ...
        $data['sale_invoice_details_info'] = $this->m_common->customeQuery($sql);
         ...
         $sql = "select dcd.*,sd.quantity as so_qty,p.product_name,p.measurement_unit,dc.dc_no,dc.manual_dc_no,dc.delivery_challan_date 
...
//                    $adjust_amount = $adjust_amount - $insertData1['received_amount'];
    ...
    function update_customer_project_ifo($id){
        $branch_id = $this->session->userdata('companyId');
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sale_invoice';
        $this->titlebackend("Invoice Information");        
        $sale_invoice_info= $this->m_common->get_row_array('tbl_sales_invoices', array('inv_id' => $id), '*');
        $do_info= $this->m_common->get_row_array('tbl_delivery_orders', array('do_id' =>$sale_invoice_info[0]['do_id']), '*');
        $order_info=$this->m_common->get_row_array('tbl_sales_orders', array('o_id' =>$do_info[0]['o_id']), '*');
        $this->m_common->update_row('tbl_sales_invoices', array('inv_id' => $id),array('customer_id'=>$order_info[0]['customer_id'],'project_id'=>$order_info[0]['project_id']));
        redirect_with_msg('sale_invoices/update', 'Successfully Updated');
    }
    ...
    function update(){
        ...
        // $sql="select v.*,do.delivery_no,c.c_name,c.c_short_name from tbl_sales_invoices v left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on q.customer_id=c.id where v.is_active=1";
       // $sql = "select v.*,do.delivery_no,c.c_name,c.c_short_name,c.deposit,tpc.category_name from tbl_sales_invoices v left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on o.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id where v.is_active=1 and v.unit_id=" . $branch_id . ' order by v.sale_invoice_date desc';//29-06-2020
        $sql = "select v.*,do.delivery_no,c.c_name,c.c_short_name,c.deposit,tpc.category_name,tp.project_name as tp_project_name from tbl_sales_invoices v left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on v.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id left join tbl_project tp on v.project_id=tp.project_id where (v.customer_id='' or v.project_id='') order by v.sale_invoice_date desc"; 
        
        $data['invoices'] = $this->m_common->customeQuery($sql);
        $this->load->view('sale_invoices/v_sale_invoice_for_update', $data);
    }
    ...
    function details_vat_sale_invoice($id, $print = false) {
        $branch_id = $this->session->userdata('companyId');
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sale_invoice';
        ...
        if ($print == false) {
            ...
            $html = $this->load->view('sale_invoices/print_vat_sale_invoice', $data, true);
            echo $html;
            exit;
        }
    }
    ...
    function add_direct_sale_invoice() {
        $branch_id=$this->session->userdata('companyId');
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sale_invoice';
        ...
        $this->load->view('sale_invoices/v_add_direct_sale_invoice', $data);
     
    }
        
    function add_direct_sale_invoice_action(){
        $branch_id = $this->session->userdata('companyId');
        $postData = $this->input->post();
        if (!empty($postData)) {
            $insertData = array();
    ...
    function edit_direct_sale_invoice($id) {
        $branch_id = $this->session->userdata('companyId');
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sale_invoice';
        ...
        $data['orders'] = $this->m_common->customeQuery($sql);
        $data['sale_invoice_info'] = $this->m_common->get_row_array('tbl_sales_invoices', array('inv_id' => $id), '*');
        $data['projects'] = $this->m_common->get_row_array('tbl_project', array('customer_id' => $data['sale_invoice_info'][0]['customer_id']), '*', '', '', 'project_id', 'DESC');
        //  $data['sale_invoice_details_info']=$this->m_common->get_row_array('tbl_sales_sale_invoice_details',array('q_id'=>$id),'*');
        //$sql = "select d.*,dcd.dc_details_id,p.product_name,dc.dc_no,dc.delivery_challan_date from tbl_sales_invoice_details d left join tbl_sales_products p on d.s_item_id=p.product_id left join tbl_delivery_challans dc on d.dc_id=dc.dc_id join tbl_delivery_challan_details dcd on dcd.dc_id=dc.dc_id where d.is_active=1 and inv_id=" . $id;
       // $data['sale_invoice_details_info'] = $this->m_common->customeQuery($sql);
        
        $sql = "select d.*,sp.product_name,sp.measurement_unit from tbl_sales_invoice_details d left join tbl_sales_products sp on d.s_item_id=sp.product_id where d.is_active=1 and d.inv_id=".$id;
        $data['sale_invoice_details_info'] = $this->m_common->customeQuery($sql);
        
        $data['product_categories']=$this->m_common->get_row_array('tbl_product_categories', array('is_active' => 1), '*'); 
        $this->load->view('sale_invoices/v_edit_direct_sale_invoice', $data);
    }

    function edit_direct_sale_invoice_action($inv_id) {
    ...
    function details_direct_sale_invoice($id, $print = false) {
        $branch_id = $this->session->userdata('companyId');
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sale_invoice';
    ...
    function delete_direct_sale_invoice($id) {
        if (!empty($id)) {
    ...
    function add_pump_bill() {
        ...
        $this->load->view('sale_invoices/v_add_pump_bill', $data);
     
    }
        
    function add_pump_bill_action(){
        $branch_id = $this->session->userdata('companyId');
        $postData = $this->input->post();
        if (!empty($postData)) {
            $insertData = array();
            ...
            //    $this->m_common->insert_row('tbl_invoice_code', array('inv_code' =>$postData['inv_code'], 'customer_id' =>$postData['customer_id'], 'unit_id' =>$branch_id));
                ...
                foreach ($postData['service_name'] as $key => $each) {
                    
                        $insertData1 = array();
                        $insertData1['inv_id'] = $result;
                        $insertData1['service_name'] = $each;
                        $insertData1['is_active'] = 1;
                        $insertData1['received_status'] = 'Pending';
    ...
    function edit_pump_bill($id) {
        ...
        $sql = "select d.* from tbl_sales_invoice_details d where d.is_active=1 and d.inv_id=".$id;
        $data['sale_invoice_details_info'] = $this->m_common->customeQuery($sql);
        
        $data['product_categories']=$this->m_common->get_row_array('tbl_product_categories', array('is_active' => 1), '*'); 
        $this->load->view('sale_invoices/v_edit_pump_bill', $data);
    }

    function edit_pump_bill_action($inv_id) {
        ...
        if (!empty($postData)) {
            ...
            if ($result >= 0){                
                ...
                foreach ($postData['service_name'] as $key => $each) {
                    ...
                    $insertData1 = array();
                    $insertData1['inv_id'] = $inv_id;
                    $insertData1['service_name'] = $postData['service_name'][$key];
                    $insertData1['is_active'] = 1;

                    if (empty($each)) {
                        continue;
                    }
                    if (empty($postData['service_name'][$key])) {
                        continue;
                    }
    ...
    function details_pump_bill($id, $print = false) {
        $branch_id = $this->session->userdata('companyId');
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'sale_invoice';
        ...
        $data['orders'] = $this->m_common->customeQuery($sql);
         ...
         $inv_sql = "select si.*,so.order_no,so.sale_order_date,c.c_name,tp.project_name from tbl_sales_invoices si left join tbl_delivery_orders do on si.do_id=do.do_id left join tbl_sales_orders so on do.o_id=so.o_id left join tbl_customers c on si.customer_id=c.id left join tbl_project tp on si.project_id=tp.project_id where si.inv_id=" . $id;
        //$data['sale_invoice_info']=$this->m_common->get_row_array('tbl_sales_invoices',array('inv_id'=>$id),'*');
        $data['sale_invoice_info'] = $this->m_common->customeQuery($inv_sql);
       ...
       // $data['sale_invoice_info'] = $this->m_common->get_row_array('tbl_sales_invoices', array('inv_id' => $id), '*');
        ...
        if ($print == false) {
            $this->load->view('sale_invoices/v_details_pump_bill', $data);
        } else {
            $html = $this->load->view('sale_invoices/print_pump_bill', $data, true);
            echo $html;
            exit;
        }
    }
...
and dcd.bill_status='Pending' and dcd.is_active=1 " . $where.' order by dc.dc_id';
    ...
    function confirm_payment(){
        try {
            ...
            if($inv_type=="Direct"){
              ...
              //  $this->m_common->update_row('tbl_sales_orders', array('o_id' => $order_info[0]['o_id']), array('receive_amount' => $net_receive_amount));
            }else{
                $this->m_common->update_row('tbl_sales_invoices', array('inv_id' => $id), array('received_amount' =>$receive_amount,'status' => $status));
                $order_info = $this->m_common->customeQuery('select s.* from tbl_sales_orders s join tbl_delivery_orders d on d.o_id=s.o_id where d.do_id=' . $invoice_info[0]['do_id']);
    ...
    function confirm_direct_invoice_payment(){
        try{
            $this->db->trans_start();
            $branch_id = $this->session->userdata('companyId');
            $this->setOutputMode(NORMAL);
            ...
            print_r($error);
            exit;
            redirect_with_msg('sale_invoices', 'Something went wrong');
        }
    }
