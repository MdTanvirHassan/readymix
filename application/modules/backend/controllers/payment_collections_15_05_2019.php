...
class Payment_collections extends Site_Controller {
    ...
    function index() {
       ...
       // $sql="select pc.*,so.order_no,c.c_name,c_short_name from tbl_payment_collections pc left join tbl_sales_orders so on pc.o_id=so.o_id left join tbl_sales_quotation sq on so.q_id=sq.q_id left join tbl_customers c on sq.customer_id=c.id where pc.is_active=1";
        $sql="select pc.*,so.order_no,c.c_name,c_short_name from tbl_payment_collections pc left join tbl_sales_orders so on pc.o_id=so.o_id left join tbl_sales_quotation sq on so.q_id=sq.q_id left join tbl_customers c on sq.customer_id=c.id where pc.is_active=1 and pc.unit_id=".$branch_id;
    ...
    function edit_collection_action($q_id) {
         ...
         if(!empty($postData)){
             ...
             if($result>=0){ 
              ...
              redirect_with_msg('payment_collections/edit_collection/'.$id, 'Please fill the form and submit');
         }
