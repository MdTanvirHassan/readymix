    ...
    <div class="">
        <div class="page-title">
            <div class="title_center">
                <h3 style="float:left;">Return Receive Details</h3>
                <a target="_blank" style="float:right;margin-top:10px;" href="<?php echo site_url('general_store/details_issue_return/'.$issue_return[0]['ir_id'].'/true'); ?>" class="btn btn-sm btn-warning">PRINT</a>
            </div>
        </div>
        <div class="row">
...
<table class="table table-bordered">
                            ...
                            <tbody>
                                <tr>
                                    <td><?php if(!empty($issue_return[0]['ir_no'])) echo $issue_return[0]['ir_no']; ?></td>
                                    <td>
                                   <?php foreach($issue_numbers as $issue_number){ ?>
                                        <?php if(!empty($issue_return[0]['issue_no']) && $issue_return[0]['issue_no']== $issue_number['issue_id'] )  echo $issue_number['issue_no']; ?>
                                    <?php } ?> 
                                    </td>
                                    <td><?php if(!empty($issue_return[0]['ir_date'])) echo date('d-m-Y',strtotime($issue_return[0]['ir_date'])); ?></td>
                                    <td><?php if(!empty($issue_return[0]['issue_date'])) echo date('d-m-Y',strtotime($issue_return[0]['issue_date'])); ?></td>
                                    <td><?php if(!empty($issue_return[0]['remarks'])) echo $issue_return[0]['remarks']; ?></td>


                                    <td>
                                        <a href="<?php echo site_url('general_store/add_issue_return'); ?>" class="btn btn-sm btn-primary">ADD RETURN RECEIVE</a>
             <?php if($issue_return[0]['ir_status']=="applied" || $issue_return[0]['ir_status']=="not received" ){ ?>
                <a href="<?php echo site_url('general_store/receive_issue_return/'.$issue_return[0]['ir_id']); ?>"><button class="btn btn-sm btn-success">RECEIVE</button></a>
            <?php }else{ ?>
                 <a href="<?php echo site_url('general_store/reject_issue_return/'.$issue_return[0]['ir_id']); ?>"><button class="btn btn-sm btn-warning">REJECT</button></a>
            <?php } ?>  
                                    </td>
                                </tr>
                            </tbody>
                ...
                <?php $i = 0;
                   ...
                   <tr class="row" id="row_<?php echo $i; ?>">
                       ...
                       <td> 
                          ...
                          <?php if(!empty($issue_return_detail['item_code'])) echo $issue_return_detail['item_code'];?>
                       </td>
                    <td><?php if (!empty($issue_return_detail['item_description'])) echo $issue_return_detail['item_description']; ?></td>
                   <td><?php if (!empty($issue_return_detail['measurement_unit'])) echo $issue_return_detail['measurement_unit']; ?></td>
                   <td><?php if (!empty($issue_return_detail['issued_qty'])) echo $issue_return_detail['issued_qty']; ?></td>
                   <td><?php if (!empty($issue_return_detail['return_qty'])) echo $issue_return_detail['return_qty']; ?></td>
                   <td><?php if (!empty($issue_return_detail['unit_price'])) echo $issue_return_detail['unit_price']; ?></td>
                 
                   <td><?php if (!empty($issue_return_detail['return_value'])) echo $issue_return_detail['return_value']; ?></td>
              
                   <td><?php if (!empty($issue_return_detail['remark'])) echo $issue_return_detail['remark']; ?></td>
