    ...
    <div class="">
        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                    <div class="x_content">
                        <form class="form-horizontal"  action="<?php echo site_url('raw_materials/payment_collections/add_collection_action'); ?>" method="post" onsubmit="javascript: return validation()">
                            <div class="row"> 
                                <div class="col-md-8">
...
function changeCustomer(){
    ...
    if(customer==''){
      ...
      $.ajax({
                url: '<?php echo site_url('raw_materials/payment_collections/get_customer_sales_order'); ?>',
                data: {'c_id': customer},
                method: 'POST',
                dataType: 'json',
...
//                end add by jubayer for collection history
    ...
    $('#radioBtn a').on('click', function () {
        ...
        if (method == "Cash") {
            ...
            $.ajax({
                url: '<?php echo site_url('raw_materials/payment_collections/bankInfo'); ?>',
                data: {},
                method: 'POST',
                dataType: 'json',
                success: function (msg) {
                    ...
                    var option1 = '<option class="form-control" value="">Select Bank</option>';
                    if (msg.self_banks != '') {
                        $(msg.self_banks).each(function (i, v) {
                           
                            option1 += '<option class="form-control" value="'+v.id+ '">' + v.b_name + '(' + v.branch_name + ')' + '</option>';
                        });
                    }
                    
                    $('#bank_id').html(option1);
...
//                 pdc_no +='</div></div>';  
            ...
            $.ajax({
                ...
                success: function (msg) {
                    ...
                    var option1 = '<option class="form-control" value="">Select Bank</option>';
                    if (msg.customer_banks != '') {
                        $(msg.customer_banks).each(function (i, v) {
