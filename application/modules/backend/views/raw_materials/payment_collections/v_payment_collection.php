...
<?php
    ...
    $this->role = checkUserPermission(18,129, $userData);
 ?>
    ...
    <div class="">
        ...
        <div class="row">
                        <div id="remover" class="col-md-6 col-md-offset-3">
                            <form id="item-form" action="<?php site_url('backend/raw_materials/payment_collections'); ?>" method="post">
                            <div class="row">
...
<!--                                    <input style="padding: 6px 40px;" type="submit" class="btn btn-primary" value="SEARCH"/>
    ...
    <?php  if (in_array(2, $this->role)) { ?>
        <a href="<?php echo site_url('raw_materials/payment_collections/add_collection'); ?>" class="btn btn-sm btn-primary">ADD COLLECTION</a>
    <?php } ?>     
                       ...
                       <?php } ?>         
                        ...
                        <td>
                            <?php  if (in_array(4, $this->role)) { ?>
                                <a href="<?php echo site_url('raw_materials/payment_collections/view_collection/'.$collection['id']); ?>"><button class="btn btn-sm btn-primary">View</button></a>
                            <?php } ?>    
                            <?php if(($collection['collection_method']=="Cash" || $collection['collection_method']=="O.Cash" || $collection['collection_method']=="Adjustment" || $collection['collection_method']=="Vat" || $collection['collection_method']=="Ait") && $collection['payment_status']!="Received" ){ ?>
                                <?php  if (in_array(2, $this->role)) { ?>    
                                    <a href="<?php echo site_url('raw_materials/payment_collections/receive_collection/'.$collection['id']); ?>"><button class="btn btn-sm btn-primary">Received</button></a>
                                <?php } ?>    
                            <?php }else{ ?>
                                <?php  if (in_array(2, $this->role)) { ?>        
                            ...
                            <?php if($collection['payment_status']!="Received"){ ?>
                               <?php  if (in_array(3, $this->role)) { ?>     
                                    <a href="<?php echo site_url('raw_materials/payment_collections/edit_collection/'.$collection['id']); ?>"><button class="btn btn-sm btn-info">Edit</button></a>
                               <?php } ?>    
                            <?php }else{ ?>
                                <?php  if (in_array(3, $this->role)) { ?>    
                            ...
                            <?php if($collection['payment_status']!="Received"){ ?>
                                <?php  if (in_array(5, $this->role)) { ?>    
                                    <button onclick="delete_row('<?php echo site_url('raw_materials/payment_collections/delete_collection/'.$collection['id']); ?>')" class="btn btn-sm btn-danger">Delete</button>
                                <?php } ?>    
                            <?php } ?> 
                                    
                                     <?php if($collection['collection_method']=="Cash" && $collection['payment_status']!="Received" ){ ?>
                                <?php  if (in_array(2, $this->role)) { ?>    
                                    <a href="<?php echo site_url('raw_materials/payment_collections/collection_voucher/'.$collection['id']); ?>"><button class="btn btn-sm btn-primary">Voucher </button></a>
                                <?php } ?>    
                            <?php } ?>
                        </td>
                    </tr>
     ...
     function returnCollectionAction(){
       ...
       // var return_date=$('#return_date').val();
        ...
        if(return_reason==''){
         ...
         $.ajax({
                url: '<?php echo site_url('raw_materials/payment_collections/returnPayment'); ?>',
