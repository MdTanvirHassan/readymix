                        ...
                        <form class="form-horizontal"  action="<?php echo site_url('payment_collections/add_collection_action'); ?>" method="post" onsubmit="javascript: return validation()">
                            <div class="row"> 
                                ...
                                <div class="col-sm-8 input-group">
                                    ...
                                    <select  id="c_id" class="form-control e1" name="c_id" onchange="javascript:changeCustomer();">
...
<!--                                        <label for="title" class="col-sm-2 control-label">
                                        ...
                                        <div class="col-sm-4 input-group">
                                            ...
                                            <span id="receive_type_error" style="color:red"></span>
                                        </div>-->
                                        <label for="title" class="col-sm-2 control-label">
...
<div id="sales_order_list">
                                ...
                                <div class="col-md-4" style="border-left: 1px solid #ddd;min-height: 250px;">
                                    ...
                                    <b style="font-size: 18px; color:#286090;">Total : <span id="total"></span> </b> <br>
                                    <b style="font-size: 18px; color:#286090;">Receive : <span id="receive"></span> </b> <br>
                                    <b style="font-size: 18px; color:#286090;">Due  : <span id="due"></span></b> <br><br>
      ...
      $.ajax({
                ...
                success: function (msg) {
                    ...
                    $('#total').html('')
                        ...
                        $('#due').html('')
                        $('#deposit').html('')
                    var str = '<table class="table"><tr class="table-info"><th>Order NO</th><th>Project</th><th>Date</th><th>Amount</th><th>Rcv Amount</th><th>Due</th><th>Action</th></tr>';
                    if(msg.sales_order){
                        var total = 0;
                        var rtotal = 0;
                        $(msg.sales_order).each(function(i,r){
                            total+=Number(r.total_amount);
                            rtotal+=Number(r.receive_amount);
                            str+='<tr>';
                            str+='<td>'+r.order_no+'</td>';
                            str+='<td>'+r.project_name+'</td>';
                            str+='<td>'+r.sale_order_date+'</td>';
                            str+='<td>'+r.total_amount+'</td>';
                            str+='<td>'+r.receive_amount+'</td>';
                            str+='<td id="due_'+r.o_id+'">'+(r.total_amount-r.receive_amount)+'</td>';
                            str+='<td><input class="o_id" id="oid_'+r.o_id+'" onclick="calculalteDue()" type="checkbox" name="o_id[]" value="'+r.o_id+'"></td></tr>';
                        })
                        str+='<tr><td colspan="3">Total</td><td id="finaltotal">'+total+'</td><td id="finalrcv">'+rtotal+'</td><td id="finaldue">'+(total-rtotal)+'</td></tr>';
                        
                        $('#total').html(total)
                        $('#receive').html(rtotal)
                        $('#due').html((total-rtotal))
                    }
                        $('#deposit').html(msg.customer.deposit)
                    $('#sales_order_list').html(str);
