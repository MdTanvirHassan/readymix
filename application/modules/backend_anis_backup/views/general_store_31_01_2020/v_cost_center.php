<?php
    ...
    $this->role = checkUserPermission(1, 6, $userData);
 ?>
<div class="right_col" style="background-color: #f1f1f1;padding-bottom:20px;">
<!--    <h2 style="text-align:center; ">Cost Center List</h2>
    <hr>-->
...
<div class="right_content">
    <?php $this->role = checkUserPermission(1, 6, $userData); if (in_array(2, $this->role)) { ?>
        <a href="<?php echo site_url('general_store/add_cost_center'); ?>" class="btn btn-sm btn-primary">ADD COST CENTER</a>
    <?php } ?>    
    <table id="datatable" class="table table-striped table-bordered table-hover dataTable no-footer">
        <thead>
            <tr>
                <th class="col-lg-2">Cost Center Name</th>
        ...
        <tbody>
            <?php if (count($cost_centers)) {
                foreach ($cost_centers as $cost_center) { ?>
                    <tr>
                        <td>
                            <?php echo $cost_center['c_c_name']; ?>
                        </td>
                 
                       <td>
                             <?php echo $cost_center['c_c_description']; ?>
                        </td>

                        <td>
                          <?php  if (in_array(3, $this->role)) { ?>  
                                <a href="<?php echo site_url('general_store/edit_cost_center/'.$cost_center['c_c_id']); ?>"><button class="btn btn-sm btn-info">Edit</button></a>
                          <?php } ?>      
                          <?php  if (in_array(5, $this->role)) { ?> 
                                <button onclick="delete_row('<?php echo site_url('general_store/delete_cost_center/'.$cost_center['c_c_id']); ?>')" class="btn btn-sm btn-danger">Delete</button>
                          <?php } ?>     
                        </td>
                    </tr>
    <?php }
