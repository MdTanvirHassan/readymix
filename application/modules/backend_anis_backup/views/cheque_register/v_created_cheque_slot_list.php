...
<!--                                <button class="addth-button pull-right " id="create_new_cheque"><span class="fa fa-plus"></span>  Create New Cheque</button>-->
        ...
        <table id="datatable" class="table table-striped table-bordered table-hover dataTable no-footer">
            ...
            <th style="width:10%;text-align: center;"> Start Cheque No</th>
            <th style="width:10%;text-align: center;"> End Cheque No</th>
            <th style="width:10%;text-align: center;"> Total Chq.</th>
            ...
            <tbody>
                <?php foreach ($data as $key => $row) { 
                    $totalchq = 0;
                    $totalchq = $row['cheque_no_end']-$row['cheque_no_start'];
                    ...
                    <tr>
                        ...
                        <td style="width:10%;text-align: center;"><?php if (!empty($row['cheque_no_start'])) echo $row['cheque_no_start']; ?></td>
                        <td style="width:10%;text-align: center;"><?php if (!empty($row['cheque_no_end'])) echo $row['cheque_no_end']; ?></td>
                        <td style="width:10%;text-align: center;"><?php echo $totalchq+1; ?></td>
                        <td style="width:15%;text-align: center;"><?php if (!empty($row['c_name'])) echo $row['c_name']; ?></td>
                        <td style="width:15%;text-align: center;"><?php if (!empty($row['bank_name'])) echo $row['bank_name']; ?></td>
                        <td style="width:15%;text-align: center;"><?php if (!empty($row['bank_branch'])) echo $row['bank_branch']; ?></td>
                        <td style="width:15%;text-align: center;"><?php if (!empty($row['b_account_no'])) echo $row['b_account_no']; ?></td>

                        <td style="text-align: center;width:20%; font-size:12px;" class="noExport">

                            <a onclick="slotDelete('<?php echo $row['slot_id']; ?>')" href='javascript:' title="Delete"><i style="color:red;font-size: 20px;" class="fa fa-trash-o" aria-hidden="true"></i></a>
                            &nbsp;&nbsp;&nbsp;&nbsp;
    <!--                                            <a onclick='edit("<?php echo $row['slot_id']; ?>")' href='javascript:' title="Edit"><i style="font-size: 20px;" class="fa fa-edit" aria-hidden="true"></i></a>-->
    ...
    <div id="myModal" class="modal fade" role="dialog">
        <div class="modal-dialog">
            <form action="<?php echo site_url('setup/create_cheque') ?>" method="post">
                <!-- Modal content-->
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <select name="company" id="company" class="form-control" onchange="changecompany()">
...
<!--                                    input type="text" name="bank_branch" value="<?php echo $bank[0]['bank_branch']; ?>" id="bank_branch" class="form-control" placeholder="Bank Branch"/>-->
                        <select name="bank_branch" id="bank_branch" onchange="changebranch()" class="form-control">
...
<!--                                    <input type="text" name="account_no" value="<?php echo $bank[0]['account_no']; ?>" id="account_no" class="form-control" placeholder="Account No"/>-->
                        ...
                        <input type="text" name="cheque_no_start" class="form-control" placeholder="Cheque No Start From"/>
                        <br>
                        <br>
                        <label>Cheque No End</label>
                        <input type="text" name="cheque_no_end" class="form-control" placeholder="Cheque No End"/>
                    </div>
...
//    }
            ...
            success: function (data) {
                ...
                if (data.msg == 'success') {
                    ...
                    $(data.value).each(function (row, val) {
                        str += '<option value="' + val.bank_id + '">' + val.bank_name + '</option>';
                        $('#bank').html(str);
                    })
    ...
    function changebranch() {
        ...
        $.ajax({
            ...
            data: {b_branch: b_branch, c_id: c_id},
            dataType: "json",
            success: function (data) {
                ...
                if (data.msg == 'success') {
                    ...
                    $(data.value).each(function (row, val) {
                        str += '<option value="' + val.account_no + '">' + val.account_no + '</option>';
                        $('#bank_account').html(str);
                    })
    ...
    function slotDelete(slot_id) {
        if (confirm('Are you suer you want to delete this ?') == true) {
            window.location.href = '<?php echo site_url('cheque_register/created_cheque_slot_delete/') ?>/' + slot_id;
