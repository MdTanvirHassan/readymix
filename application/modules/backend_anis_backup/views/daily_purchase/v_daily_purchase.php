...
<div class="right_col" style="background-color: #f1f1f1;padding-bottom:20px;">
  ...
  <div class="right_content">
     <?php $this->role = checkUserPermission(2, 41, $userData);  if (in_array(2, $this->role)) { ?>
        <a href="<?php echo site_url('daily_purchase/add_purchase_order'); ?>" class="btn btn-sm btn-primary">ADD PURCHASE</a>
     <?php } ?>    
    <table id="datatable" class="table table-striped table-bordered table-hover dataTable no-footer">
        <thead>
            <tr>
                <th class="col-lg-1">Date</th>
                <th class="col-lg-1">Purchase No.</th>
                <th class="col-lg-1">Purchase Type</th>
                <th class="col-lg-1">Purchase</th>
                ...
                foreach ($purchase_orders as $purchase_order) { 
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php } ?>     
                                <a href="<?php echo site_url('daily_purchase/details_purchase_order/'.$purchase_order['o_id']); ?>"><button class="btn btn-sm btn-success">View</button></a>
                                
                                
                            <?php } ?>
                            <?php if($purchase_order['approve_status']=="Pending"){ ?>
                                <?php  if (in_array(3, $this->role)) { ?>
                                    <a href="<?php echo site_url('daily_purchase/edit_purchase_order/'.$purchase_order['o_id']); ?>"><button class="btn btn-sm btn-info">Edit</button></a>
                                <?php } ?>
                                <?php  if (in_array(5, $this->role)) { ?>    
                                    <button onclick="delete_row('<?php echo site_url('daily_purchase/delete_purchase_order/'.$purchase_order['o_id']); ?>')" class="btn btn-sm btn-danger">Delete</button>
                                <?php } ?>
                            <?php }else{ ?>
                                <?php  if (in_array(3, $this->role)) { ?>    
                           ...
                           <?php  if($user_type == 1){ ?>  
                                  <?php if($purchase_order['approve_status']=="Approved"){ ?>                             
                                        <button onclick="reject('<?php echo site_url('daily_purchase/reject_purchase_order/'.$purchase_order['o_id']); ?>')" class="btn btn-sm btn-warning">Reject</button>
                                  <?php }else if($purchase_order['approve_status']=="Rejected"){ ?>
                                        <button onclick="approve('<?php echo site_url('daily_purchase/approve_purchase_order/'.$purchase_order['o_id']); ?>')" class="btn btn-sm btn-primary">Approve</button>
                                   <?php }else{ ?>
                                        <button onclick="approve('<?php echo site_url('daily_purchase/approve_purchase_order/'.$purchase_order['o_id']); ?>')" class="btn btn-sm btn-primary">Approve</button>
                                        <button onclick="reject('<?php echo site_url('daily_purchase/reject_purchase_order/'.$purchase_order['o_id']); ?>')" class="btn btn-sm btn-warning">Reject</button>
                                   <?php } ?>
                          ...
                          <?php }else{ ?>   
                                    ...
                                    <?php  if($employee_id == $approver[0]) {  ?>
                                         <?php  if(!empty($approver[1]) ) { ?>   
                                                 <button onclick="approve('<?php echo site_url('daily_purchase/forward_purchase_order/'.$purchase_order['o_id']); ?>')" class="btn btn-sm btn-success">Forward</button>
                                                 <button onclick="reject('<?php echo site_url('daily_purchase/reject_purchase_order/'.$purchase_order['o_id']); ?>')" class="btn btn-sm btn-warning">Reject</button>
                                         <?php }else{ ?> 
                                                <button onclick="approve('<?php echo site_url('daily_purchase/approve_purchase_order/'.$purchase_order['o_id']); ?>')" class="btn btn-sm btn-primary">Approve</button>
                                                <button onclick="reject('<?php echo site_url('daily_purchase/reject_purchase_order/'.$purchase_order['o_id']); ?>')" class="btn btn-sm btn-warning">Reject</button>
