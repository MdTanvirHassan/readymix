...
class Sale_quotations extends Site_Controller {
    ...
    function index() {
      ...
      //  $sql="select q.*,c.c_name from tbl_sales_quotation q left join tbl_customers c on q.customer_id=c.id where q.is_active=1";
        $sql="select q.*,c.c_name from tbl_sales_quotation q left join tbl_customers c on q.customer_id=c.id where q.is_active=1 and q.unit_id=".$branch_id.' order by q.quotation_date desc';
     ...
     function add_quotation() {
        ...
        foreach($data['customers'] as $key=>$value){
            $costing_info=array();
            $costing_info=$this->m_common->get_row_array('tbl_product_quote_price',array('status'=>"Pending",'customer_id'=>$data['customers'][$key]['id']),'*');
