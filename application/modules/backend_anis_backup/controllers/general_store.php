        ...
        if($user_type==1 || $user_type==3){
           // $sql="select * from v_ipo_material_indent where department_id=".$companyId;
            if(!empty($f_date) && !empty($f_date)){
                ...
                $sql="select * from v_ipo_material_indent where department_id=$branch_id and status!='Rejected' and (date>='$f_date' and date<='$to_date') order by ipo_m_id DESC";
            }else{
                $sql="select * from v_ipo_material_indent where department_id=$branch_id and status!='Rejected' order by ipo_m_id DESC limit 20";
            }
        }else{
            if(!empty($f_date) && !empty($f_date)){
                $f_date=date('Y-m-d',strtotime($f_date));
                ...
                $sql="select * from v_ipo_material_indent where employeeId=".$employee_id." or approver_id=".$employee_id." and (date>='$f_date' and date<='$to_date') order by ipo_m_id DESC";
            }else{
                $sql="select * from v_ipo_material_indent where employeeId=".$employee_id." or approver_id=".$employee_id." order by ipo_m_id DESC limit 20";
            } 
        }
