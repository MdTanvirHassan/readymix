                    ...
                    if($report_type=="summary"){
                         if($item=="all"){
                          ...
                          foreach($data['items'] as $key=>$item_info){
                             $issue_qty=array();
                             if(!empty($c_c_id)){
                                $sql='Select sum(consumption_quantity) as issue_qty,sum(amount) as issue_value from tbl_item_comsumption where consumption_date>="'.$from_date.'" and consumption_date<="'.$too_date.'" and item_id='.$item_info['id'].' and status="Approved" and tic.unit_id='.$branch_id.' and c_c_id='.$c_c_id.' order by tic.consumption_date ';
                             }else{
                                $sql='Select sum(consumption_quantity) as issue_qty,sum(amount) as issue_value from tbl_item_comsumption where consumption_date>="'.$from_date.'" and consumption_date<="'.$too_date.'" and item_id='.$item_info['id'].' and status="Approved" and unit_id='.$branch_id; 
                             }   
                             $issue_qty=$this->m_common->customeQuery($sql);
                        ...
                        if($item=="all"){
                           //$sql='Select tic.*,i.item_name,tmu.meas_unit from tbl_item_comsumption tic left join items i on tic.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id where tic.consumption_date>="'.$from_date.'" and tic.consumption_date<="'.$too_date.'" and tic.status="Approved" and tic.unit_id='.$branch_id.' order by tic.consumption_date ';
                            ...
                            if(!empty($c_c_id)){
                                $sql='Select tic.*,i.item_name,tmu.meas_unit from tbl_item_comsumption tic left join items i on tic.item_id=i.id left join tbl_measurement_unit tmu on i.mu_id=tmu.id where tic.consumption_date>="'.$from_date.'" and tic.consumption_date<="'.$too_date.'" and tic.item_id='.$item.' and tic.status="Approved" and tic.unit_id='.$branch_id.' and c_c_id='.$c_c_id.' order by tic.consumption_date ';
                            }else{
