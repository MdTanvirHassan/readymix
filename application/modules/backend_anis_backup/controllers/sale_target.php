...
class Sale_target extends Site_Controller
{
    ...
    function achivement_()
    {
        ...
        $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity from tbl_sales_invoice_details dcd join tbl_sales_invoices dc on dcd.inv_id=dc.inv_id where dc.status!='Canceled' and dcd.amount>0 and dc.sale_invoice_date between '" . $start_date . "' and '" . $end_date . "'";
        ...
        foreach ($teams as $key => $team) {
            ...
            foreach ($target as $t => $each) {
                ...
                foreach ($cat as $c) {
                    ...
                    $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity from tbl_sales_invoice_details dcd join tbl_sales_invoices dc on dcd.inv_id=dc.inv_id join tbl_delivery_orders as do on dc.do_id=do.do_id join tbl_sales_orders as so on do.o_id=so.o_id join tbl_sales_products as sp on sp.product_id=dcd.s_item_id
                    where dc.status!='Canceled' and dcd.amount>0 and sp.category_id=" . $c['category_id'] . " and so.sale_person_id=" . $each['employee_id'] . " and dc.sale_invoice_date between '" . $start_date . "' and '" . $end_date . "'";
                    ...
                    $array[$team['id']]['target'] += $tgt[0]['target_qty'];
                    $array[$team['id']]['achive'] += $aciv[0]['quantity'];
                }
            }
        }
        $data['cat'] = $cat;
    ...
    function achivement()
    {
        ...
        $sql = "select mtc.*,st.team_name,e.name,d.designation_name,pc.category_name,pc.measurement_unit from member_target_category mtc left join employees as e on e.id=mtc.employee_id left join designation d on e.designation_id=d.id left join sales_team st on st.id=mtc.team_id  left join tbl_product_categories pc on pc.category_id=mtc.category_id where month='" . $month . "' order by mtc.team_id,e.id";
        ...
        foreach ($data['targets'] as $key => $tgt) {
            ...
            where sp.measurement_unit!='SQ.M' and dc.status!='Canceled' and dcd.amount>0 and so.sale_person_id in(" . $tgt['employee_id'] . ") and dc.sale_invoice_date between '" . $start_date . "' and '" . $end_date . "' and sp.category_id=" . $tgt['category_id'];
            $aciv = $this->m_common->customeQuery($sql);
    ...
    function achivement_print_($month)
    {
        $this->setOutputMode(NORMAL);
        $branch_id = $this->session->userdata('companyId');
        $user_id = $this->session->userdata('user_id');
        ...
        foreach ($teams as $key => $team) {
            ...
            foreach ($target as $t => $each) {
                ...
                foreach ($cat as $c) {
                    ...
                    where dc.status!='Canceled' and dcd.amount>0 and dcd.s_item_id=" . $c['category_id'] . " and so.sale_person_id=" . $each['employee_id'] . " and dc.sale_invoice_date between '" . $start_date . "' and '" . $end_date . "'";
                    $aciv = $this->m_common->customeQuery($sql);
    ...
    function provide_incentive()
    {
        ...
        $data['cat'] = $this->m_common->get_row_array('tbl_product_categories', '', '*');
        if (!empty($postData['category'])) {
            $category = $postData['category'];
        } else {
            $category = $data['cat'][0]['category_id'];
        }

        $data['cat_name'] = $this->m_common->get_row_array('tbl_product_categories', array('category_id'=>$category), '*');
        $data['company_target'] = $this->m_common->customeQuery("SELECT st.month,std.* FROM tbl_sales_target st join tbl_sales_target_details std on st.st_id=std.st_id where st.month='".$month."' and std.category_id=$category");
        $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity from tbl_sales_invoice_details dcd join tbl_sales_invoices dc on dcd.inv_id=dc.inv_id join tbl_sales_products as sp on sp.product_id=dcd.s_item_id where dc.status!='Canceled' and dcd.amount>0 and dc.sale_invoice_date between '" . $start_date . "' and '" . $end_date . "' and sp.category_id=".$category;
        $data['company_achive'] = $this->m_common->customeQuery($sql);


        $sql = "select mtc.*,st.team_name,e.name,d.designation_name,pc.category_name,pc.measurement_unit from member_target_category mtc left join employees as e on e.id=mtc.employee_id left join designation d on e.designation_id=d.id left join sales_team st on st.id=mtc.team_id  left join tbl_product_categories pc on pc.category_id=mtc.category_id where month='" . $month . "' and pc.category_id=$category order by mtc.team_id,e.id";
        $data['targets'] = $this->m_common->customeQuery($sql);
        foreach ($data['targets'] as $key => $tgt) {
            $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity,sum(dcd.received_amount) as paid,
            group_concat(DISTINCT so.customer_id) as customer_id 
            from tbl_sales_invoice_details dcd 
                        join tbl_sales_invoices dc on dcd.inv_id=dc.inv_id
            join tbl_delivery_orders as do on dc.do_id=do.do_id
            ...
            $data['targets'][$key]['achive_total'] = !empty($aciv) ? $aciv[0]['total'] : 0;
            $data['targets'][$key]['achive_quantity'] = !empty($aciv) ? $aciv[0]['quantity'] : 0;
            $data['targets'][$key]['paid'] = !empty($aciv) ? $aciv[0]['paid'] : 0;
        }
        $teams = array();
        foreach ($data['targets'] as $key => $tgt) {
            $teams[$tgt['team_name']][] = $tgt;
        }
        $data['ins'] = $this->m_common->customeQuery('select * from sales_incentive where product_id=' . $category . ' order by start_amount asc limit 1');
        $data['teams'] = $teams;
        $this->load->view('sale_target/v_provide_incentive', $data);
    }
    function provide_incentive__()
    {
        $branch_id = $this->session->userdata('companyId');
        $user_id = $this->session->userdata('user_id');
        $user_type = $this->session->userdata('user_type');
    ...
    function print_provide_incentive($category,$month)
    {
        $this->setOutputMode(NORMAL);
        $branch_id = $this->session->userdata('companyId');
        $user_id = $this->session->userdata('user_id');
    ...
    function print_provide_incentive_($month)
    {
        $this->setOutputMode(NORMAL);
        $branch_id = $this->session->userdata('companyId');
        $user_id = $this->session->userdata('user_id');
    ...
    function provide_incentive_()
    {
        ...
        $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity from tbl_sales_invoice_details dcd 
                    join tbl_sales_invoices dc on dcd.inv_id=dc.inv_id where dc.status!='Canceled' and dcd.amount>0 and dc.sale_invoice_date between '" . $start_date . "' and '" . $end_date . "'";
        $data['company_achive'] = $this->m_common->customeQuery($sql);
        $data['company_target'] = $this->m_common->get_row_array('tbl_sales_target', array('month' => $month), '*');
        ...
        foreach ($sales_team as $key => $team) {
            ...
            $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity,group_concat(DISTINCT so.customer_id) as customer_id from tbl_sales_invoice_details dcd 
                    ...
                    join tbl_delivery_orders as do on dc.do_id=do.do_id
                    join tbl_sales_orders as so on do.o_id=so.o_id
                    where dc.status!='Canceled' and dcd.amount>0 and so.sale_person_id in(" . $sales_persons[0]['sales_person'] . ") and dc.sale_invoice_date between '" . $start_date . "' and '" . $end_date . "'";
            $aciv = $this->m_common->customeQuery($sql);
    ...
    function provide_incentive_employee($month, $team_id = 0)
    {
        $branch_id = $this->session->userdata('companyId');
        $user_id = $this->session->userdata('user_id');
        $user_type = $this->session->userdata('user_type');
        ...
        foreach ($sales_team as $key => $team) {
            ...
            $sql = "select sum(dcd.amount) as total, sum(dcd.quantity) as quantity,group_concat(DISTINCT so.customer_id) as customer_id from tbl_sales_invoice_details dcd 
                    ...
                    join tbl_delivery_orders as do on dc.do_id=do.do_id
                    join tbl_sales_orders as so on do.o_id=so.o_id
                    where dc.status!='Canceled' and dcd.amount>0 and so.sale_person_id in(" . $team['employee_id'] . ") and dc.sale_invoice_date between '" . $start_date . "' and '" . $end_date . "'";
            $aciv = $this->m_common->customeQuery($sql);
            if (!empty($aciv[0]['customer_id'])) {
                $sql = "select sum(amount) as total from tbl_payment_collections where customer_id in (" . $aciv[0]['customer_id'] . ") and payment_status='Received'  and receive_date between '" . $start_date . "' and '" . $end_date . "'";
                $collection = $this->m_common->customeQuery($sql);
    ...
    function employee_achievement($month, $employee_id = false)
    {
        ...
        $sql = "select dc.inv_id,dc.inv_no,dc.sale_invoice_date,dcd.mu_name,sum(dcd.quantity) as quantity,sum(dcd.amount) as amount,dcd.unit_price,do.delivery_no,so.order_no,sp.product_name,c.c_name as c_short_name 
            from tbl_sales_invoice_details dcd 
                        join tbl_sales_invoices dc on dcd.inv_id=dc.inv_id
            join tbl_delivery_orders as do on dc.do_id=do.do_id
            join tbl_sales_orders as so on do.o_id=so.o_id
            ...
            where sp.measurement_unit!='SQ.M' and dc.status!='Canceled' and dcd.amount>0 and so.sale_person_id in(" . $employee_id . ") and dc.sale_invoice_date between '" . $start_date . "' and '" . $end_date . "' group by dc.inv_id";
