...
class Rm_receive extends Site_Controller {
    ...
    function index() {
        ...
        $this->load->view('raw_materials/rm_receive/v_rm_receive',$data);
    }
    ...
    //    $this->sub_menu = 'rm_receive';
       ...
       $mrr_last_code=$this->m_common->get_row_array('rm_receive_code',array('branch_id'=>$branch_id),'*','',1,'id','DESC');
       if(!empty($mrr_last_code)){
        ...
        $this->load->view('raw_materials/rm_receive/v_add_rm_receive',$data);
    }
    
    function add_action_rm_receive(){
        $this->menu = 'general_store';
        $this->sub_menu = 'rm_receive';
        $this->titlebackend("Add Material Receive Requisition");
        $branch_id= $this->session->userdata('companyId');
        ...
        if (!empty($postData)) {
           ...
           // $insertData['unit_id'] =$branch_id;
            ...
            if(empty($postData['item_select'])){
                 redirect_with_msg('raw_materials/rm_receive/add_rm_receive', 'Please Select Item');
            }
               ...
               //  $this->m_common->insert_row('material_receive_code', array('mrr_code'=>$mrr_code));
                ...
                foreach ($postData['item_id'] as $key => $each) {
                      if(in_array($key+1,$postData['item_select'])){
                                ...
                                $insertData1['inv_qty'] = $postData['inv_qty'][$key];
                ...
                redirect_with_msg('raw_materials/rm_receive/rm_receive', 'Successfully  Added Material Receive Requistion');
            } else {
                redirect_with_msg('raw_materials/rm_receive/add_rm_receive', 'Data not saved for an unexpected error');
            }
        } else {
            redirect_with_msg('raw_materials/rm_receive/add_rm_receive', 'Please fill the form and submit');
        }
        
    }
    
function edit_rm_receive($id) {
    $this->menu = 'general_store';
    $this->sub_inner_menu = 'rm_receive';
    $user_type = $this->session->userdata('user_type');
    $branch_id= $this->session->userdata('companyId');
    ...
    $sql="select rmrd.*,rmi.item_code,rmi.item_name,rmi.origin,rmi.item_grade,tmu.meas_unit from rm_receive_details rmrd left join rm_receive rmr on rmrd.mrr_id=rmr.mrr_id left join rm_items rmi on rmrd.item_id=rmi.id left join tbl_measurement_unit tmu on rmi.mu_id=tmu.id  where rmrd.mrr_id=".$id;
    $data['receive_items']=$this->m_common->customeQuery($sql);

    $this->load->view('raw_materials/rm_receive/v_edit_rm_receive',$data);
}
    
    
function details_rm_receive($id,$print=false) {
        $this->menu = 'general_store';
        $this->sub_inner_menu = 'rm_receive';
        $user_type = $this->session->userdata('user_type');
        $branch_id= $this->session->userdata('companyId');
        ...
        if($print==false){
            $this->load->view('raw_materials/rm_receive/v_details_rm_receive',$data);
        }else{
            $html=$this->load->view('raw_materials/rm_receive/print_mrr',$data,true);
            echo $html;
            exit; 
        }
    }
...
function edit_action_rm_receive($id) {
        $this->menu = 'general_store';
        $this->sub_menu = 'rm_receive';
        $this->titlebackend("Edit Material Receive Requisition");
        $branch_id= $this->session->userdata('companyId');
        ...
        if(!empty($postData)){
           ...
           // $insertData['unit_id'] =$branch_info[0]['unit_id'];
            ...
            if ($s_id>=0) {
                 ...
                 redirect_with_msg('raw_materials/rm_receive/details_rm_receive/'.$id, 'Successfully Updated Material Receive Requisition');
              
            }
        } else {
            redirect_with_msg('raw_materials/rm_receive/edit_rm_receive/'.$id, 'Please fill the form and submit');
        }
    }
    
    function delete_rm_receive($id) {
        $this->menu = 'general_store';
        $this->sub_menu = 'rm_receive';
        $this->titlebackend("Material Receive Requisition");
         if (!empty($id)) {  
            ...
            if(!empty($ids)){             
                ...
                redirect_with_msg('raw_materials/rm_receive', 'Successfully Deleted');
            } else {
                redirect_with_msg('raw_materials/rm_receive', 'Data not deleted for an unexpected error');
            }
        } else {
            redirect_with_msg('raw_materials/rm_receive', 'Please click on delete button');
        }
    }
    ...
    function confirmReceive($id){
        ...
        if(!empty($result)){
            foreach($receive_items as $item){
                ...
                $lot_item=$this->m_common->get_row_array('rm_lots_details',array('id'=>$item['lot_d_id']),'*');
                $receive_quanity=$lot_item[0]['final_receive_qty']+$item['receive_qty'];
                if($receive_quanity==$lot_item[0]['accepted_weight']){
                    $status="Received";
                }else{
                     $status="Partially Received";
                }
                $this->m_common->update_row('rm_lots_details',array('id'=>$item['lot_d_id']),array('final_receive_qty'=>$receive_quanity,'receive_status'=>$status));
            }
        }
        
        
        $lot_items=$this->m_common->get_row_array('rm_lots_details',array('lot_id'=>$receive_items[0]['lot_id']),'*');
        $j=0;
        foreach($lot_items as $p_item){
        ...
        if($j==1){
            $this->m_common->update_row('rm_lots',array('id'=>$receive_items[0]['lot_id']),array('status'=>"Partially Received"));
        }else{
            $this->m_common->update_row('rm_lots',array('id'=>$receive_items[0]['lot_id']),array('status'=>"Received"));
        }
       redirect_with_msg('raw_materials/rm_receive', 'Successfully Received Items');
    }
