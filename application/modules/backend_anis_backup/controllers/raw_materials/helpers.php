...
class Helpers extends Site_Controller {
    ...
    function index() {
        ...
        $this->load->view('raw_materials/helper/v_helper',$data);
    }
    ...
    function add_helper() {
        ...
        $this->load->view('raw_materials/helper/v_add_helper');
    }
         ...
         if(!empty($postData)){
            ...
            if(!empty($postData['blood_group'])){
             ...
             if(!empty($result)){
                  ...
                  redirect_with_msg('raw_materials/helpers', 'Successfully Inserted');
             }
         }else{
              redirect_with_msg('raw_materials/helpers/add_helper', 'Please fill the form and submit');
         }
    ...
    function edit_helper($id) {
        ...
        $this->load->view('raw_materials/helper/v_edit_helper',$data);
    }
    ...
    function edit_helper_action($id) {
         ...
         if(!empty($postData)){
           ...
           // }
             ...
             if($result>=0){              
                  redirect_with_msg('raw_materials/helpers', 'Successfully Updated');
             }
         }else{
              redirect_with_msg('raw_materials/helpers', 'Please fill the form and submit');
         }
    ...
    function details_helper($id,$print=false) {
        ...
        $data['helper_info']=$this->m_common->get_row_array('tbl_helper',array('helper_id'=>$id),'*');      
        $this->load->view('raw_materials/helper/v_details_helper',$data);
    ...
    function delete_helper($id) {
        ...
        if(!empty($id)) {
            ...
            if(!empty($id)) {
                redirect_with_msg('raw_materials/helpers/index', 'Successfully Deleted');
            } else {
                redirect_with_msg('raw_materials/helpers/index', 'Data not deleted for an unexpected error');
            }
        } else {
            redirect_with_msg('raw_materials/helpers/index', 'Please click on delete button');
        }
    }
