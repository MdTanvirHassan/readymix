...
class Accounts extends Site_Controller {
    ...
    function add_payment_action() {
         ...
         if(!empty($postData)){
             ...
             if(!empty($postData['inv_id'])){
                 $insertData['inv_id']=$postData['inv_id'];
             }else{
                 redirect_with_msg('accounts/add_payment', 'Please select invoice');
             }
             
             //$payment_condition=$this->m_common->get_row_array('tbl_sales_order_payment_condition',array('o_id'=>$postData['o_id']),'*');
             $order_info=$this->m_common->get_row_array('tbl_purchase_invoices',array('inv_id'=>$postData['inv_id']),'*');
             $insertData['o_id']=$order_info[0]['po_id'];
            ...
            if(!empty($postData['payment_type'])){
                 $insertData['payment_type']=$postData['payment_type'];
             }   
