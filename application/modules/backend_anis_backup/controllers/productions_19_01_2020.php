...
class Productions extends Site_Controller {
    ...
    function index() {
        ...
        $sql = "select s.*,(select GROUP_CONCAT(delivery_no) from tbl_delivery_orders d join tbl_production_schedule_details pd on d.do_id=pd.do_id where pd.schedule_id=s.id) as delivery_no from tbl_production_schedule s WHERE unit_id=$branch_id and is_active=1 ORDER BY id DESC";
    ...
    function delivery_orders() {
        ...
        $sql = "select d.*,so.order_no,c.c_name from tbl_delivery_orders d left join tbl_sales_orders so on d.o_id=so.o_id left join tbl_sales_quotation q on so.q_id=q.q_id left join tbl_customers c on so.customer_id=c.id where d.is_active=1 and d.unit_id=" . $branch_id . ' and d.do_status="Approved" order by d.do_id DESC';

        $data['delivery_orders'] = $this->m_common->customeQuery($sql);
        $this->load->view('productions/v_delivery_order', $data);
    }
...
function details_delivery_order($id,$print=false) {
    ...
    function production_statement() {
        ...
        $sql = "select s.*,tps.schedule_no,(select GROUP_CONCAT(delivery_no) from tbl_delivery_orders d join tbl_production_statement_details pd on d.do_id=pd.do_id where pd.pst_id=s.pst_id) as delivery_no from tbl_production_statement s join tbl_production_schedule tps on s.schedule_id=tps.id WHERE s.unit_id=$branch_id and s.is_active=1 ORDER BY s.pst_id DESC";
...
where (sd.production_status='Pending' or sd.production_status='Partial Done') GROUP BY s.id";
...
where (sd.production_status='Pending' or sd.production_status='Partial Done') and sd.schedule_id=" . $postData['schedule_id'] . "";
    ...
    function details_production_statement($id, $print = '') {
        ...
        $this->titlebackend("Production Schedule Information");
        $data['production_statement_info'] = $this->m_common->get_row_array('tbl_production_statement', array('pst_id' => $id), '*');
        $sql = "select d.*,do.delivery_no,do.delivery_order_date,do.project_name,c.c_name,c.c_short_name,sp.product_name,sp.measurement_unit from tbl_production_statement_details d left join tbl_delivery_orders do on d.do_id=do.do_id left join tbl_sales_products sp on d.product_id=sp.product_id left join tbl_sales_orders so on do.o_id=so.o_id left join  tbl_sales_quotation q on so.q_id=q.q_id left join  tbl_customers c on q.customer_id=c.id where d.is_active=1 and d.pst_id=" . $id;
        $data['production_details'] = $this->m_common->customeQuery($sql);
        $data['production_schedules'] = $this->m_common->get_row_array('tbl_production_schedule', array('unit_id' => $branch_id, 'is_active' => 1, 'id' => $data['production_statement_info'][0]['schedule_id']), '*');
