...
class Sales_report extends Site_Controller {
    ...
    function customerLedgerReport(){
       ...
       // $data['customers']=$this->m_common->get_row_array('tbl_customers',array('is_active'=>1),'*');
        ...
        $sql="select tsi.*,tc.c_name from  tbl_sales_invoices as tsi left join tbl_customers tc on tsi.customer_id=tc.id  where tsi.is_active=1 group by tsi.customer_id";
    ...
    function receivableSummaryReportExcell($print = false) {
     ...
     //   $object->getActiveSheet()->setCellValueByColumnAndRow(8, 2, $data['branch_info'][0]['dep_description']);   
       ...
       // $object->getActiveSheet()->setCellValueByColumnAndRow(8, 3, 'Invoice Aging Report');
        ...
        $table_columns = array("SL","Customer Name","Product Type","Value","Age Days");
...
//                    $sub_total=$sub_total+$order['total_amount'];
                    ...
                    $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, number_format($order['total_amount'], 2));                   
                    $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, $dif_day);
                    $excel_row++;
                }
                
                $object->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row,'Sub Total');
                
                $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, number_format($sub_total, 2));
                $object->getActiveSheet()->getStyle('H'.$excel_row)->getFont()->setBold(true);
                $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row,'');
        ...
        $object->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row, 'Grand Total');
       
        $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, number_format($grand_total, 2));
        $object->getActiveSheet()->getStyle('H'.$excel_row)->getFont()->setBold(true);
        $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row,'');
        ...
        $object->getActiveSheet()->getStyle('F5:J' . $excel_row)->applyFromArray($styleArray);
...
//        $w=numberToWord($ye);
    ...
    //    $sql = "select pc.*,c.c_name,c_short_name,b.b_name,b.b_short_name,b.branch_name from tbl_payment_collections pc left join tbl_banks b on pc.bank_id=b.id  left join tbl_customers c on pc.customer_id=c.id  where pc.is_active=1 and pc.payment_status='Collected' and (collection_method='Pdc' or collection_method='Lc' or collection_method='Bg' or collection_method='Po') ";
        ...
        if(!empty($customer_id)){
            $data['all_customers']=$this->m_common->get_row_array('tbl_customers',array('id'=>$customer_id),'*');
        }else{
            $data['all_customers']=$this->m_common->get_row_array('tbl_customers','','*');
        }
...
//            }
    ...
    function realizedChequeExcel($print = false) {
     ...
     //   $table_columns = array("SL", "S. Order", "Customer Name.", "Project", "Pdc/Lc/Bg No.", "Pdc/Lc/Bg Date", "Mat Date", "Bank", "Value");
        $table_columns = array("SL", "Customer Name.","Product Type","Collection Method","Pdc/Lc/Bg No.", "Deposit Date","Honor Date","Bank", "Value");
    ...
    function dishonoredChequeAtHand($print = false){
        ...
        if (!empty($postData)) {
            ...
            if (!empty($customer_id)) {
             ...
             if(!empty($f_date) & !empty($to_date)){              
               //   $sql = "select dr.*,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name from tbl_deposit_realization dr left join tbl_payment_collections pc on dr.collection_id=pc.id left join tbl_banks b on dr.bank_id=b.id left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and dr.realization_date>='" . $f_date . "' and dr.realization_date<='" . $to_date . "' and dr.realization_status='Dishonored' order by dr.deposit_date DESC";
                 $sql = "select pc.*,tc.c_name,c_short_name,tpc.category_name from tbl_payment_collections pc left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and pc.dishonor_date>='" . $f_date . "' and pc.dishonor_date<='" . $to_date . "' and pc.payment_status='Dishonored' order by pc.dishonor_date DESC";
             }else if (!empty($f_date)) {              
                //  $sql = "select dr.*,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name from tbl_deposit_realization dr left join tbl_payment_collections pc on dr.collection_id=pc.id left join tbl_banks b on dr.bank_id=b.id left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and dr.realization_date>='".$f_date."' and dr.realization_status='Dishonored' order by dr.deposit_date DESC";
                $sql = "select pc.*,tc.c_name,c_short_name,tpc.category_name from tbl_payment_collections pc left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and pc.dishonor_date>='" . $f_date . "' and pc.payment_status='Dishonored' order by pc.dishonor_date DESC";
             }else if (!empty($to_date)) {               
                  //$sql = "select dr.*,pc.collection_method,pc.collection_method,pc.amount,pc.no,b.b_name,b.b_short_name,b.branch_name,tc.c_name,c_short_name,tpc.category_name from tbl_deposit_realization dr left join tbl_payment_collections pc on dr.collection_id=pc.id left join tbl_banks b on dr.bank_id=b.id left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and dr.realization_date<='" . $to_date . "' and dr.realization_status='Dishonored' order by dr.deposit_date DESC";
                 $sql = "select pc.*,tc.c_name,c_short_name,tpc.category_name from tbl_payment_collections pc left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and pc.dishonor_date<='" . $to_date . "' and pc.payment_status='Dishonored' order by pc.dishonor_date DESC";
             }else{                
                  $sql = "select pc.*,tc.c_name,c_short_name,tpc.category_name from tbl_payment_collections pc left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and pc.payment_status='Dishonored' order by pc.dishonor_date DESC";
             }   
        }else{
            $sql = "select pc.*,tc.c_name,c_short_name,tpc.category_name from tbl_payment_collections pc left join tbl_sales_orders tso on pc.o_id=tso.o_id left join tbl_customers tc on pc.customer_id=tc.id left join tbl_product_categories tpc on pc.payment_for_id=tpc.category_id where $where and pc.payment_status='Dishonored' order by pc.dishonor_date DESC";
        }
   ...
   function dishonoredChequeAtHandExcel($print = false) {
     ...
     //   $table_columns = array("SL", "S. Order", "Customer Name.", "Project", "Pdc/Lc/Bg No.", "Pdc/Lc/Bg Date", "Mat Date", "Bank", "Value");
        $table_columns = array("SL", "Customer Name.","Product Type","Collection Method","Pdc/Lc/Bg No.","Dishonor Date","Value");
        ...
        if (!empty($data['orders'])) {
            ...
            foreach ($data['orders'] as $order) {
                ...
                $object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, $order['dishonor_date']);
                
                
                $object->getActiveSheet()->setCellValueByColumnAndRow(11, $excel_row, round($order['amount'], 2));
        ...
        $object->getActiveSheet()->getStyle('F5:L'.$excel_row)->applyFromArray($styleArray);
        $object->getActiveSheet()->getStyle('L6:L'.$excel_row)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
