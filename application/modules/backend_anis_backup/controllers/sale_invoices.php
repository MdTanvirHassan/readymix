    ...
    function index($cust_id=false){
      ...
      //  $sql = "select v.*,do.delivery_no,c.c_name,c.c_short_name,c.deposit,c.id as customer_id,tpc.category_name,tp.project_name as tp_project_name from tbl_sales_invoices v left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on v.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id left join tbl_project tp on v.project_id=tp.project_id where v.status!='Canceled' and v.is_active=1 and v.unit_id=" . $branch_id . ' order by v.sale_invoice_date desc'; //27-09-2020
        ...
        //  $sql = "select v.*,o.order_no,do.delivery_no,c.c_name,c.c_short_name,c.deposit,c.id as customer_id,tpc.category_name,tp.project_name as tp_project_name,sum(tsid.quantity) as total_qty,tsid.unit_price,tsid.mu_name from tbl_sales_invoices v left join tbl_sales_invoice_details tsid on v.inv_id=tsid.inv_id left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on v.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id left join tbl_project tp on v.project_id=tp.project_id where v.status!='Canceled' and v.is_active=1 and v.unit_id=" . $branch_id . " and v.customer_id=" .$cust_id.' group by v.inv_id order by v.sale_invoice_date desc'; 
            $sql = "select v.*,o.order_no,do.delivery_no,c.c_name,c.c_short_name,c.deposit,c.id as customer_id,tpc.category_name,tp.project_name as tp_project_name,sum(tsid.quantity) as total_qty,tsid.unit_price,tsid.mu_name from tbl_sales_invoices v left join tbl_sales_invoice_details tsid on v.inv_id=tsid.inv_id left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on v.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id left join tbl_project tp on v.project_id=tp.project_id where v.status!='Canceled' and v.status!='Received'  and v.is_active=1  and v.customer_id=" .$cust_id.' group by v.inv_id order by v.sale_invoice_date desc';
          $data['invoices'] = $this->m_common->customeQuery($sql);
       }else{
           $sql = "select v.*,o.order_no,do.delivery_no,c.c_name,c.c_short_name,c.deposit,c.id as customer_id,tpc.category_name,tp.project_name as tp_project_name,sum(tsid.quantity) as total_qty,tsid.unit_price,tsid.mu_name from tbl_sales_invoices v left join tbl_sales_invoice_details tsid on v.inv_id=tsid.inv_id left join tbl_delivery_orders do on v.do_id=do.do_id left join tbl_sales_orders o on do.o_id=o.o_id left join tbl_sales_quotation q on o.q_id=q.q_id left join tbl_customers c on v.customer_id=c.id left join tbl_product_categories tpc on v.category_id=tpc.category_id left join tbl_project tp on v.project_id=tp.project_id where v.status!='Canceled' and v.status!='Received' and v.is_active=1 group by v.inv_id order by v.sale_invoice_date desc";        
           $data['invoices'] = $this->m_common->customeQuery($sql);
       }
