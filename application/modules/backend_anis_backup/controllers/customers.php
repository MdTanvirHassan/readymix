...
class Customers extends Site_Controller {
    ...
    function index() {
        $this->menu = 'sales';
        $this->sub_menu = 'sale';
        $this->sub_inner_menu = 'customers';
        $this->titlebackend("Customers Basic Info");
        $data['customers']=$this->m_common->get_row_array('tbl_customers',array('is_active'=>1,'customer_category'=>'Finish Good'),'*');
        $this->load->view('customers/v_customers',$data);
    }
    function opening_balance() {
        ...
        $this->load->view('customers/v_opening_balance',$data);
    }
    ...
    function add_customer() {
        ...
        $this->load->view('customers/v_add_customer',$data);
    }
    ...
    function add_customer_action() {
        ...
        if (!empty($data)) {
            ...
            if(!empty($pre_customer)){
                redirect_with_msg('customers/add_customer', 'This customer already exists.');
            }
            $data['is_active']=1;
            $data['customer_category']="Finish Good";
            ...
            if (!empty($id)) {
                ...
                redirect_with_msg('customers/add_customer', 'Successfully Inserted');
            } else {
                redirect_with_msg('customers/add_customer', 'Data not saved for an unexpected error');
            }
        } else {
            redirect_with_msg('customers/add_customer', 'Please fill the form and submit');
        }
    }
    ...
    function edit_customer($id){
        ...
        $this->sub_inner_menu = 'customers';
        $this->titlebackend("Customers Basic Info");
        $data['customer_info']=$this->m_common->get_row_array('tbl_customers',array('id'=>$id),'*');
        $this->load->view('customers/v_edit_customer',$data);
    }
    ...
    function edit_customer_action($id) {
        ...
        if (!empty($data)) {
            ...
            if (!empty($id)) {
                redirect_with_msg('customers/index', 'Successfully Updated');
            } else {
                redirect_with_msg('customers/edit_customer/'.$id, 'Data not updated for an unexpected error');
            }
        } else {
            redirect_with_msg('customers/edit_customer/'.$id, 'Please fill the form and submit');
        }
    }
...
where so.customer_id=".$id;
       ...
       // $pc_sql = "select pc.*,c.c_name,c_short_name from tbl_payment_collections pc left join tbl_customers c on pc.customer_id=c.id where pc.is_active=1 and pc.payment_status='Collected' order by pc.id desc";
        ...
        //$this->load->view('customers/v_details_customer',$data);
        $this->load->view('customers/v_details_customer_new',$data);
    }
     ...
     function delete_customer($id) {
        if (!empty($id)) {
            ...
            if (!empty($id)) {
                redirect_with_msg('customers/index', 'Successfully Deleted');
            } else {
                redirect_with_msg('customers/index', 'Data not deleted for an unexpected error');
            }
        } else {
            redirect_with_msg('customers/index', 'Please click on delete button');
        }
    }
