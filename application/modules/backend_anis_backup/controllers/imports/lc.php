...
class Lc extends Site_Controller
{
    ...
    function index()
    {
       ...
       // $sql = "select sl.*,c.SUP_NAME as buyer_name,b.b_name as buyer_bank,b1.b_name as our_bank from import_lc sl  left join supplier as c on c.ID=sl.sup_id join tbl_banks b on b.id=sl.party_bank join tbl_banks b1 on b1.id=sl.our_bank where sl.branch_id=$branch_id order by lc_id DESC";
        ...
        $this->load->view('imports/lc/v_lc', $data);
    }
    ...
    function add_action_lc()
    {
        ...
        if(!empty($postData)){
            ...
            if(empty($postData['pi_d_id'])){
               redirect_with_msg('imports/lc/add_lc', 'Please select at least one item');       
            }
            ...
            if (!empty($postData['total_amount'])) {
                $insertData['amount'] = $postData['total_amount'];
            }
            ...
            $approver = fetch_approver(18,110, $userData);
            if (empty($approver[0])) {
    ...
    function detailsLc($lc_id)
    {
       ...
       // $data['suppliers'] = $this->m_common->get_row_array('supplier',array('LOCAL'=>"Foreign"), '*');
        ...
        where iscd.lc_id=".$lc_id;
        $data['lc_details']=$this->m_common->customeQuery($sql);
        $this->load->view('imports/lc/details_lc',$data);
