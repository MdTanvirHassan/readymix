...
class Money_indent extends Site_Controller {
    ...
    function index() {
     ...
     //   $data['money_indents']=$this->m_common->get_row_array('tbl_money_indent',array('is_active'=>1),'*');
        $sql="select * from tbl_money_indent where status!='Rejected' order by mi_id DESC";
...
//             }
   ...
   function delete_money_indent($id) {
        if(!empty($id)) {
            ...
            $id = $this->m_common->update_row('tbl_money_indent', array('mi_id' => $id),array('is_active'=>0));
            if(!empty($id)) {
