            ...
            $item_brands=  unserialize($item['brand_id']);
             foreach($brands as $key1=>$brand){
                if(in_array($brand['id'],$item_brands)){
                    ...
                    $data['items'][$key]['item_brands'].=$brand['brand_name'];
                }
            }
            reset($brands);
           ...
           // $data['stock_amount'] = $this->input->post('opening_value');
            ...
            if (!empty($ids) || $ids==0) {
                 if(empty($pre_opening_stock_info)){
                    $this->m_common->insert_row('tbl_item_opening_stock',array('unit_id'=>$branch_id,'item_id'=>$id,'opening_stock'=>$opeining_value));
                }else{
...
//                exit;
     ...
     //                         }
                               ...
                               if(!empty($postData['a_remark'][$key])){
                                    $insertData1['remark'] = $postData['a_remark'][$key];
                               }
                               $this->m_common->insert_row('ipo_material_indent_details', $insertData1); 
                          }
                     }
    ...
    function reject_budget($id) {
        ...
        $approver_data = fetch_approver(2, 8, $approvers_info); //
         ...
         if($user_type ==1){
                    ...
                    $this->m_common->update_row('budget', array('b_id' => $id), array('b_approve_status' => 'Rejected', 'approver_id' =>'', 'approver_name' => $approver_name,'approved_by'=>$employee_id));
                    $array = array(
 ...
 function receive_material_receive_requisition($id) {
        ...
        if(!empty($result)){
            foreach($receive_items as $item){
                ...
                $purchase_order_item=$this->m_common->get_row_array('tbl_purchase_order_details',array('o_id'=>$material_receive_info[0]['po_id'],'item_id'=>$item['item_id']),'*');
                ...
                $this->m_common->update_row('tbl_purchase_order_details',array('o_id'=>$material_receive_info[0]['po_id'],'item_id'=>$item['item_id']),array('receive_quantity'=>$receive_quanity,'receive_status'=>$status));
            }
        }
         ...
         function add_action_cost_center() {
                ...
                if (!empty($data)) {
                   // $data['created'] = date('Y-m-d');
                    ...
                    if (!empty($id)) {
                        redirect_with_msg('general_store/add_cost_center', 'Successfully Added Cost Center');
                    } else {
