...
class Mahana_messaging
{
    ...
    function get_message($msg_id, $user_group)
    {
        ...
        if ($message = $this->ci->mahana_model->get_message($msg_id, $user_group))
        {
    ...
    function get_full_thread($thread_id, $user_group, $full_thread = FALSE, $order_by = 'ASC')
    {
        ...
        if ($message = $this->ci->mahana_model->get_full_thread($thread_id, $user_group, $full_thread, $order_by))
        {
            return $this->_success($message);
        }
    ...
    function get_all_threads($user_group, $full_thread = FALSE, $order_by = 'ASC')
    {
        ...
        if ($message = $this->ci->mahana_model->get_all_threads($user_group, $full_thread, $order_by))
        {
            return $this->_success($message);
        }
     ...
     * @return  array
     */
    function get_all_threads_grouped($user_group, $full_thread = FALSE, $order_by = 'ASC')
    {
        if (empty($user_group))
        {
            return $this->_invalid_id(MSG_ERR_INVALID_USER_ID);
        ...
        if ($message = $this->ci->mahana_model->get_all_threads($user_group, $full_thread, $order_by))
        {
            ...
            foreach ($message as $msg)
            {
                if ( ! isset($threads[$msg['thread_id']]))
                {
                    ...
                    $threads[$msg['thread_id']]['name'] = $msg['name'];
                    $threads[$msg['thread_id']]['recipient_name'] = $msg['recipient_name'];
                    $threads[$msg['thread_id']]['subject'] = $msg['subject'];
                    $threads[$msg['thread_id']]['cdate'] = $msg['cdate'];
    ...
    function update_message_status($msg_id, $user_group, $status_id )
    {
        ...
        if ($this->ci->mahana_model->update_message_status($msg_id, $user_group, $status_id))
        {
    ...
    function add_participant($thread_id, $user_group)
    {
        ...
        if ( ! $this->ci->mahana_model->valid_new_participant($thread_id, $user_group))
        {
            $this->_particpant_error(MSG_ERR_PARTICIPANT_EXISTS);
        }

        if ( ! $this->ci->mahana_model->application_user($user_group))
        {
            $this->_particpant_error(MSG_ERR_PARTICIPANT_NONSYSTEM);
        }

        if ($this->ci->mahana_model->add_participant($thread_id, $user_group ))
        {
    ...
    function remove_participant($thread_id, $user_group)
    {
        ...
        if ($this->ci->mahana_model->remove_participant($thread_id, $user_group))
        {
     ...
     * @return  array
     */
    function reply_to_message($msg_id, $sender_id, $subject = '', $body = '', $recipient_id, $priority = PRIORITY_NORMAL)
    {
        if (empty($sender_id))
        {
            return $this->_invalid_id(MSG_ERR_INVALID_SENDER_ID);
        ...
        if ($this->ci->mahana_model->reply_to_message($msg_id, $sender_id, $body, $recipient_id, $priority))
        {
    ...
    function get_msg_count($where, $status_id = MSG_STATUS_UNREAD)
    {
        ...
        if ($message = $this->ci->mahana_model->get_msg_count($where, $status_id))
        {
            return $this->_success($message);
        }
